...
public:
    ...
    cache::NodeCacheKey cache_key(const RenderGraphContext& ctx) const override {
        return cache::NodeCacheKey{
            .scope = "precomp",
            .frame = m_cache_frame >= 0 ? m_cache_frame : (frame_dependent() ? (ctx.frame.frame - m_start_frame) : Frame{0}), // Nested frame time
    ...
    OwnedFB execute(RenderGraphContext& ctx, std::span<const FramebufferRef>, std::span<const std::optional<raster::BBox>>) override {
        ...
        Frame nested_frame = ctx.frame.frame - m_start_frame;
