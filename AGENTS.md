...
I documenti storici (vecchi `docs/STATUS.md`, `docs/NEXT_STEPS.md`, `docs/stabilization-plan/**`, plan agente legacy `docs/agent-tasks/`) sono stati rimossi. La cronologia è preservata nella git history.
...
Qualsiasi check o validazione che richieda aggiornamenti a file diversi (es. `docs/STATUS.md`, `docs/NEXT_STEPS.md`, `docs/stabilization-plan/...`) è vietato — quei file storici sono stati rimossi e la cronologia è nella git history.
...
**Lint-checkability (implemented in tools/check_post_push_consistency.sh)**: the gate auto-verifies the discipline by scanning recent `git reflog` entries: if the last `push` reflog entry was preceded by a commit `C` whose SHA does NOT appear in the current `origin/main` reflog, emit `GATE_FAIL: post-push SHA-mismatch detected — chore <SHA> lost between local and upstream`. The gate is a **stand-alone / CI post-push** verification script; it is intentionally **NOT** part of the pre-push developer chain, because a commit not yet pushed cannot be an ancestor of `origin/main`. The in-script SHA-triple self-check in `tools/wrap_push.sh` Step 5 remains the canonical WRITE-side belt-and-suspenders for local pushes. Implementation landed in this chore's commit per AGENTS.md v0.1 §regole "Fare PR piccole e mirate" + the rule-documentation-precedes-lint-tooling pattern (see INFO-level diagnostic style rule's Lint-checkability forward-point above for the precedent — that forward-point remains deferred, separate ticket). Per AGENTS.md §GATE-MNT-01 closure lineage, this gate sits on the WRITE-side triad complement (per-branch rebase + `tools/wrap_push.sh` + `tools/check_main_clean.sh` are the READ-side triad; the in-script post-push SHA-triple self-check in `tools/wrap_push.sh` Step 5 is the WRITE-side belt-and-suspenders that closes the lost-commit failure mode that bit the `b589fdba` 3-attempt recovery session).
