...
families = {
    "names": ["lower_third_safe", "name_fade_in", "name_slide_up", "name_pop_in", "name_slide_left", "name_scale_in"],
    "images": ["image_focus_in", "image_fade_in", "image_slide_left", "image_slide_right", "image_scale_in"],
    "important-phrases": ["caption_card", "phrase_fade_in", "phrase_scale_in", "phrase_slide_up", "phrase_soft_pop", "phrase_word_reveal"],
}
for family, presets in families.items():
    for preset in presets:
        ...
        plan = {"schema":"chronon.render-plan", "version":1, "job_id":f"showcase-{family}-{preset}", "canvas":{"width":1280,"height":720,"fps":30,"duration_frames":90}, "layers":[{"id":"background","type":"color","color":[0.025,0.025,0.04,1.0]}, layer], "output":{"path":"result.mp4","format":"mp4","codec":"h264"}}
...
PY
done < <(find "$OUT/plans" -name '*.json' -print | sort)
...
manifest={'schema':'chronon.preset-showcase.v1','canvas':{'width':1280,'height':720,'fps':30,'frames':90},'families':{'names':6,'images':5,'important-phrases':6},'files':files}
