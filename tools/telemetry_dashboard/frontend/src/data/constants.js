...
export const INFO_DESCRIPTIONS = {
  ...
  framebuffer_acquire_wall_ms: 'Tempo speso dal thread di rendering per acquisire un framebuffer dal pool (attesa + allocazione). Alto = pool scarso o contenzione.',
  framebuffer_enqueue_wall_ms: 'Tempo speso per rimettere in coda il framebuffer dopo l\'uso, prima che venga riutilizzato dal pool. Alto = contenzione sul pool o overhead di sincronizzazione.',
  framebuffer_clear_wall_ms: 'Tempo speso per pulire/resettare i pixel del framebuffer. Alto su memoria "fredda" (prima allocazione), basso su pool riutilizzato (cache L3 calda).',
  ...
  frame_conversion_copy_wall_ms: 'Tempo speso per convertire/copiare i pixel prima di inviarli all\'encoder FFmpeg. Alto = conversione colore o memcpy nascosta.',
  ...
  video_graph_eval_wall_ms: 'Time spent evaluating the scene graph, dirty regions, and render decisions for the video export.',
  video_conversion_wall_ms: 'SIMD conversion time for RGBA to the encoder pixel format such as YUV420p or NV12.',
  video_pipe_write_wall_ms: 'Time spent writing encoded frame data into the FFmpeg pipe.',
  video_ffmpeg_wait_ms: 'Time Chronon is blocked waiting for FFmpeg to drain its internal buffer.',
