...
PipeExportSession setup_pipe_export_session(
    ...
    Frame end)
{
    ...
    session.triple_arena = TripleBufferArena(8, arena_size);

    // ── Writer thread (context stored in session so it outlives the thread) ─
    session.writer_ctx = WriterThreadContext{
        .queue = session.queue,
    ...
    session.writer_thread = std::thread(run_writer_thread, std::ref(session.writer_ctx));
