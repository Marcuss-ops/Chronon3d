...
RunSummary query_run_summary(sqlite3* db, const std::string& run_id) {
    ...
    if (prepare_with_run_id(db, &stmt, run_sql, run_id) && sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        run.framebuffer_acquire_wall_ms = get_i64("framebuffer_acquire_wall_ms");
        run.framebuffer_clear_wall_ms = get_i64("framebuffer_clear_wall_ms");
        run.clearnode_wall_ms = get_i64("clearnode_wall_ms");
        run.framebuffer_pool_clear_wall_ms = get_i64("framebuffer_pool_clear_wall_ms");
        run.framebuffer_enqueue_wall_ms = get_i64("framebuffer_enqueue_wall_ms");
        ...
        run.frame_conversion_copy_wall_ms = get_i64("frame_conversion_copy_wall_ms");
        run.video_graph_eval_wall_ms = get_i64("video_graph_eval_wall_ms");
        run.video_conversion_wall_ms = get_i64("video_conversion_wall_ms");
        run.video_pipe_write_wall_ms = get_i64("video_pipe_write_wall_ms");
        run.video_ffmpeg_wait_ms = get_i64("video_ffmpeg_wait_ms");

        run.io_queue_push_wait_ms = get_i64("io_queue_push_wait_ms");
        ...
        run.ffmpeg_pipe_write_wall_ms = get_i64("ffmpeg_pipe_write_wall_ms");
        run.converted_frame_cache_hits = get_i64("converted_frame_cache_hits");
        run.ffmpeg_flush_wall_ms = get_i64("ffmpeg_flush_wall_ms");
        run.io_queue_peak_bytes = get_i64("io_queue_peak_bytes");

        run.setup_graph_parsing_wall_ms = get_i64("setup_graph_parsing_wall_ms");
        run.setup_asset_io_load_wall_ms = get_i64("setup_asset_io_load_wall_ms");
        run.setup_pool_preallocation_wall_ms = get_i64("setup_pool_preallocation_wall_ms");
        run.image_decode_wall_ms = get_double("image_decode_wall_ms");
        run.image_sample_ms = get_double("image_sample_ms");
        run.image_sampled_pixels = get_i64("image_sampled_pixels");

        run.compiled_graph_refresh_wall_ms = get_i64("compiled_graph_refresh_wall_ms");
        run.cache_eval_wall_ms = get_i64("cache_eval_wall_ms");
        run.dirty_eval_wall_ms = get_i64("dirty_eval_wall_ms");
        run.input_resolve_wall_ms = get_i64("input_resolve_wall_ms");
        run.framebuffer_lifetime_wall_ms = get_i64("framebuffer_lifetime_wall_ms");
        run.node_schedule_wall_ms = get_i64("node_schedule_wall_ms");
        run.node_dispatch_wall_ms = get_i64("node_dispatch_wall_ms");
        run.node_execute_actual_wall_ms = get_i64("node_execute_actual_wall_ms");
        run.telemetry_emit_wall_ms = get_i64("telemetry_emit_wall_ms");
