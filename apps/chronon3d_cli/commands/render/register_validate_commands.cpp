...
std::string prop_type_name(PropType type) {
    switch (type) {
...
int run_validate(CliContext& ctx, const ValidateState& args) {
    ...
    Composition comp = [&]() {
        try {
            return ctx.registry.create(args.comp_id, loaded.props);
        } catch (const std::exception& e) {
            throw std::runtime_error(
                "Props validation failed for '" + args.comp_id + "': " + e.what());
        }
    }();
    ...
    fmt::print("[4/6] metadata: {}x{}  {}/{} fps  {} frames\n",
               ...
               static_cast<long long>(comp.duration()));
    ...
    fmt::print("[6/6] output/settings valid: profile={}{}\n",
               args.profile,
               args.output.empty() ? "" : fmt::format(" output={}", args.output));
