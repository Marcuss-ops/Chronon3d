...
void register_inspect_commands(CLI::App& app, CliContext& ctx) {
    ...
    // Per-node TextRun audit with structured JSON output + exit code
    // mapping (0=PASS, 1=FAIL, 2=VIOLATION). Gated on
    // CHRONON3D_BUILD_DIAGNOSTICS; in non-diagnostic builds the command
    // emits an error JSON and exits 1.
    {
        auto state = std::make_shared<InspectTextState>();
        auto& args = *state->args;
        auto* cmd = app.add_subcommand("inspect-text",
            "Per-node TextRun audit with structured JSON output (exit 0=PASS, 1=FAIL, 2=VIOLATION)");
        cmd->add_option("id", args.comp_id, "Composition name")->required();
        cmd->add_option("--frame", args.frame, "Frame number to inspect")->required();
        cmd->add_flag("--json,!--no-json", args.json,
            "Emit JSON to stdout (default: on)");
        cmd->allow_windows_style_options();
        cmd->callback([state, &ctx]() {
            ctx.exit_code = command_inspect_text(ctx.registry, *state->args);
        });
    }
}
