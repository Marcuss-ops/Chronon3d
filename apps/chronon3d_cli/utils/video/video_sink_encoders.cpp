...
bool RawVideoSinkEncoder::open(const FfmpegPipeOptions& options) {
    options_ = options;
    frames_written_ = 0;

    out_.open(options.output_path, std::ios::binary);
...
bool RawVideoSinkEncoder::write_frame(const Framebuffer& fb) {
    const auto t0 = profiling::now();
    ...
    const auto t1 = profiling::now();
    const uint64_t conv_ms = static_cast<uint64_t>(profiling::duration_ms(t0, t1));

    if (counters_) {
        counters_->video_conversion_ms.fetch_add(conv_ms, std::memory_order_relaxed);
        counters_->frame_conversion_copy_ms.fetch_add(conv_ms, std::memory_order_relaxed);
        counters_->video_frames_submitted.fetch_add(1, std::memory_order_relaxed);
        counters_->video_frames_converted.fetch_add(1, std::memory_order_relaxed);
    }
    ...
    const auto w0 = profiling::now();
    out_.write(reinterpret_cast<const char*>(buffer_.data()),
               static_cast<std::streamsize>(converted_bytes));
    const auto w1 = profiling::now();

    if (!out_) return false;

    if (counters_) {
        const uint64_t write_ms = static_cast<uint64_t>(profiling::duration_ms(w0, w1));
        counters_->video_pipe_write_ms.fetch_add(write_ms, std::memory_order_relaxed);
...
bool RawVideoSinkEncoder::close() {
    if (out_.is_open()) {
        out_.flush();
        out_.close();
    }
