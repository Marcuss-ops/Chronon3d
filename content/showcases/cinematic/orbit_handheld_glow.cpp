...
//  orbit evaluated with `motion.evaluate(ctx.local_time(), 0, 239)` +
...
Composition orbit_handheld_glow() {
    return composition({
        ...
        // brace-init pattern that the current TextDefinition doesn't accept; this
    ...
    .frame = {
        ...
        Camera2_5D cam = motion.evaluate(ctx.local_time(), Frame{0}, Frame{239});
