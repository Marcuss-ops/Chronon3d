...
inline void add_header(SceneBuilder& s, const std::string& id, const std::string& title, const std::string& subtitle) {
    ...
    s.layer("header_" + id, [=](LayerBuilder& l) {
        ...
        l.text("badge_txt", {.text = id, .size = {52.0f, 52.0f}, .pos = {0.0f, 0.0f, 0.0f}, .font_path = "assets/fonts/Poppins-Bold.ttf", .font_family = "Inter", .font_weight = 900, .font_style = "normal", .font_size = 24.0f, .color = kWhite, .align = TextAlign::Center, .vertical_align = VerticalAlign::Middle});
        l.text("title", {.text = title, .size = {1060.0f, 36.0f}, .pos = {84.0f, -2.0f, 0.0f}, .font_path = "assets/fonts/Poppins-Bold.ttf", .font_family = "Inter", .font_weight = 900, .font_style = "normal", .font_size = title_font_size, .color = kWhite, .align = TextAlign::Left, .vertical_align = VerticalAlign::Middle});
        l.text("subtitle", {.text = subtitle, .size = {1060.0f, 24.0f}, .pos = {84.0f, 28.0f, 0.0f}, .font_path = "assets/fonts/Poppins-Regular.ttf", .font_family = "Inter", .font_weight = 400, .font_style = "normal", .font_size = subtitle_font_size, .color = {0.78f, 0.82f, 0.92f, 1.0f}, .align = TextAlign::Left, .vertical_align = VerticalAlign::Middle});
    });
}
