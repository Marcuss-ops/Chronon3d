// content/effects/glow_v2_ab_tests.cpp
//
// V2-ONLY glow test compositions using the new TextGlowSpec API.
//
// All V1 (legacy `l.glow(radius, intensity, color)`) variants have been
// removed — V2 is the canonical glow API going forward.
//
// Tests included:
...
// Dark navy gradient (same as TextGlowReveal in the typewriter family)
inline void test_bg(SceneBuilder& s) {
    deep_bg(s, Color{0.01f, 0.012f, 0.022f, 1.0f}, Color{0.025f, 0.018f, 0.04f, 1.0f});
}

// Common text params — matches Minimalist family convention
struct TestTextParams {
    f32 font_size{64.0f};
...
inline TextParams make_test_text(const std::string& s, f32 y_offset, const TestTextParams& p = {}) {
    return TextParams{
        .text = s,
        .size = p.size,
        .pos = {0.0f, y_offset, 0.0f},
...
// =============================================================================
// TEST 1 — Sharpness: short text "CHRONON3D GLOW TEST"
// =============================================================================
//
// V2 (TextGlowSpec): text stays sharp (core_preservation) + tight glow + soft bloom
//
Composition glow_sharpness_test() {
    return composition({.name = "GlowSharpnessTest", .width = kW, .height = kH, .duration = 60},
...
Composition glow_radius_compare_test() {
    return composition({.name = "GlowRadiusCompareTest", .width = kW, .height = kH, .duration = 60},
    [](const FrameContext& ctx) {
        SceneBuilder s(ctx);
        test_bg(s);

        // 2x2 grid layout: A (top-left), B (top-right), C (bot-left), D (bot-right)
        struct Panel { const char* label; f32 inner, mid, bloom; Vec3 pos; };
        Panel panels[] = {
            {"A: 3/10/24",    3.0f, 10.0f, 24.0f, {-kHW*0.5f,  kHH*0.25f, 0.0f}},
            {"B: 4/14/34 ★",  4.0f, 14.0f, 34.0f, { kHW*0.5f,  kHH*0.25f, 0.0f}},
            {"C: 6/20/46",    6.0f, 20.0f, 46.0f, {-kHW*0.5f, -kHH*0.25f, 0.0f}},
            {"D: 8/28/60",    8.0f, 28.0f, 60.0f, { kHW*0.5f, -kHH*0.25f, 0.0f}},
        };

        for (const auto& p : panels) {
            // Text layer with V2 glow
            ...
            // Label below text
            s.layer("lbl_" + std::string{p.label}, [p](LayerBuilder& l) {
                l.pin_to(Anchor::Center);
                l.position({p.pos.x, p.pos.y - 80.0f, 0.0f});
                l.text("t", {
                    .text = p.label,
                    .size = {420.0f, 30.0f},
                    .font_size = 16.0f,
                    .color = {0.55f, 0.72f, 0.90f, 1.0f},
                    .align = TextAlign::Center,
                    .vertical_align = VerticalAlign::Middle,
                });
            });
...
Composition glow_typewriter_reveal_test() {
    return composition({.name = "GlowTypewriterRevealTest", .width = kW, .height = kH, .duration = 120},
    [](const FrameContext& ctx) {
        SceneBuilder s(ctx);
        test_bg(s);
...
Composition glow_shadow_balance_test() {
    ...
    [](const FrameContext& ctx) {
        ...
        // 3 panels: A (no shadow), B (blur 10, op 0.12), C (blur 18, op 0.18)
        struct Panel { const char* label; f32 blur; f32 opacity; Vec3 pos; };
        Panel panels[] = {
            {"A: no shadow",        0.0f, 0.0f,  {-kHW*0.66f, 0.0f, 0.0f}},
            {"B: blur 10 / op 0.12", 10.0f, 0.12f, { 0.0f,      0.0f, 0.0f}},
            {"C: blur 18 / op 0.18", 18.0f, 0.18f, { kHW*0.66f, 0.0f, 0.0f}},
        };
            ...
            s.layer("txt_" + std::string{p.label}, [p](LayerBuilder& l) {
                ...
                if (p.blur > 0.0f) {
                    l.drop_shadow({0.0f, 4.0f},
                                  Color{0.0f, 0.02f, 0.12f, p.opacity},
                                  p.blur);
                }
                l.text("t", {
                    .text = "GLOW",
