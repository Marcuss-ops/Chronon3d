...
inline Composition make_typewriter(
    ...
    return composition({.name = std::move(name), .width = canvas_width, .height = canvas_height, .duration = duration_frames}, 
        ...
        SceneBuilder s(ctx);
        const f32 p = std::clamp(static_cast<f32>(ctx.frame.frame.frame) / static_cast<f32>(std::max<Frame>(duration_frames, 1)), 0.0f, 1.0f);
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            ...
            std::string revealed = line.reveal(ctx.frame.frame.frame);
            if (revealed.empty()) continue;

            f32 opacity = opacity_fn(ctx.frame.frame.frame, duration_frames);

            s.layer("tw_" + std::to_string(i), [i, &line, revealed = std::move(revealed), opacity, frame = ctx.frame.frame.frame, glow](auto& l) {
