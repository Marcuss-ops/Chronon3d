...
3. **Regola di isolation** (hard): `#include <unicode/*>` consentito solo in `src/text/boundary_resolver/` + `src/backends/text/icu_resolver.cpp`. Nessun altra TU può includere ICU headers. tools/check_architecture_boundaries.sh §5 (Gate 5) (gate check) deve essere extended a verificare questa invariante.
