...
| **Code-reviewer verdict** | PASS with 2 NEEDS-FIX recommendations (commit `e18be869`): **(i)** merge sub-tickets A+B into 1 atomic commit (both harden the motion-blur-no-recompile doc-block contract); **(ii)** replace the `§G` cross-link anchor (non-existent in canonical source) with the actual section names `§5th stage — framing` + `§7th stage — validation finale` or `§HONEST GAP (real-bounds query)` per the TICKET-FRAMING-V1 CHANGELOG entry. |
| **Blocca** | Camera V1 cert — type-system enforcement of motion-blur-no-recompile invariant; grep-discoverable cross-link to TICKET-FRAMING-V1 §5th stage — framing; near/far configurable wire-up across all 4 legacy paths; real layer bounds for framing solver |
| **Stato** | OPEN |
...
The code-reviewer verdict on commit `f60c8c54` raised 3 NEEDS-FIX items + 1 cross-ticket forward-point. This ticket consolidates all 4 into a single Phase 2 workstream. Per the code-reviewer's recommendation, sub-tickets A+B are merged into 1 atomic commit (both harden the motion-blur-no-recompile doc-block contract); sub-tickets C and D remain separate atomic commits. Net: **3 atomic commits on `main`**, not 4.

## Sub-tickets (3 atomic commits on `main`, per code-reviewer verdict A+B merged recommendation)

### (A + B merged) Type-signature hardening for motion-blur-no-recompile invariant + cross-link to TICKET-FRAMING-V1 §5th stage — framing

**Sub-ticket A — type-signature hardening (prose → type)**: convert the `CameraEvaluatorFn` from a doc-block-protected contract to a type-system-enforced contract.

**Sub-ticket B — cross-link** (merged with A): add the cross-link to TICKET-FRAMING-V1 §5th stage — framing (5th-stage framing block + 7th-stage validation finale) in the new doc-block. (Note: the original cross-link anchor `§G` was a typo; the corrected anchor is the actual section name from the TICKET-FRAMING-V1 CHANGELOG entry, per the code-reviewer NEEDS-FIX recommendation (ii).)
...
**Target state (type-level invariant + cross-link)**: convert the `CameraEvaluatorFn` from a doc-block-protected contract to a type-system-enforced contract. Two candidate shapes (machine-verified during implementation):
...
**Cross-link (sub-ticket B, merged)**: add a 1-line comment at the top of the existing doc-block in EACH of the 2 files (`shutter_pose_sampler.cpp` + `per_pixel_dof_node.cpp`):

```cpp
// TICKET-PROJECTION-V1 doc-block — see TICKET-FRAMING-V1 §5th stage — framing (5th-stage framing block + 7th-stage validation finale) for the upstream/downstream contract.
```

**Files changed**: `include/chronon3d/scene/camera/camera_v1/internal/shutter_pose_sampler.hpp` (typedef update) + 1-3 call sites in `src/scene/camera/camera_v1/internal/shutter_pose_sampler.cpp` (lambda capture changes) + the doc-block update (adds cross-link + removes the prose-level invariant, replaces with the type-level invariant citation) + 1 line in `src/render_graph/nodes/per_pixel_dof_node.cpp` (cross-link only).

**Regression lock**: `tests/scene/camera/test_motion_blur_torture_pr1.cpp` (existing) + a new TEST_CASE that verifies a closure with the OLD signature (taking `CameraDescriptor&`) does NOT compile (negative-type test via `static_assert` in a `.cpp` file that is NOT included in the build target; this is the canonical C++ pattern for negative-type tests) + `git grep -nE 'TICKET-FRAMING-V1.*§5th stage'` returns ≥ 2 hits (the 2 new cross-links) after the commit lands.
...
**Next step** (3 atomic commits, per code-reviewer verdict A+B merged recommendation):

1. Sub-ticket A+B first (type-signature hardening + cross-link — the highest-priority NEEDS-FIX because it converts prose to type AND adds grep-discoverable cross-link in 1 atomic commit).
2. Sub-ticket C second (near/far + pixel-aspect — the user-spec's "pixel aspect e near/far risultano ancora parziali" forward-point). Borderline-split: C1 (config struct + 4 source paths) + C2 (24-TEST_CASE matrix) if implementation balloons.
3. Sub-ticket D third (real layer bounds — the TICKET-FRAMING-V1 §HONEST GAP (real-bounds query) forward-point; the largest change).

**Macchina-verifica**: deferred to working build host (vcpkg `doctest` install + tmpfs quota for full cmake build). The 3 sub-tickets can each be code-reviewed + committed independently; the test execution is deferred.
...
- [`src/scene/camera/camera_v1/internal/shutter_pose_sampler.cpp`](../../src/scene/camera/camera_v1/internal/shutter_pose_sampler.cpp) (the motion-blur-no-recompile doc-block; target of sub-ticket A+B)
...
- AGENTS.md §honesty (1 documented parent honest gap per the TICKET-PROJECTION-V1 + TICKET-FRAMING-V1 entries; the 3 sub-tickets each document their own honest gap in their commit CHANGELOG entry)

## Non-goal

- claim "AGENTS.md §honesty" *DONE* without macchina-verification;
- expand the new symbols beyond the 3 sub-tickets (no gratuitous `ProjectionContractConfig::operator==` + `hash` unless a future test needs them);
- sub-ticket D's `framing_targets` field removal in the SAME commit as the deprecation (deprecation is the cat-3 forward-only invariant; 1 release wait then remove in a separate commit per AGENTS.md §Cat-3);
