...
**PLANNED** (action-plan landing 2026-07-07). Scope: introduzione del sistema canonico `AssetRef + AssetManifest + AssetPreflightResolver + AssetPreflightResult` + eliminazione del legacy "asset controllati durante il render / fallback silenziosi / catch MESSAGE return nei test" via sequenza 4-step safe migration. **Workstream design-FROZEN** (no codice toccato in questo commit di landing; solo governance canonico + grep-audit backlog + path di eliminazione).
