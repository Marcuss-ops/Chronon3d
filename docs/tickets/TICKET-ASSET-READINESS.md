...
**PARTIAL** — Step 1/4 DONE (commit TBD-pending this session, 2026-07-07). Code landed: 5 nuovi simboli pubblici canonici `AssetKind` enum + `AssetRef` POD + `AssetManifest` class + `AssetPreflightResult` POD + `AssetPreflightResolver` class in `include/chronon3d/assets/asset_readiness_v2.hpp` namespace `chronon3d::assets::v2` (distinto da `chronon3d::AssetType` esistente in `asset_metadata.hpp` — semantica manifest/owner-centric vs metadata/registry). Manifest aggiornato in `cmake/Chronon3DPublicHeaders.cmake`. Header `g++ -std=c++20 -fsyntax-only` exit 0; `/tmp/_step1_asset_consumer_check.cpp` compile + run exit 0 con output `OK Step 1 Asset Readiness typecheck + behavior invariants` (8 invariants: AssetKind 4 valori + AssetRef defaults + AssetManifest add/entry_for/all + preflight placeholder + namespace isolation). Code-reviewer-minimax-m3 APPROVED round 1 (1 nit minore doc-only `LIFO semantics` → `first-inserted-wins` risolto). ZERO codice esistente toccato (test preesistenti bit-identicali garantiti). AGENTS Cat-3 freeze-compliant + ABI pubblico preservato (l'esistente `chronon3d::AssetRegistry` + `chronon3d::AssetMetadata` restano intatti). Steps 2/3/4 (legacy adapters + migrate new content + eliminate legacy) restano forward-point. **Workstream design-FROZEN 2026-07-07** per le 3-step rimanenti.
