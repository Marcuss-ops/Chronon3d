...
| d | OPEN (P2)   | **Gate #25 [3/4] regex-tightening per i 20 false-positive callsites surfaced questo chore** (vedi [TICKET-CHECK-NO-DUAL-TEXT-API-REGEX-IMPROVEMENT](TICKET-CHECK-NO-DUAL-TEXT-API-REGEX-IMPROVEMENT.md)). cat-3 minimal-surface: 1 file (`tools/check_no_dual_text_api.sh`), 1-2 line gate-tightening (NON silent downgrade — precision improvement che mantiene il coverage dei real `TextSpec.position = X` callsite). Without questo forward-point, il push del commit `753401a0` + this chaser-chore rimane blocked by `bash tools/wrap_push.sh origin main` GATE_FAIL (gate over-triggers su RenderNode/Transform/Camera `.position`). |
...
Il forward-point (d) è una **registrazione di gate-over-trigger incident** (non una chore di rot-fix) — il gate regex in `tools/check_no_dual_text_api.sh:194` necessita tightening da `\.position(?=\s*[={])` (generico, over-triggers su 20 call sites non-correlati) a `\b(spec|text|TextSpec)\.position(?=\s*[={])` (qualificato TextSpec, mantiene coverage reale). Tracciato in [TICKET-CHECK-NO-DUAL-TEXT-API-REGEX-IMPROVEMENT](TICKET-CHECK-NO-DUAL-TEXT-API-REGEX-IMPROVEMENT.md) per AGENTS.md `push frequent, transient failure tracked as forward-point`.
