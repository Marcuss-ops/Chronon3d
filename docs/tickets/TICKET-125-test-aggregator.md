...
# Dual-executor verdict

**Verdetto:** `GraphExecutor` è la sola autorità produttiva di esecuzione.
`executor.cpp` decide il percorso (`fully_recorded` compiled program oppure
fallback DAG via `execute_levels`), possiede il lifecycle del frame, pubblica
gli errori e restituisce l'output finale.

`node_runner.cpp`, `executor_levels.cpp` e `node_executor.cpp` sono helper con
ownership non sovrapposta: rispettivamente orchestrazione per-node, scheduling
dei livelli e primitive `RenderGraphNode::execute`/ownership framebuffer.

`command_plan_executor.cpp` è un **test-only adapter** per parity/readiness test:
ha zero caller produttivi in `apps/`, `src/` e `tools/`. La sua dichiarazione è
confinata a `tests/helpers/command_plan_executor.hpp`; il simbolo
`execute_command_plan` resta compilato come ABI-compatibility shim perché è
presente nella baseline libabigail `tools/sdk/chronon3d_c.abi`.

Non esistono quindi due executor produttivi che possiedono la stessa decisione.
Dettaglio call graph e profiling compilazione: [`EXECUTOR_OWNERSHIP.md`](../EXECUTOR_OWNERSHIP.md).
...
| 13 | Camera brutal / camera certification | `docs/tickets/TICKET-TEST-13-CORE.md` + `tools/check_camera_architecture.sh` + `tools/verify_camera_functional_linux.sh` | Entrambi i gate exit 0, camera golden statico/animato disponibile, sanitizer clean e output deterministico sullo stesso SHA | exit 1, golden mismatch, sanitizer finding, non-determinismo o prerequisito mancante | OPEN (slot distinto da Test 11; contratto canonico in TICKET-TEST-13-CORE) |
...
|---|---|---|---|---|
| TICKET-TEST-13-CORE | `docs/tickets/TICKET-TEST-13-CORE.md` | OPEN | this TICKET-125 row 13; TICKET-127 audit resolved distinct-from-Test-11 | P1 |
...
| `docs/FOLLOWUP_TICKETS.md` | §Open Blockers | TICKET-125-TEST-AGGREGATOR (P0) row + TICKET-TEST-13-CORE (P1) row (references to TICKET-125 inline) |
...
- **§honesty Test 13**: l'ambiguità è risolta come test distinto (Camera brutal); il runtime resta OPEN/PARTIAL finché i gate camera e sanitizer non sono eseguiti sul WBH.
...
- Executor ownership and profiling verdict: `docs/EXECUTOR_OWNERSHIP.md`
