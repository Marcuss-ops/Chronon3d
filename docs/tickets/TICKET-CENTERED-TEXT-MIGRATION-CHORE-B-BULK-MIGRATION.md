...
- [x] **Sub-chore Blocco 5.2.F: TESTS-DETERMINISTIC-AREA** DONE (audit 2026-07-14) — audit cronaca in [TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-NON-VACUOUS-AUDIT](TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-NON-VACUOUS-AUDIT.md) (NON-VACUOUS, 9 rg matches in test_visual_regression_scenarios.cpp; 2nd file FILE_NOT_FOUND). Migration forward-pointed to [TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-MIGRATION](TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-MIGRATION.md) (future chore, byte-equivalence regression lock preservation obbligatorio). Phase 3 remove forward-pointed to (f)-phase3-remove post-migration.
...
| e | CONTENT-OTHER-AREA                 | `content/text/text_reveal.cpp` + `content/text/text_glow_helpers.hpp`       |                  ~5 | DONE (vacuous, 2026-07-14, callers already pre-migrated M1.8 §2D + glow_text macro absent) |
| f | TESTS-DETERMINISTIC-AREA           | `tests/deterministic/test_visual_regression_scenarios.cpp` 5 callsites + `tests/text/test_visual_regression_scenarios.cpp` | ~5 | DONE audit |
...
| e | DONE (vacuous, 2026-07-14) | Sub-chore Blocco 5.2.E: CONTENT-OTHER-AREA migration pre-completed pre-this-session (cronologia chiusura: [TICKET-CENTERED-TEXT-OTHER-AREA-VACUOUS-VERIFY](TICKET-CENTERED-TEXT-OTHER-AREA-VACUOUS-VERIFY.md)). ZERO source questo turno (cat-3 minimal-surface). 0 code-only callers across content/text/ post comment-strip filter. NO macchina-verifica additional work needed. |
| f | DONE (audit, 2026-07-14) | Sub-chore Blocco 5.2.F: TESTS-DETERMINISTIC-AREA audit DONE 2026-07-14 (cronologia chiusura: [TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-NON-VACUOUS-AUDIT](TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-NON-VACUOUS-AUDIT.md) — NON-VACUOUS finding, 9 real `centered_text(` callers in test_visual_regression_scenarios.cpp; 2nd file FILE_NOT_FOUND). ZERO source questo turno (cat-3 minimal-surface audit-only). Real source migration forward-pointed to (f)-migration row. |
| (f)-migration | OPEN (P2, future chore) | TICKET-CENTERED-TEXT-TESTS-DETERMINISTIC-AREA-MIGRATION: source code migration dei 9 chiamanti in test_visual_regression_scenarios.cpp da centered_text(CenterTextOptions) a TextDefinition{} direct construction. **CRITICAL**: byte-equivalence regression lock preservation obbligatorio (kRefVR* sentinel hashes MUST remain byte-equivalent). Mapping table (CenterTextOptions → TextDefinition) verbatim in audit chaser-ticket-home §CRITICAL. |
| (f)-phase3-remove | OPEN (P3, future chore post-(f)-migration) | After (f)-migration DONE: `centered_text` function + `CenterTextOptions` struct removal da `content/text/text_helpers_centered.hpp` per TICKET-CENTERED-TEXT-MIGRATION §Criteri (Phase 3 overload REMOVAL post V0.1). The `CenterTextOptions` struct itself is also legacy-deprecated; same Phase 3 removal scope. |
