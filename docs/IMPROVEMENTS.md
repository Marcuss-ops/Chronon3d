# Chronon3D — Improvement Roadmap (Archivio)

> 📦 **Questo file è stato suddiviso in documenti separati per chiarezza.**
> - ✅ **Completati:** Vedi [`CHANGELOG.md`](CHANGELOG.md)
> - 📋 **Attivi:** Vedi [`ROADMAP.md`](ROADMAP.md)
> - 🏗️ **V3 Blueprint (tile-based):** Vedi [`V3_BLUEPRINT.md`](V3_BLUEPRINT.md)
> - 💡 **Speculativo:** Vedi [`Future Implementations.md`](Future%20Implementations.md)

---

## 📊 Riepilogo dei risultati PRs 1-4
...
## 🚀 NEXT-GEN — Visione Rivoluzionaria: Da Renderer a Compilatore Video Headless

> Le idee seguenti rappresentano la visione a lungo termine per trasformare Chronon3D
> da un "renderer più veloce" a un **compilatore video headless** — un sistema che
> decide cosa non fare invece di ottimizzare cosa fare.

---

### V1. Segment Encoder — Non Codificare 60 Frame Identici

**Problema:** Anche se la scena è statica, Chronon produce frame 0, 1, 2, ... 59 identici e li processa tutti.

**Soluzione:** Introdurre un `TimelineAnalyzer` che crea `VisualSegment` — blocchi di frame identici.
...
Poi l'encoder nativo ha due metodi:
```cpp
write_frame(const Framebuffer& fb);
write_static_frame_duration(const Framebuffer& fb, int duration_frames);
```

**Perché è potente:** Per video YouTube automatizzati, tantissime scene sono background statico, testo statico per alcuni secondi, immagine statica. Invece di 3000 frame, Chronon processa solo 80 stati visivi.

**Dove:** Nuova fase `Timeline Analyzer` prima dell'export + estensione `NativeAvEncoder`.

---

### V2. Encoder-Owned Rendering — Renderizzare Direttamente in YUV

**Problema:** Pipeline attuale: `Framebuffer float → AVFrame YUV420P → avcodec_send_frame`. C'è una conversione intermedia.

**Soluzione:** Il target finale del render graph non è sempre un `Framebuffer` float RGBA. Può essere direttamente un target video YUV/NV12.
...
struct RenderTargetView {
    ...
    int width, height;
    Color* rgba;        // se FloatRGBA
    uint8_t* y, *u, *v; // se YUV
    uint8_t* uv;        // se NV12
    int stride_y, stride_u, stride_v, stride_uv;
};
```

**Cosa cambia:** I processor dichiarano `supports_direct_yuv()`. Background, solid fill, image opaque, checkerboard, scanlines, noise scrivono direttamente in YUV. Blur, glow, alpha compositing restano in FloatRGBA. Il render graph diventa "format-aware".

---

### V3. Tile Cache Globale tra Frame, Video e Canali

**Problema:** Per 200 canali con migliaia di video, stesse grafiche, stessi background, stessi template, stesse intro — ogni canale rifà tutto da capo.

**Soluzione:** `PersistentTileCache` globale. Ogni frame diviso in tile 128×128 o 256×256. Ogni tile ha un `TileKey` con `content_hash`. Se già renderizzato e convertito in YUV, non lo rifai.
...
Ogni frame diventa un Merkle tree: `FrameHash → TileHash 0, TileHash 1, ...`. La cache è persistente su disco: `output/cache/tiles/`.

**Perché è pazzesco:** Su scala 200 canali, template ripetuti, background, cornici, watermark, loghi, lower thirds — Chronon non deve "essere veloce ogni volta". Deve diventare: la prima volta calcolo, dalla seconda volta recupero.

---

### V4. Scene-to-Kernel Compiler

**Problema:** Ogni nodo è una chiamata virtuale, switch su shape type, lookup nel registry, hash ripetuti.

**Soluzione:** Compilare una composition in un kernel specializzato. Invece di graph generico, Chronon genera un piano specializzato.

**Versione 1 — Bytecode interno:**
```cpp
enum class RenderOpCode {
    ClearSolid, DrawGrid, DrawImage,
    DrawTextCached, CompositeNormal,
    ApplyBlur, ConvertToYuv,
};

struct RenderOp {
    RenderOpCode op;
    uint32_t dst, src, params_index;
};
```

**Versione 2:** Generare C++ o LLVM per composizioni hot. Dentro `render_DarkGridBackground_compiled()` non ci sono più virtual call, varianti, allocazioni — solo comandi necessari.

**Perché può battere gli altri:** Chronon non fa editing interattivo. Fa rendering batch/headless. Può spendere tempo prima per compilare la scena, poi renderizzare velocissimo. È come passare da "interpreto la ricetta ogni volta" a "ho una macchina costruita per quella ricetta".

---

### V5. Motion Reuse — Ri-usare il Frame Precedente

**Problema:** Se la camera fa un pan di 2 pixel, un engine normale rifà tutto.

**Soluzione:** Chronon prende il frame precedente, lo sposta di 2 pixel, renderizza solo la striscia nuova.

**Versione 1 — Integer pan:**
```cpp
previous_frame.shift(-2, 0);
render_exposed_strip({width-2, 0, width, height});
```

**Versione 2 — Subpixel pan:** Camera si muove di 0.5 px → resample bilineare + repair edges.

**Versione 3 — Affine transform:** Zoom leggero → scale around center + render borders.

**Versione 4 — 2.5D reproject:** Per scene 2.5D con depth/layer info → reproject + fill holes + rerender uncertain regions.

**Struttura:**
```cpp
struct FrameReusePlan {
    enum class Type {
        None, ExactSame, IntegerScroll,
        SubpixelScroll, AffineReproject, LayerReproject
    };
    Type type;
    Mat3 previous_to_current;
    raster::BBox repair_rect;
    float confidence;
};
```

Prima di renderizzare: `FrameReusePlan plan = analyze_temporal_reuse(prev_state, curr_state)`. Se confidence alta → reuse + repair. Se bassa → full render.

---

### V6. Coverage Certificates — Sapere Cosa è Nascosto

**Problema:** Skip-When-Opaque esiste per il caso semplice, ma non c'è analisi generale di copertura.

**Soluzione:** Ogni layer produce un `CoverageCertificate`:
```cpp
struct CoverageCertificate {
    bool fully_opaque;
    raster::BBox bounds;
    TileMask opaque_tiles;
    TileMask translucent_tiles;
};
```

Il graph builder lavora dall'alto verso il basso: layer sopra copre questi tile → layer sotto visibile solo dove non coperto. Se una parte è completamente nascosta, non la renderizzi proprio.

**Esempio:** 10 layer full-frame ma gli ultimi 3 coprono tutto → renderizzi solo quelli necessari.

---

### V7. Encoder-Aware Render Graph

**Problema:** Renderer e encoder sono separati. L'encoder non sa cosa sta comprimendo.

**Soluzione:** Il renderer produce frame + semantic map. Per ogni tile: testo, volto, background piatto, griglia, bordo, blur, logo.

Poi l'encoder usa la semantic map per guidare la compressione:
- Testo e loghi → QP migliore (qualità alta)
- Sfondi piatti → QP più aggressivo
- Griglia → attenzione aliasing

Chronon conosce il contenuto semanticamente, FFmpeg no. Chronon può guidare l'encoder meglio di un encoder generico.

---

### V8. Perceptual Dirty Rendering

**Problema:** Il dirty rect è binario: cambiato o non cambiato. Ma se la differenza è impercettibile?

**Soluzione:** Invece di "il tile è cambiato?", chiedi "il tile è cambiato abbastanza da essere visto?".

**API:**
```cpp
enum class TemporalQuality {
    Exact,
    VisuallyLossless,
    FastPreview,
    YouTubeOptimized
...
Se `Exact` → tutto deterministico perfetto. Se `YouTubeOptimized` → Chronon può riusare tile quando la differenza è impercettibile.

**Esempio:** Background animato con rumore leggerissimo: update ogni 3 frame. Blur glow: update ogni 2 frame. Testo: update ogni frame.

---

### V9. CPU Autotuner

**Problema:** Ogni VPS/CPU è diverso. Parametri fissi non funzionano su hardware vario.

**Soluzione:** Al primo avvio, Chronon fa benchmark automatici e salva `.chronon/perf_profile.json`.

```json
{
  "cpu": "8 cores generic",
  "best_tile_size": 128,
  "best_render_threads": 6,
  "best_convert_threads": 2,
  "best_encoder_threads": 2,
  "best_yuv_path": "highway_avx2",
  "recommended_x264_preset": "veryfast"
}
```

Così Chronon si auto-configura per qualsiasi VPS, senza GPU, senza configurazione manuale.

---

### V10. Multi-Language Render Once

**Problema:** Per contenuti globali, devi renderizzare 20 volte lo stesso video in 20 lingue.

**Soluzione:** Base video una volta, audio track multiple, subtitle track multiple, overlay localizzato solo se serve.

**Pipeline:**
```text
BaseRender
LocalizedOverlayRender
MuxLanguageTracks
```

Se cambia solo lingua/audio/sottotitoli: non rerenderizzare background, non rerenderizzare animazioni, non ricodificare tutto.

Questo si collega perfettamente alla missione: un solo prodotto, una sola piattaforma, tutte le lingue del mondo.

---

### Le 3 Idee Più Forti

Se vuoi una scelta secca, queste tre sono le più differenzianti:

1. **Segment Encoder** — Non processare 3000 frame se sono solo 80 stati visivi
2. **Encoder-Owned Rendering / Direct AVFrame Target** — Salta la conversione FloatRGBA→YUV
3. **Scene-to-Kernel Compiler** — Niente virtual call, niente switch, niente graph traversal a runtime

La sequenza perfetta:
```text
NativeAvEncoder
→ direct AVFrame write
→ segment encoder static duration
→ direct float-to-YUV
→ scene compiler
→ tile cache globale
```
...
Chronon3D non deve essere solo "un renderer più veloce".

Deve diventare un **compilatore video headless**.

Input:
```text
composition declarativa
timeline
template
asset
lingua
...
Output:
```text
video codificato nel modo più intelligente possibile
```

E internamente decide:
```text
questo frame non cambia → non renderizzare
questo segmento è statico → non convertire 60 volte
questo layer è coperto → non rasterizzare
questo tile è già in cache → recuperalo
questo background può andare diretto in YUV → niente framebuffer float
questa composizione è ripetuta → usa kernel compilato
```
