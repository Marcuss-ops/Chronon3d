...
4. **`camera_descriptor_adapters.{hpp,cpp}`** (`include/chronon3d/scene/camera/camera_v1/camera_descriptor_adapters.{hpp,cpp}` + `src/scene/camera/v1/camera_descriptor_adapters.cpp`) — adapter layer bridging pre-canonical descriptor formats; once items 1–3 delete, adapters have zero in-tree consumer.  **20 in-tree matches**.
