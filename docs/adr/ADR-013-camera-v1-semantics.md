...
- `tests/runtime/test_camera_session_keep_last_valid.cpp` — 2-frame regression lock.  SUBCASE A is the PRIMARY gate-(c) lock: evaluate Frame 0 (passes → caches `last_valid_camera`); modify descriptor / evaluate Frame 1 with `distance-zero` failure under `KeepLastValidCamera` policy → MUST receive `EvaluatedCamera { .camera = frame-0-camera, .diagnostics has "Recovered:" warning with constraint index + reason }` rather than a `CameraErrorCode::ConstraintFailure`.
- SUBCASE B pairs the inverse contract: after `session.reset()` the cache is empty, so a single-frame `KeepLastValidCamera` failure under `Stop`-equivalent conditions MUST fall through to `CameraErrorCode::ConstraintFailure`.
