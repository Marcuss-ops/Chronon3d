...
## TICKET-033 — CameraDescriptor payload on `LayerKind::Camera` (no side-list)

| Field | Value |
|---|---|
| **Status** | MERGED on `codex/cam-camera-payload` as commit `791eac8` (separate branch from TICKET-034).  Cross-referenced here for documentation bookkeeping. |
| **Symptom** | `SceneBuilder::camera_layer<Fn>(name, fn)` populates the default `Camera2_5D` on the legacy `Composition::camera` field but does NOT carry the descriptor itself on `LayerKind::Camera` — downstream readers must re-author the camera through a separate side-list (`AnimationGraph::camera_layers()`-style), a footgun that violates "never hidden as a constant" and prevents fingerprint-based cache keys. |
| **Root cause analysis** | `Layer` had only a `Transform` + `Effect stack` payload surface; the `camera_descriptor` was carried implicitly via the default-camera-via-Scene-function closure and the side-list kept by `SceneBuilder::m_camera_payloads`.  TICKET-033 replaced that with a typed `Layer::m_camera_payload : unique_ptr<Camera2_5DRuntime>` so the descriptor is reachable from any `Layer` of `kind == LayerKind::Camera` without indirection.  The `accept_payload=false` knob was removed because it silently dropped the descriptor; the safety-net branch was removed because it produced double-storage. |
| **Out-of-scope rationale for prior cleanup chains** | The side-list pattern pre-dates TICKET-033 by ≥ the camera_v1 migration; it was used by every test fixture and the golden-harness suite as "shortcut" to look up the camera bound to each layer.  The cleanup chain (`remove side-list + flatten onto Layer`) requires all 7 reader call-sites to migrate in the same commit to avoid mid-state inconsistency. |
| **Suggested fix approach** | Committed in commit `791eac8` on `codex/cam-camera-payload`: (1) add `Layer::m_camera_payload : std::unique_ptr<Camera2_5DRuntime>` + `Layer::camera_payload() -> Camera2_5DRuntime*`; (2) `LayerBuilder::camera_descriptor(Camera2_5D)` single-arg setter (auto-promotes kind + builds payload); (3) `SceneBuilder::camera_layer(name, fn)` lambda path routes through the same canonical pipeline; (4) `find_active_camera_payload(scene, frame)` resolver (LAST-wins) replaces the side-list lookup; (5) tests at `tests/scene/camera/test_camera_payload.cpp` exercise round-trip, fingerprint-stable serialisation, last-wins semantics. |
| **Acceptance criteria** | (1) `git log --oneline codex/cam-camera-payload` shows commit `791eac8` at HEAD; (2) `grep -nE 'Layer::camera_payload' include/chronon3d/scene/model/layer/layer.hpp` returns the new accessor; (3) `git grep -nE 'm_camera_payloads\b' src/scene/builders/scene_builder.*` returns 0 hits (side-list is gone); (4) `tests/scene/camera/test_camera_payload.cpp` compiles + passes on `codex/cam-camera-payload` once TICKET-029 rot is also fixed (the camera_v1 compile path is shared with TICKET-034). |
| **Cross-references** | `include/chronon3d/scene/model/layer/layer.hpp`; `include/chronon3d/scene/builders/layer_builder.hpp`; `include/chronon3d/scene/builders/scene_builder.hpp`; `src/render_graph/layer_resolver.cpp` (`find_active_camera_payload`); `tests/scene/camera/test_camera_payload.cpp`; `tests/scene_tests.cmake` (registration line). |

## TICKET-034 — `CameraDescriptor` as canonical default in composition settings (no `DefaultCameraSpec`, no renderer-side default constant)

| Field | Value |
|---|---|
| **Status** | MERGED-ON-BRANCH on `codex/cam-default-comp-camera` (NEEDS REBASE on main once TICKET-029 is fixed — the camera_v1 TU compilation chain is shared with that pre-existing P0 rot and currently produces `'CameraProgram' was not declared / 'PoseTracksSource' does not name a type` errors at `src/scene/camera/camera_v1/camera_program_compiler.cpp`).  The TICKET-034 changes themselves are isolated from that rot (composition.cpp compiles fine in isolation against the camera_v1 headers); TICKET-029's fix is a separate workstream. |
| **Symptom** | The default camera was a renderer-side constant (`Camera default_camera;` declared in `src/runtime/render_pipeline.cpp:61` and `src/backends/software/software_renderer.cpp:357`), with no authoring surface on the composition itself.  Authors either had to (a) mutate the legacy `Composition::camera` field directly (no compile path, no evaluation, no fingerprint), or (b) ignore the renderer constant and ship renderable code without a defined default.  No `CameraDescriptor` surface existed in composition settings.  The TICKET-029 `·` `DefaultCameraSpec` was a hypothetical alternative shape — explicitly NOT introduced per user spec ("Rimuovi DefaultCameraSpec come tipo separato"). |
| **Root cause analysis** | Composition's authoring surface only carried `CompositionSpec { name, width, height, frame_rate, duration, assets_root }` and a `SceneFunction m_render` closure.  Any default-authoring went through the legacy `Camera camera` field, which is a SLIM struct (`transform + fov_deg + near_plane + far_plane`) — unsuitable for `Camera2_5D`-shaped authoring (parent_name, target_name, point_of_interest, DoF, lens, motion_blur).  The user spec demands: (1) the default camera is a normal `CameraDescriptor` on composition settings; (2) compiled by the canonical `compile_camera()` pipeline; (3) evaluated by the same `CameraProgram` runtime; (4) serialisable + testable; (5) NEVER hidden as a renderer-side constant. |
| **Out-of-scope rationale for prior cleanup chains** | The premium on this ticket is "settings is canonical + renderable projection onto the slim legacy field is best-effort".  The legacy `Composition::camera` field is intentionally retained for backwards compatibility with golden harnesses that mutate it directly; new authoring goes through `Composition::default_camera_descriptor(...)` + `Composition::redecompose_camera_from_descriptor(...)`.  Full rich-field migration (parent_name / target_name / point_of_interest / DoF / Lens / MotionBlur / ZoomProjection-derived fov_deg) is deferred to TICKET-035 since the slim legacy `Camera` struct cannot carry those fields.  The header trade-off (full `#include <camera_v1/camera_descriptor.hpp>` + `<camera_v1/camera_program.hpp>` for value-member completeness) is documented; PIMPL re-light is queued for TICKET-035 to keep the "LIGHT authoring-header" goal achievable later. |
...
## TICKET-051 — A4.3 per-preset diagnostic: surface the cinematic composition that triggers the hash-collision
...
| **Discovered date** | 2026-06-24 |
| **Resolver** | AGENT-4 owner (re-base su `main`) |
| **Symptom** | `tools/verify_cinematic_showcase.sh` reports `A4.3 OK` MISSING (no marker found in `chronon3d_cinematic_camera_showcase_tests` stdout). The A4.3 TEST_CASE iterates the 5 cinematic compositions from `cinematic_text_camera.cpp` and asserts `hash(F0) ≠ hash(Fdur/2) ≠ hash(Fdur-1)` pairwise distinct; per-composition MESSAGE forensic dump prints `hash[0]/hash[mid]/hash[final]` + `(DISTINCT)` or `(COLLISION)` per preset. The FAIL'd run also surfaced `[error] missing FontEngine for text run` log lines → assets/Poppins-Bold.ttf non risolto per i compositions hero_text rendering. |
| **Root cause** | Probably: `rack_focus_title_swap` o `abyss_freefall_stagger` con `Poppins-Bold.ttf` non montato correttamente → hero_text/glyphs invisibili a tutti e 3 i sample frames → 3 framebuffers ridotti a (camera + bg) identici bit-by-bit → 3 hash collidenti che fanno scattare `CHECK(all_distinct)` o `CHECK(presets_passed == 5)`. Verifica con `--test-case='AGENT4: A4.3 text motion per preset*'` per ottenere la riga `(COLLISION)` per ogni composizione canaglia. |
| **Out-of-scope rationale** | AGENT-4 chiude per contract su 5/6 gates OK + contact sheet PNG valido; l'investigazione del failure specifico del composition è un followup che richiede root-cause sul FontEngine asset path resolution nel test binary, separabile da AGENT-4 atomic delivery. AGENTS.md §"Fare PR piccole e mirate" — una PR atomica non può contenere un asset-path debugging branch. |
| **Suggested fix approach** | 1) Eseguire `build/chronon/linux-ci/tests/chronon3d_cinematic_camera_showcase_tests --test-case='AGENT4: A4.3 text motion per preset*'` per ottenere le 5 righe forensic `(DISTINCT)` vs `(COLLISION)`. 2) Per ogni `(COLLISION)`, ispezionare il framebuffer A4.5 contact sheet visualmente o via ImageMagick per confermare assenza di hero_text. 3) Verificare `tests/test_main.cpp`'s `test_assets.mount(current_path())` + il path canonico `assets/fonts/Poppins-Bold.ttf` risolva correttamente dalla root del repo when CWD = `${CMAKE_SOURCE_DIR}` (presumibile che funzioni dato che A4.4 determinism OK per `deep_parallax_cascade` ha 4 text layers con lo stesso asset). 4) Se uno specifico composition produce collision nonostante l'asset OK, investigare se il composition timeline lascia il text invisibile a F0/dur/2/dur-1 (e.g. hero_text stagger delays > dur/2). |
| **Acceptance criteria** | (a) `tools/verify_cinematic_showcase.sh` riporta `A4.3 OK` cone `verdi` (5/5 cinematic compositions pass text-motion gate). (b) `grep -E '(COLLISION)' /tmp/agent4-binary.log` ritorna 0 hit. (c) Commit annota quale composition ha causato la collisione e perché, se non-FontEngine-related. |
...
## TICKET-054 — `build-fast.sh` target forwarding (`--target <name>`)

| Field | Value |
|---|---|
| **Status** | 🔵 Planned |
| **Affected file(s)** | `build-fast.sh` |
| **Discovered during** | AGENT-4 bootstrap+build attempt: `./build-fast.sh --target chronon3d_cinematic_camera_showcase_tests` ritorna `CMake Error: Invalid value used with --target` perché `build-fast.sh` non parsifica `--target`, solo positionali come `cli`, `dev-fast`, ecc. |
| **Discovered date** | 2026-06-24 |
| **Resolver** | AGENT-4 owner + dev-infra team |
| **Symptom** | AGENT-4 docs (incluso `AGENT_4_VERIFY_VISUAL_INTEGRATION.md`) indicano invocation `./build-fast.sh --target chronon3d_cinematic_camera_showcase_tests` ma `build-fast.sh` rifiuta il flag. Workaround documentato: `cmake --build build/chronon/linux-ci --target chronon3d_cinematic_camera_showcase_tests -j $(nproc)`. |
| **Root cause** | `build-fast.sh` mappa positional args a target name via tabella implicita; `--target` non è nella tabella e non è passato al `cmake --build` call. |
| **Out-of-scope rationale** | Pure dev-infra ergonomics; AGENT-4 docs workaround funziona (cmake --build direct). Fix migliora ergonomics ma non è bloccante. AGENTS.md §"Fare PR piccole e mirate" → meglio come commit separato. |
| **Suggested fix approach** | Extend `build-fast.sh` con parser semplice: se il primo arg è `--target`, esegui il prossimo arg target-name via `cmake --build "$BUILD_DIR" --target "$target"`. Oppure document esplicitamente nel AGENT-4 doc che il workaround è il comando canonico cmake direct invocation, e aggiungi una nota a README.md di `tools/`. |
| **Acceptance criteria** | `./build-fast.sh --target chronon3d_cinematic_camera_showcase_tests` exit rc=0 + binary linkato. |
...
|---|---|
| **Status** | 🔵 Planned |
| **Affected file(s)** | `tests/showcase/test_cinematic_camera_showcase.cpp` (post A4.6 gate, prima di fine TEST_CASE) |
| **Discovered during** | AGENT-4 code-review feedback post-commit (reviewer finding R6: no individual frame PNGs dumped). |
| **Discovered date** | 2026-06-24 |
| **Resolver** | AGENT-4 owner |
| **Symptom** | `output/showcase/contact_sheet.png` è l'unico output visivo AGENT-4; investigator che vuole ispezionare un singolo keyframe di un singolo composition (e.g. whip_pan_hero_reveal F022 mid-stagger per validare stagger typewriter timing) non può estrarre senza ricompilare o scrivere PNG dump separato. |
| **Root cause** | A4.5 contatta master grid → single PNG output; nessun path di dump per-keyframe. |
| **Out-of-scope rationale** | AGENT-4 chiude per contract con contact sheet master. Per-keyframe dump è una usability improvement per investigator forensics; non blocca AGENT-4 ma è complemento di TICKET-051 (quando A4.3 collision è investigata, per-keyframe dumps permetteranno di discriminare visivamente il collision culprit senza re-render. |
| **Suggested fix approach** | In A4.5 (o nuovo A4.7 forensic), dopo la cache A4.3 sweep, salvare per ogni composition × 3 hashes: `output/showcase/keyframe_<composition>_<frame>.png` via `save_png(master_or_fb, path)`. Totale: 15 PNG (5 compositions × 3 frames). Dimension expected: 5×3×10 MB ≈ 150 MB disk (gestibile). Opzionale: alternative `output/showcase_<preset>_<NNN>.png` se 15 file è troppo. |
| **Acceptance criteria** | Dopo AGENT-4 commit fix, `ls output/showcase/keyframe_*.png` lista 15 PNG (o 6+3 = 9 se si combina con A4.5 6-frame); ciascuno PNG valid via Python `struct.unpack('>II')`. |

---

## TICKET-056 — A4.4 + A4.5 memory envelope tuning (~600 MB transient)

| Field | Value |
|---|---|
| **Status** | 🔵 Planned (was TICKET-052 body content before 2026-06-24 closing-pass renumber; bumped to 056 to make space for AGENT-4 doc-lag closure under TICKET-052) |
| **Affected file(s)** | `tests/showcase/test_cinematic_camera_showcase.cpp` (master PNG allocation site), `tools/render_showcase_contact_sheet.sh` (post-processing site, no current changes) |
| **Discovered during** | AGENT-4 dry-run mental budgeting: 6 framebuffers A4.4 + 6 framebuffers A4.5 contact sheet × 1920×1080×16B ≈ 198 MB + 5760×2160 master PNG ≈ 199 MB raw = ~600 MB transient peak. |
| **Discovered date** | 2026-06-24 |
| **Resolver** | AGENT-4 owner + chronon3d_runtime team |
| **Symptom** | Su CI runners con 2 GB memory cap, `chronon3d_cinematic_camera_showcase_tests` potrebbe OOM durante A4.5 master PNG compositing. Test non è bloccante oggi (su host 4 GB+ lavora) ma rende A4.5 fragile per gated CI matrix. |
| **Root cause** | `Framebuffer master(kCompW * kCols, kCompH * kRows)` in A4.5 + 6 cached framebuffers + SoftwareRenderer internal state all concorrono nel RSS. 1920×1080×16B ≈ 33 MB per framebuffer; 6+ cache + master ≈ 200+200+overhead = ~600 MB. |
| **Out-of-scope rationale** | AGENT-4 chiude per contract: 5/6 OK gates + contact sheet PNG valido. Downscaling master PNG a 1920×720 implica modificare la logica di compositing (`blit` non downscale by default) — noioso refactor separabile. |
| **Suggested fix approach** | 1) **Path A**: Sostituire master PNG 5760×2160 con 1920×720 cell-resident downscaled via 3-step nearest-neighbor box-blit (manual nested-loop rows × cols). 2) **Path B**: Aggiungere env gate `CHRONON3D_AGENT4_SAVE_PNG=0` → skip A4.5 save_png in CI tight-runner case ma mantieni contact sheet localmente. 3) **Path C**: Liberare A4.4 Run A framebuffer cache prima di A4.5 (`run_a.clear()` determinism-test prima del master alloc). Path C è il minimo-effort refactor. |
| **Acceptance criteria** | Peak RSS durante `chronon3d_cinematic_camera_showcase_tests` < 400 MB su 2 GB CI runner. Master PNG dims opzionalmente ridotti a 1920×720 (Path A) oppure A4.5 PNG skipped su env gate (Path B). |

### Cross-references

- TICKET-053 (closing-pass commit, 2026-06-24) — the A4.6 lifetime-hook extension that provides measured heap envelope data. Once TICKET-053's runtime envelope measurement is in place, TICKET-056's budgeting becomes data-driven rather than rule-of-thumb-driven.
- TICKET-051 — A4.3 per-preset diagnostic; TICKET-056 retention-tune is a complementary forensic to TICKET-051's hash-collision root-cause investigation.

---

## TICKET-057 — Drop redundant `tests/helpers/render_fixtures.hpp` include

| Field | Value |
|---|---|
| **Status** | 🔵 Planned (was TICKET-053 body content before 2026-06-24 closing-pass renumber; bumped to 057 to make space for AGENT-4 memory-envelope closure under TICKET-053) |
| **Affected file(s)** | `tests/showcase/test_cinematic_camera_showcase.cpp` |
| **Discovered during** | AGENT-4 code-review feedback post-commit (reviewer finding R3 — redundant include). |
| **Discovered date** | 2026-06-24 |
| **Resolver** | AGENT-4 owner |
| **Symptom** | `#include <tests/helpers/render_fixtures.hpp>` è lista nei TU includes ma `make_renderer()` vive invece in `<tests/helpers/test_utils.hpp>` per convenzione canonica di `tests/text/test_text_preset_visual.cpp` e `tests/visual_tests.cmake`. Include ridondante → confusion su dove risieda il factory + possibile bloat compile-time se `render_fixtures.hpp` tira dentro helpers non usati. |
| **Root cause** | Initial AGENT-4 grep-against-helpers ha pescato due `#include <tests/helpers/...>` candidates; selezionati entrambi per safety invece che solo `test_utils.hpp`. |
| **Out-of-scope rationale** | Pure cosmetico; non blocca AGENT-4 contract. AGENTS.md §"Non duplicare helper/service locator" si applica → la canonica single-source-of-truth per `make_renderer()` è `test_utils.hpp`. |
| **Suggested fix approach** | Single-line str_replace: drop `#include <tests/helpers/render_fixtures.hpp>` da `tests/showcase/test_cinematic_camera_showcase.cpp` (mantieni singolo `#include <tests/helpers/test_utils.hpp>`). Verify build + ctest. |
| **Acceptance criteria** | `grep -E 'render_fixtures\.hpp' tests/showcase/test_cinematic_camera_showcase.cpp` ritorna 0 hit; `cmake --build build/chronon/linux-ci --target chronon3d_cinematic_camera_showcase_tests` rc=0. |

### Cross-references

- TICKET-053 — A4.6 lifetime-hook extension; the test file in TICKET-057's affected list is the same one TICKET-053 modified. Co-fix the include-cleanup in the same follow-up commit when the AGENT-4 closing-pass atomic commit lands.
- AGENT-4 atomic commit `db81d51d`: where the redundant include was introduced.
