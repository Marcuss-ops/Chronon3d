...
| M1.5#2 | text_run_driver.cpp orchestratore (530 LOC → slim) + 3 helpers in `src/text/driver/` (text_state_sampler / text_font_state / text_layout_rebuild) + struct pubblica `EffectiveTextState{text, FontLayoutIdentity, TextDirection, Bcp47LanguageTag, TextShapingFeatures}` in `include/chronon3d/text/text_run_driver.hpp` (operator== copre tutti i 5 campi in declaration order) + test_effective_text_state.cpp locka semantica 5-campi + crossfade lifecycle. `chronon3d_text_core` lib compile clean (`LIB_EXIT=0`); nessun nuovo singleton/registry/cache (cache flows via `TextRunShape::cache` o `TextLayoutCache*` esplicito). `process_wide_*` e `shared_text_layout_cache()` confermati rimossi (Fase B2+B3 chiusa). Stesso blocker pre-esistente `TICKET-camera-policy-pre-existing` blocca il test target anche per M1.5#2 — fuori scope M1.5#2 (one-commit-per-responsibility). | Done |
| TICKET-camera-policy-pre-existing (M1.5#2 carryover) | Stesso pre-existing rot di `src/render_graph/pipeline/camera_change_policy.cpp:24` (`Camera2_5D::projection_mode`) blocca ANCHE il build del target `chronon3d_core_tests` (necessario per eseguire `test_effective_text_state.cpp` + `test_text_run_driver.cpp`). `git diff HEAD -- src/render_graph/pipeline/camera_change_policy.cpp` vuoto per il commit M1.5#2 → NON causato da questo refactor. Suggested remediation: ripristinare il field `projection_mode` come accessor separato o refactor call-site verso `CameraProjection({...})` come già fa `CameraProjectionResolver` in M1.5 prerequisites (post-baseline). | PLANNED |
