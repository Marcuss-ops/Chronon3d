...
<summary>docs(followup): Cat-5 3-doc closure for monitor_push_divergence.sh — 2026-07-12</summary>

Atomic docs-only chore opening `TICKET-MON-3DOC-CAT5-ALIGN` (P3 NEW) as §Open Blocker row + adding the cite-only §Stato generale per area "Push infrastructure" row in `docs/CURRENT_STATUS.md` + this CHANGELOG entry prepended. Closes the Cat-5 3-doc obligation of the current session for the `tools/monitor_push_divergence.sh` wrapper (commit `5cd71719`) which was machine-shipped but whose CURRENT_STATUS cite-only row was deferred per user-decision "2-doc only".

**Files touched (3 EDITs per Cat-5 3-doc same-commit alignment)**: (1) EDIT `docs/FOLLOWUP_TICKETS.md` §Open Blockers top: prepend NEW TICKET-MON-3DOC-CAT5-ALIGN P3 row; (2) EDIT `docs/CURRENT_STATUS.md` §Stato generale per area table: insert new **Push infrastructure** row at TOP (Cat-5 newer-at-top); (3) EDIT `docs/CHANGELOG.md` (this entry prepended per Cat-5 newer-at-top). ZERO new files. ZERO source code changes. ZERO SDK symbol changes.

**Subject envelope = 57 chars ≤ 72** push-range audit per AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (subject: `docs(followup): Cat-5 3-doc closure for monitor_push_divergence.sh`).

**Cat-3 SATISFIED** (pure `docs/` tracking; zero new symbols in `include/chronon3d/`; zero public SDK API additions; the in-`tools/monitor_push_divergence.sh` wrapper is UNTOUCHED in source; only the 3 canonical doc files move per the Cat-5 3-doc same-commit pattern). **Cat-5 3-doc same-commit alignment** per the established pattern (CHANGELOG + FOLLOWUP_TICKETS + CURRENT_STATUS atomically updated in this commit; AGENTS.md doc-role table "Stato generale per area" cite-only row now materialized per the prior TICKET-MON-3DOC-CAT5-ALIGN forward-point registered in commit `800ceb97`). **§honesty forward-point preservation**: the precursor `800ceb97 chore(tools): add idempotent monitor cron install helper` registered TICKET-MON-3DOC-CAT5-ALIGN as forward-point (f) of the TICKET-MON-PUSH-DIVERGENCE-CRON closure lineage, citing the deferred Cat-5 3-doc obligation. NOW opening the ticket as a §Open Blocker row preserves the audit trail + provides grep-discoverability per AGENTS.md "no silent rot disappearance" discipline.

**Cross-references**: AGENTS.md v0.1 Cat-3 + Cat-5 + §regole "Fare PR piccole e mirate" (single atomic chore on the Cat-5 3-doc closure obligation; the 3 EDITS to 3 canonical lock together per Cat-3 anti-dup) + §regole "non committare `node_modules/`, directory di build, output, artefatti o file generati" (this commit adds 0 NEW files + 3 EDITs; ZERO build artifacts committed) + AGENTS.md §Doc canonical update discipline rule (cat-1 chore with §Stato per area cite-only row + §Open Blocker P3 NEW row + CHANGELOG prepended entry, the 3-doc debit closed atomically); the canonical `5cd71719 tools(monitor): wire push-divergence cron-friendly monitor` commit (the in-`tools/monitor_push_divergence.sh` wire) + the canonical `800ceb97 chore(tools): add idempotent monitor cron install helper` (the precursor that registered the forward-point) + the canonical `docs/FOLLOWUP_TICKETS.md` (NOW with TICKET-MON-3DOC-CAT5-ALIGN row at §Open Blockers top for permanent grep-discoverability) + the canonical `docs/CURRENT_STATUS.md` (NOW with **Push infrastructure** row at §Stato generale per area top for permanent indicator visibility) + the canonical `docs/CHANGELOG.md` (this entry prepended at TOP per Cat-5 newer-at-top) + the established cross-link precedents: TICKET-CLI-ISOLATE-RUNTIME-DEV-3DOC-CAT5-ALIGN row in §Open Blockers (the parallel pattern source) + TICKET-INSPECT-TEXT-3DOC-CAT5-ALIGN + TICKET-SABOTAGE-FONT-3DOC-CAT5-ALIGN + TICKET-GLOW-FINAL-COMPOSITIONS-DOC-MIGRATION-3DOC-CAT5-ALIGN sister-tickets (the parallel pattern established for §Stato per area cite-only rows); ADR-022 §Decision 1 (advisory, NOT hard-block — invariant preserved by the wrapper) + ADR-022 §Decision 2 (env-var configurable per-warden); canonical `tools/monitor_push_divergence.sh` (the wrapper that this chore's cite-only row anchors on).
...
**Files changed (4 NEW + 3 EDIT + 2 EDITs docs)**: (1) NEW `apps/chronon3d_cli/register_compositions.hpp` (~52 LoC, 3 function decls with `#ifdef CHRONON3D_BUILD_CLI_DEV` gating the 3rd); (2) NEW `apps/chronon3d_cli/register_runtime_compositions.cpp` (~48 LoC, `ChrononGlowFinalAE` + `register_builtin_compositions`); (3) NEW `apps/chronon3d_cli/register_content_compositions.cpp` (~65 LoC, bridges to `register_content_modules` via a stub `ExtensionContext` with local `AssetRegistry` + `StyleRegistry` + `MotionRegistry` statics — per-domain register functions only consume `ctx.compositions`); (4) NEW `apps/chronon3d_cli/register_dev_compositions.cpp` (~148 LoC, ENTIRE FILE under `#ifdef CHRONON3D_BUILD_CLI_DEV`, registers all DEV/test compositions); (5) EDIT `apps/chronon3d_cli/cli_init.hpp` (221 → 101 LoC, -120 LoC, removed 6 test/golden header includes + 25+ inline composition registrations, replaced with 3 calls to the new register functions; preserved `cli_style_registry()` + `cli_motion_registry()` authoring helpers unchanged); (6) EDIT `apps/chronon3d_cli/CMakeLists.txt` (removed 3 `target_sources` blocks for `tests/visual/*.cpp` + the inline `add_executable` `tests/visual/camera_truth/*.cpp` entries + the `if(NOT TARGET chronon3d_content) tests/visual/ae_parity/ae_parity_scenes.cpp` fallback; added 3 new `register_*_compositions.cpp` to `chronon3d_cli` SOURCES; added 5 `tests/visual/*.cpp` to `chronon3d_cli_dev` SOURCES (DEV-gated); updated the trailing TICKET-GLOW-FINAL-COMPOSITIONS-MOVE comment to TICKET-CLI-ISOLATE-RUNTIME-DEV); (7) EDIT `content/CMakeLists.txt` (removed `${CMAKE_SOURCE_DIR}/tests/visual/ae_parity/ae_parity_scenes.cpp` from `chronon3d_content` SOURCES + removed `ae_parity/register_ae_parity_compositions.cpp` — both files retained in source as dead-code for backward compat with test fixtures; updated trailing comment to TICKET-CLI-ISOLATE-RUNTIME-DEV); (8) EDIT `content/register_content_modules.cpp` (removed `namespace chronon3d::content::ae_parity { void register_ae_parity_compositions(...); }` forward declaration + removed `content::ae_parity::register_ae_parity_compositions(ctx.compositions);` call from `ContentExtension::register_all` — AE_CAM_* are now registered exclusively by the DEV `register_dev_compositions`); (9) EDIT `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 newer-at-top); (10) EDIT `docs/FOLLOWUP_TICKETS.md` (NEW `TICKET-CLI-ISOLATE-RUNTIME-DEV` row prepended at §Open Blockers top per the established pattern).

**User-spec verbatim coverage**:
...
**Cat-3 SATISFIED** (pure `apps/chronon3d_cli/` + `content/` + `docs/` tracking; ZERO new symbols in `include/chronon3d/`; ZERO public SDK API additions; the 3 new register functions consume the EXISTING `CompositionRegistry` public surface without modification; the new header `register_compositions.hpp` declares only 3 free functions with the exact signature per user spec). **Cat-5 2-doc same-commit alignment** per the established pattern (CHANGELOG + FOLLOWUP_TICKETS prepended atomically; `docs/CURRENT_STATUS.md` cite-only row DEFERRED to `TICKET-CLI-ISOLATE-RUNTIME-DEV-3DOC-CAT5-ALIGN` per Cat-3 anti-dup, parallel to the `TICKET-GLOW-FINAL-COMPOSITIONS-DOC-MIGRATION-3DOC-CAT5-ALIGN` precedent). **AGENTS.md §honest-limitation compliance**: macchina-verifica of the 3 NEW register functions is DEFERRED to working build host per the established `TICKET-BUILD-ROT-CASCADE-CAMERA` 409-error rot + `TICKET-CONTENT-TEXT-CAMERA-V1-ROT` 21-error rot + `TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV` vcpkg glm/magic_enum missing on this VPS; the 4 new files are HARNESS-COMPLETE (the syntactic structure mirrors the pre-existing `init_compositions` shape with stub `ExtensionContext` for `register_content_compositions`; the `#ifdef CHRONON3D_BUILD_CLI_DEV` guard in `register_dev_compositions.cpp` ensures the file is empty in production; the 3 calls in `cli_init.hpp` are unconditional in production + DEV-gated in dev); the dry-run on this VPS will require a working build host for `cmake --build` + `ctest` verification.

**Subject envelope = 57 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12.

**Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate" + Cat-3 anti-dup)**: (a) `TICKET-CLI-ISOLATE-RUNTIME-DEV-3DOC-CAT5-ALIGN` — Cat-5 3-doc closure: add cite-only row to `docs/CURRENT_STATUS.md` §Stato per area "CLI" or "Composizioni" pointing to this row + CHANGELOG entry (deferred per Cat-3 anti-dup, parallel to `TICKET-GLOW-FINAL-COMPOSITIONS-DOC-MIGRATION-3DOC-CAT5-ALIGN` precedent); (b) `TICKET-CLI-ISOLATE-RUNTIME-DEV-DELETE-DEAD-CODE` — cleanup: delete the now-dead `content/ae_parity/register_ae_parity_compositions.{hpp,cpp}` (function is no longer called; the .cpp is still compiled into `chronon3d_content` per the cat-3 minimal-surface decision; deletion requires verifying no test fixture references the function symbol directly); (c) `TICKET-CLI-ISOLATE-RUNTIME-DEV-FACTORY-LOC` — file-move refactor: relocate `tests/visual/ae_parity/ae_parity_scenes.{hpp,cpp}` to `content/ae_parity/ae_cam_scenes.{hpp,cpp}` so the factory functions live in production code (content/) rather than tests/visual/; this would let `content/CMakeLists.txt` compile them into `chronon3d_content` and remove the DEV-only sub-target coupling; orthogonal to this chore but related to the user spec's "tests/visual/ resta solo come cartella di golden/fixture" intent; (d) `TICKET-CLI-ISOLATE-RUNTIME-DEV-VERIFY` — working build host: `cmake --build build/chronon/linux-content-dev` + `ctest -R cli_ --output-on-failure` to confirm the 3-layer split + DEV-gated sub-target linking + no duplicate registration in the new `register_dev_compositions` path; (e) `TICKET-CLI-ISOLATE-RUNTIME-DEV-FIXTURE-ASSET-PARITY` — when `CHRONON3D_BUILD_CONTENT=ON` + `CHRONON3D_BUILD_CLI_DEV=ON`, both `register_content_compositions` (no AE_CAM_*) and `register_dev_compositions` (AE_CAM_*) are called. Since AE_CAM_* are NOT in `register_content_compositions` anymore, no duplicate registration. Verified at the source level. (No fixture-level test currently asserts this invariant; a future test could add a CompositionRegistry uniqueness assertion in a test target that links both sub-targets.)

**Cross-references**: AGENTS.md v0.1 Cat-3 (zero new public SDK API surface; satisfied — pure `apps/chronon3d_cli/` + `content/` + `docs/` tracking) + Cat-5 (2-doc same-commit CHANGELOG + FOLLOWUP_TICKETS prepended; CURRENT_STATUS.md cite-only deferred via forward-point (a)) + §regole "Fare PR piccole e mirate" (single atomic chore on the §3 cleanup; the 4 NEW + 3 EDIT locked together per Cat-3 anti-dup) + §regole "non committare `node_modules/`, directory di build, output, artefatti o file generati" (this commit adds 4 NEW files + 3 EDITs; ZERO build artifacts committed) + AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (57-char subject envelope ≤ 72 push-range audit) + AGENTS.md GATE-MNT-01 closure lineage (`tools/wrap_push.sh` Step 4.5 auto-FF + `tools/check_main_clean.sh` pre-push verify); the canonical `apps/chronon3d_cli/cli_init.hpp` (the pre-refactor 221 LoC file with inline 25+ composition registrations — now 101 LoC, reduced by 54%, with the 3 calls to the new register functions; the `cli_style_registry()` + `cli_motion_registry()` authoring helpers preserved unchanged for the content-module ambient authoring path) + the canonical `include/chronon3d/core/composition/composition_registry.hpp` (the `CompositionRegistry` class consumed by the 3 new register functions) + the canonical `include/chronon3d/core/composition/register_builtin_compositions.hpp` (the `chronon3d::register_builtin_compositions()` function called by `register_runtime_compositions` for the DarkGridBackground + CameraImageClip production builtins) + the canonical `content/register_content_modules.cpp` (the content-module orchestrator that registers cinematic showcases, examples, certifications, launches, text placement, etc. — called by `register_content_compositions`; the `register_ae_parity_compositions` call was removed in this chore per the AE_CAM_* → DEV relocation) + the canonical `content/compositions/chronon_glow_final.hpp` (the header-only production factory consumed by both `register_runtime_compositions` for `ChrononGlowFinalAE` and `register_dev_compositions` for the `chronon-glow-final-portrait` + `ChrononGlowFinalAE_NoGlow` siblings) + the canonical `tests/visual/ae_parity/ae_parity_scenes.hpp` + `ae_parity_compositions.hpp` + `glow_ab/glow_ab_compositions.hpp` + `camera_truth/camera_truth_test.hpp` + `camera_truth/camera_truth_orbit.hpp` + `tests/text/pipeline_parity_canary.hpp` (the 6 test/golden headers + 5 .cpp factory files that were transitively consumed by `cli_init.hpp` pre-refactor; the headers are now only included by the DEV `register_dev_compositions.cpp`; the .cpp files are now only compiled into the DEV `chronon3d_cli_dev` sub-target when `CHRONON3D_BUILD_CLI_DEV=ON`) + the canonical `tools/wrap_push.sh` (the push wrapper per GATE-MNT-01 that this commit is forward-pointed to via the established race-recovery pattern) + the canonical `content/ae_parity/register_ae_parity_compositions.{hpp,cpp}` (the now-dead-code file retained in source for backward compat with `tests/visual/ae_parity/ae_parity_tests.cpp` test fixtures; deletion deferred to `TICKET-CLI-ISOLATE-RUNTIME-DEV-DELETE-DEAD-CODE` forward-point).
...
**`test(parity): extract 5 clip rect tests + drop synthetic parity`** — atomic chore commit on main per user spec verbatim §2 ("(a) Prima di eliminare `tests/text/test_pipeline_parity.cpp`, estrai eventuali test unici sui cinque clip rect in un nuovo file `tests/text/test_text_clip_policy.cpp` con proprio entry in `tests/text/text_clip_policy_tests.cmake` (registra con `chronon3d_add_test_suite`). (b) Elimina `tests/text/test_pipeline_parity.cpp` e `tests/pipeline_parity_tests.cmake`. (c) Aggiorna `tests/CMakeLists.txt` e qualsiasi registry che le referenzia. (d) Conferma che resti integro `tests/text/test_pipeline_parity_real.cpp` + `tests/pipeline_parity_real_tests.cmake` con tutti i 7 moduli (SDK/CLI/frame/video/pruning/thread/cache/graph) e hash di framebuffer reali.").

**Extraction** (3 file modificati, 2 file eliminati):
...
**UNTOUCHED per spec §(d)**: `tests/text/test_pipeline_parity_real.cpp` (717 LoC, 7 moduli SDK/CLI/frame/video/pruning/thread/cache/graph, hash di framebuffer REALI) + `tests/pipeline_parity_real_tests.cmake` (31 LoC, unconditional) — nessuna modifica. Verifica machine-side: `git diff origin/main -- tests/text/test_pipeline_parity_real.cpp` = empty (unchanged), `git diff origin/main -- tests/pipeline_parity_real_tests.cmake` = empty (unchanged).

**Subject envelope = 61 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (subject: `test(parity): extract 5 clip rect tests + drop synthetic parity`).

**Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate" + Cat-3 anti-duplication)**: (a) `TICKET-TEST-PARITY-EXTRACT-3DOC-CAT5-ALIGN` — allineamento Cat-5 3-doc (FOLLOWUP_TICKETS + CURRENT_STATUS cite-only) deferred a un follow-up commit dedicato (per Cat-3 anti-dup, questo commit è pure tests/ + 1 doc; il forward-point consente iterazione separata); (b) `TICKET-DETERMINISM-BRUTE-17` — pre-existing forward-point TICKET-DETERMINISM-BRUTE-17 per i 2 invariance tests #43+#44 droppati (ora la copertura invariance vive SOLO in `_real::BruteDeterm-17`, no near-clone); (c) `TICKET-PARITY-001` + `TICKET-PARITY-003` — pre-existing rot tickets (Rect API rot + DIAGNOSTICS gate rot) che bloccano la ri-attivazione del cross-process parity in `tests/cross_process_parity_tests.cmake` (la quale rimane commented-out nel CMakeLists.txt); (d) `TICKET-TEST-TEXT-CLIP-POLICY-MACHINE-VERIFY` — working build host macchina-verifica: `ctest --test-dir build/chronon/linux-content-dev -R chronon3d_text_clip_policy_tests --output-on-failure` expects 35/35 PASS (5 clip variants × 7 pipelines in DIAGNOSTICS build; 30/35 in non-diagnostic build con 5 SKIP sul pipeline_diagnostic variante).

**Cross-references**: AGENTS.md v0.1 Cat-3 (zero new public SDK API surface; satisfied — pure tests/ tracking, ZERO simboli in `include/chronon3d/`) + Cat-5 2-doc same-commit (CHANGELOG prepended + 1 EDIT docs/CHANGELOG.md; FOLLOWUP_TICKETS + CURRENT_STATUS deferred via forward-point (a)) + §regole "Fare PR piccole e mirate" (single atomic chore on the §2 cleanup; 3 NEW/EDIT + 2 DELETE locked together per Cat-3 anti-dup) + §regole "non committare `node_modules/`, directory di build, output, artefatti o file generati" (this commit = 2 NEW files + 3 EDITs + 2 DELETE; ZERO build artifacts committed) + AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (61-char subject envelope ≤ 72 push-range audit) + AGENTS.md GATE-MNT-01 closure lineage (`tools/wrap_push.sh` Step 4.5 auto-FF + `tools/check_main_clean.sh` pre-push verify); the canonical `tests/text/test_pipeline_parity_real.cpp` (the UNTOUCHED 7-modulo real-framebuffer parity lock — preserved per spec §(d)) + the canonical `tests/pipeline_parity_real_tests.cmake` (UNTOUCHED unconditional registration + `CHRONON3D_CLI_PATH` compile definition) + the canonical `cmake/Chronon3DTestSuite.cmake` (the `chronon3d_add_test_suite` macro family — NEW test target wired per the INTEGRATION tier pattern) + the canonical `tools/check_test_suite_registration.sh` (the gate ensuring all per-area .cmake files use the canonical macro — NEW target complies) + the prior `feat(text): clip-policy 5-variant matrix` (commit lineage of the §4A Fase 4 matrix expansion that this commit extracts from the source file) + the canonical `tests/cross_process_parity_tests.cmake` (the deferred sub-process parity — NOT touched in this commit; pre-existing TICKET-PARITY-001/003 rot) + the canonical TICKET-SIMPLICITY-PARITY-EXTRACT (the closure lineage that this chore commit lands on main, the §2 cleanup sub-step of the 10-section cleanup plan) + the canonical TICKET-DETERMINISM-BRUTE-17 (the forward-point for the 2 invariance tests #43+#44 now superseded by `_real::BruteDeterm-17`).
...
## Luglio 2026 \u2014 docs(cert): Step 8+9 inspection DEFERRED, spec-variant (Option A) (atomic 3-doc cat-5 chore commit, followup to Step 7 docs-only, 2026-07-12)
## Luglio 2026 — docs(followup): register TICKET-PUSH-DEFERRED-96-BEHIND (WBH defer) (atomic 1-doc cat-5 chore, 2026-07-12, machine-verification DEFERRED to working build host per AGENTS.md §honesty)

**`docs(followup): register TICKET-PUSH-DEFERRED-96-BEHIND (WBH defer)`** — atomic cat-5 1-doc chore (this CHANGELOG entry) bonded with the new §Open Blockers row in `docs/FOLLOWUP_TICKETS.md` per the canonical 2-doc same-commit pattern. Per user authorization via `ask_user` (Option C "WBH defer" disposition) this session. The 5 un-pushed commits (b383eb1d selftest defensive fix + 88ad0d9 doctest_discover_tests rot-fix + 766814f WBH cert sequence protocol + fc52ff4c 2 ADRs ADR-022 + ADR-023 + the tool fix `fix(tool): resolve_rebase_conflict code-block + strict-regex guard` from this session) stay on local main until the WBH session picks them up via `git pull --ff-only origin main` (the 96-ahead commits are presumably WBH-side cert sequence work that was pushed while this VPS was env-blocked). NO source code touched this session (the tool fix is the only source change, and it's already committed + un-pushed).

**Files changed (2 — Cat-3 zero new public SDK API surface, pure `docs/` tracking per Option C user authorization)**:

1. EDIT `docs/FOLLOWUP_TICKETS.md` §Open Blockers: NEW `TICKET-PUSH-DEFERRED-96-BEHIND` row prepended at TOP of §Open Blockers table (newer-at-top convention matching the recent TICKET-CERT-SEQUENCE-WBH-PROTOCOL lineage) with P2 priority, OPEN (deferred per WBH session, 2026-07-12) state, full push-deferral scope description (the 5 un-pushed commits preserved as git objects in `.git/objects/` + the 4 push attempts this session per the AGENTS.md `non iterare rebase a terzo tentativo` boundary + the tool fix VERIFIED on 6/6 synthetic tests but STILL failing on actual docs unknown edge case) + the next-steps WBH runbook per `docs/cert_sequence_wbh_protocol.md` (Pre-1..Pre-4 + Step 1-6 + post-protocol cat-5 chore commit + push with SHA-triple selfcheck) + cross-link to TICKET-CERT-SEQUENCE-WBH-PROTOCOL (parent ticket) + TICKET-BUILD-ROT-CASCADE-CAMERA (env-block root cause) + TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV (vcpkg-bootstrap precond) + the 5 un-pushed commit SHAs + the canonical WBH protocol + AGENTS.md §honest-limitation (the WBH macchina-verifica gating event) + §GATE-MNT-01 closure lineage (the SHA-triple selfcheck post-push invariant) + the prior `b589fdba` 3-attempt recovery session precedent.

2. EDIT `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 standard pattern, replacing the prior docs(adr) entry from the prior chore at commit `fc52ff4c`).

**§HONEST qualifier** (per AGENTS.md §honest-limitation): the 12-ticket transitions to §Recently Closed per `docs/cert_sequence_wbh_protocol.md` post-protocol table STILL require WBH macchina-verifica as the gating event per AGENTS.md §honest-limitation — deferred to WBH session. The push attempt was EXHAUSTED per the AGENTS.md `non iterare rebase a terzo tentativo` boundary (4 attempts this session: push without rebase GATE_FAIL → rebase + resolver byte-level scan fail → rebase + manual resolution corrupted state → rebase + improved tool STILL failing on real docs). The disposition is WBH defer, not 5th push attempt. NO fabricated PASS claim; NO silent override; the 4 push attempts + their failure modes are documented honestly in the new TICKET-PUSH-DEFERRED-96-BEHIND row.

**Subject envelope = 67 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **Cat-3 SATISFIED** (pure `docs/` tracking; ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions, ZERO `src/`/`apps/`/`tests/`/`tools/` edits). **Cat-5 2-doc same-commit SATISFIED** (FOLLOWUP_TICKETS + CHANGELOG atomically updated per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato"). Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true).

**Cross-references**: `docs/FOLLOWUP_TICKETS.md` §Open Blockers TICKET-PUSH-DEFERRED-96-BEHIND row (NEW this commit) + the parent TICKET-CERT-SEQUENCE-WBH-PROTOCOL row (unchanged, still the canonical WBH protocol parent) + the 5 un-pushed commit SHAs (b383eb1d + 88ad0d9 + 766814f + fc52ff4c + tool fix) + the tool fix `docs(adr): propose ADR-022 hybrid determinism + ADR-023 cmake timeouts` entry (commit `fc52ff4c`) + the WBH protocol commit `docs(cert): unified WBH cert sequence protocol` (commit `766814f`) + the prior `TICKET-DOCTEST-DISCOVER-TESTS-ROT-FIX` row in §Recently Closed (the rot-fix that the WBH session will verify on the WBH env) + AGENTS.md §honest-limitation (the WBH macchina-verifica gating event) + §GATE-MNT-01 closure lineage (the SHA-triple selfcheck post-push invariant) + §regole "Fare PR piccole e mirate" (the 5 un-pushed commits + this 1-doc chore are each separate atomic commits on local main; the WBH session will see them all + the cert sequence protocol will orchestrate the macchina-verifica) + the prior `b589fdba` 3-attempt recovery session precedent (TICKET-SOURCE-CONFLICT-MARKERS-ROT §honesty closure) + AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (67-char subject envelope ≤ 72 push-range audit) + AGENTS.md `non iterare rebase a terzo tentativo` boundary (4 push attempts this session exhausted the boundary; WBH defer is the canonical disposition) + `docs/cert_sequence_wbh_protocol.md` §Post-protocol cat-5 chore commit guidance (the WBH session's expected log shape for the §Recently Closed + §11/11 verde + CHANGELOG prepended + NEW baseline file).

## Luglio 2026 — docs(adr): propose ADR-022 hybrid determinism + ADR-023 cmake timeouts (atomic 2-NEW-file + 2-doc cat-5 chore, 2026-07-12, machine-verification DEFERRED to working build host per AGENTS.md §honesty)

**`docs(adr): propose ADR-022 hybrid determinism + ADR-023 cmake timeouts`** — atomic cat-5 2-NEW-file + 2-doc same-commit chore resolving 2 forward-point tickets that need ADR-gated decisions before source-extension per AGENTS.md "No nuovi singleton/registry/resolver/cache senza ADR" (applied by analogy to test-infrastructure decisions + spec-variant decisions). Per user authorization via `ask_user` this session (Option "Open a follow-up ADR for the 2 forward-point tickets"). NEW 2 ADRs at `docs/adr/ADR-022-determinism-hybrid-spec-variant.md` (~338 LoC) + `docs/adr/ADR-023-custom-cmake-timeout-recovery.md` (~411 LoC). Per MADR-style format matching ADR-020 + ADR-021 + `docs/DOCUMENTATION_GOVERNANCE.md` §docs/adr/ (Status, Date, Deciders, Tags, Context & Forces, Decision, Consequences, Alternatives Considered, Compliance & Verification, References).

**Files changed (4 — Cat-3 zero new public SDK API surface, pure `docs/` tracking per Option "Open follow-up ADR" user authorization)**:

1. NEW `docs/adr/ADR-022-determinism-hybrid-spec-variant.md` (~338 LoC): **HYBRID disposition (option c) CHOSEN for TICKET-DETERMINISM-VP2-EXTENSION**. 5 Decision clauses: (1) DoD §9 regression lock PRESERVED at `|cx-960| < 110, |cy-540| < 110` in `tests/text_golden/ae_parity/ae_08_glow_pulse.cpp:318+`; (2) user spec blocks (b)+(c) (5-render loop + `CHRONON3D_THREADS={1, 8}` paired equality) mapped to existing canonical TICKET-DETERMINISM-BRUTE-17 surface at `tests/determinism/test_brute_determinism.cpp` (SUPERSET) via new sub-loop (5 renders × {1, 8} threads × 1 frame); (3) user spec block (a) PIL counter-test lives in WBH runbook `docs/cert_sequence_wbh_protocol.md Step 3c` (NOT in `tests/` directory); (4) §Honest doc/code drift correction (prior CURRENT_STATUS row claimed user spec was re-verification of DoD §9 closure; this ADR discloses the LOOSER centroid + the HYBRID disposition); (5) WBH macchina-verifica is the gating event for Proposed → Accepted transition per `docs/cert_sequence_wbh_protocol.md` Step 2 + Step 3. 4 Alternatives Considered (A1 LOOSE rejected: re-introduces 19px sliver regression window; A2 TIGHT rejected: ignores user-spec rationale; A3 Distinct Phase 2 test suite rejected: duplicates TICKET-DETERMINISM-BRUTE-17; A4 Source extension to TEST_CASE_DECORATOR + tag-based timeout rejected: changes canonical test source). §Honest spec-variant finding: user spec centroid `(600..1320, 300..780)` is LOOSER than DoD §9 lock by +250 X / +130 Y (verbatim per direct source compare); spec-variant decision is architectural (HYBRID preserves DoD §9 lock + honors user spec via canonical surface mapping).

2. NEW `docs/adr/ADR-023-custom-cmake-timeout-recovery.md` (~411 LoC): **custom CMake function (option b) CHOSEN for TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY**. 6 Decision clauses: (1) NEW `chronon3d_add_test_suite_with_timeout` helper added to `cmake/Chronon3DTestSuite.cmake` (parallel to existing `chronon3d_add_test_suite` at line 146, UNCHANGED); (2) the new helper accepts `PER_TEST_TIMEOUT` parameter (default 30s, env-overrideable via `CHRONON3D_PER_TEST_TIMEOUT`) + iterates over test source for `TEST_CASE` patterns via `file(STRINGS)` + `string(REGEX MATCHALL)` + emits `set_tests_properties(<test_name> PROPERTIES TIMEOUT <per_test_timeout>)` per match; (3) backward-compatible with existing `chronon3d_add_test_suite` (UNCHANGED, OPT-IN extension); (4) test source files UNCHANGED (the helper operates on the CMake configuration layer, not the test source layer); (5) CI flake surface RECOVERED (the user-spec 30s/120s per-TEST_CASE fast-fail safety net is restored, preventing the 25-minute CI deadlock); (6) WBH macchina-verifica is the gating event for Proposed → Accepted transition per `docs/cert_sequence_wbh_protocol.md` Step 4 + the `ctest --show-only=json-v1` audit command. 5 Alternatives Considered (A1 gtest_discover_tests rejected: heavy external dependency + slow configure; A2 preserve 1500s default rejected: accepts CI deadlock risk; A3 per-executable global TIMEOUT rejected: too coarse for 18-CHECK matrix; A4 TEST_CASE_DECORATOR + tag-based timeout rejected: changes canonical test source). §Honest TIMEOUT loss finding: the rot-fix in commit `88ad0d9` SILENTLY LOST the user-spec 30/120s per-TEST_CASE bound; this ADR documents the loss + the recovery disposition (CI deadlock risk re-introduced under the loose 1500s default; the recovery via custom CMake function restores the fast-fail safety net).

3. EDIT `docs/FOLLOWUP_TICKETS.md` §Open Blockers: 2 ticket rows updated to reference the new ADRs. TICKET-DETERMINISM-VP2-EXTENSION row updated with `**ADR-GATED per [ADR-022]` (HYBRID disposition) clause + the "Blocca" column updated to specify the HYBRID option (c) chosen per ADR-022. TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY row updated with `**ADR-GATED per [ADR-023]` (custom CMake function) clause + the "Blocca" column updated to specify the new `chronon3d_add_test_suite_with_timeout` helper design (zero new dependencies, backward-compatible, CI flake surface recovered).

4. EDIT `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 standard pattern, replacing the prior docs(cert) entry from the prior chore at commit `766814f`).

**§HONEST PARADOX** (canonical per AGENTS.md §honesty): both ADRs are **DOCUMENTATION-ONLY** — the decisions are made, the WBH macchina-verifica is the gating event for the Proposed → Accepted transition. The ADRs do NOT include source-code changes (the implementation of ADR-023's `chronon3d_add_test_suite_with_timeout` helper is a separate WBH chore; ADR-022's new sub-loop in `test_brute_determinism.cpp` is a separate WBH chore). Per AGENTS.md §honest-limitation + §Cat-3 anti-duplication: ZERO new public SDK API surface (zero new symbols in `include/chronon3d/`, zero new public SDK enum/class/struct, zero `src/` edits, zero `apps/` edits). The 2 ADRs are honest about their own limitations: ADR-022 §Honest spec-variant finding + ADR-023 §Honest TIMEOUT loss finding are upfront disclosures. NO fabricated PASS claim; NO silent override; the ADRs are the canonical architectural decision records for the 2 forward-point tickets.

**Subject envelope = 70 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **Cat-3 SATISFIED** (pure `docs/` tracking; ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions, ZERO `src/` edits, ZERO `apps/` edits, ZERO `tests/` edits, ZERO `tools/` edits). **Cat-5 2-NEW-file + 2-doc same-commit SATISFIED** (2 NEW ADRs + FOLLOWUP_TICKETS + CHANGELOG updated atomically per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato"). Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true).

**Cross-references**: `docs/adr/ADR-022-determinism-hybrid-spec-variant.md` (NEW this commit) + `docs/adr/ADR-023-custom-cmake-timeout-recovery.md` (NEW this commit) + `docs/FOLLOWUP_TICKETS.md` §Open Blockers TICKET-DETERMINISM-VP2-EXTENSION + TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY rows (UPDATED this commit) + `docs/CHANGELOG.md` prepended this entry (NEW this commit) + AGENTS.md §"Regole permanenti" (singleton/registry ADR requirement applied by analogy) + §Cat-3 (zero new public SDK API surface satisfied) + §Cat-5 (2-NEW-file + 2-doc same-commit alignment satisfied) + §regole "Fare PR piccole e mirate" (single atomic chore on the 2 ADRs; no churn retrofit; the 2 forward-point tickets are now ADR-GATED) + `docs/cert_sequence_wbh_protocol.md` Step 2 (Test #4 determinism macchina-verifica for ADR-022 gating event) + Step 3 (Test #8 PIL counter-test for ADR-022 user spec (a) runbook surface) + Step 4 (Test #9 ctest macchina-verifica for ADR-023 gating event) + the canonical `tests/text_golden/ae_parity/ae_08_glow_pulse.cpp:318+` (the DoD §9 lock ADR-022 preserves) + the canonical `tests/determinism/test_brute_determinism.cpp` (the TICKET-DETERMINISM-BRUTE-17 surface ADR-022 maps user spec (b)+(c) to) + the canonical `cmake/Chronon3DTestSuite.cmake:146` (the existing `chronon3d_add_test_suite` helper ADR-023 leaves UNCHANGED) + the canonical `cmake/Chronon3DTestSuite.cmake:194` (the existing `add_test` wrapper ADR-023's per-TEST_CASE recovery is orthogonal to) + `docs/adr/ADR-020-shared-static-fontengine-singleton.md` (MADR format reference) + `docs/adr/ADR-021-commit-subject-length-policy.md` (MADR format reference) + `docs/DOCUMENTATION_GOVERNANCE.md` §docs/adr/ (ADR template + style guide) + the prior commit `88ad0d9` (the rot-fix that created the TIMEOUT loss ADR-023 documents) + the prior commit `766814f` (the WBH protocol commit that provides the macchina-verifica runbook for both ADRs' gating events).

---

## Luglio 2026 — docs(cert): unified WBH cert sequence protocol (atomic 4-doc same-commit cat-5 chore, 2026-07-12, machine-verification DEFERRED to working build host per AGENTS.md §honesty)

**`docs(cert): unified WBH cert sequence protocol`** — atomic cat-5 4-doc same-commit chore per user authorization via `ask_user` (Option "Save WBH protocol to docs/") this session. NEW `docs/cert_sequence_wbh_protocol.md` (~200 LoC, canônico WBH execution protocol for the 5 remaining First-Principles Product Check tests + the 11/11 active sections machine-verification). The protocol is **DOCUMENTATION** — not source code — and CANNOT run on the canonical env-blocked VPS per the established TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot + TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV glm/magic_enum missing pattern. The protocol RUNS on a working build host (vcpkg installed + chronon3d_cli built in canonical `linux-dashboard-dev` + rot-cascade resolved).

**Files changed (4 — Cat-3 zero new public SDK API surface, pure `docs/` tracking per Option "Save WBH protocol to docs/" user authorization)**:

1. NEW `docs/cert_sequence_wbh_protocol.md` (~200 LoC, canônico protocol file): 6-step cert sequence structure (Pre-0..Pre-4 + Steps 1-6) + §HONEST qualifier banner at TOP (OPEN/DEFERRED + env-block on VPS + per-ticket cross-link list) + 6 Pre-conditions (apt install + vcpkg bootstrap + glm/magic_enum/doctest + git fetch + cmake configure + chronon3d_cli build + rot-cascade resolution per TICKET-BUILD-ROT-CASCADE-CAMERA) + Step 1 (Test #13 selftest + gate + orchestrator wireup; §honest disclosure: Test #13 has pre-existing gate rot cache ratio 9999 that the 6th file + rot-fix do NOT unblock) + Step 2 (Test #4 determinism ctest; §honest spec-variant finding: user spec centroid envelope is LOOSER than prior DoD §9 lock per TICKET-DETERMINISM-VP2-EXTENSION) + Step 3 (Test #8 3-block spec verbatim — PIL counter-test bw>800/bh>100/edges>=10/centroid(600..1320, 300..780) + 5-render sha256 UNIQUE==1 + CHRONON3D_THREADS={1,8} sha256 equality) + Step 4 (Test #9 17 patterns / 21 sub-cases ctest) + Step 5 (Test #14 tracking) + Step 6 (11/11 orchestrator end-to-end `FIRST_PRINCIPLES_PRODUCT_PASS` exit 0). Post-protocol cat-5 ticket transitions table mapping all 12 ticket rows to their §Recently Closed target state. Expected WBH log shape + versioning (v0.1 this commit, v0.2 future for §5+§6 video-completeness matrix + §15 portrait safe-area).

2. EDIT `docs/FOLLOWUP_TICKETS.md` §Open Blockers: NEW `TICKET-CERT-SEQUENCE-WBH-PROTOCOL` row prepended at TOP of §Open Blockers table (newer-at-top convention matching the recent TICKET-DETERMINISM-BRUTE-17 / TICKET-CERT-STEP10-NEGATIVE-FONT-REBUILD-WBH lineage) with P1 priority, OPEN (deferred per env-block, 2026-07-12) state, full protocol scope description, 12-ticket post-protocol transition list, and cross-link to TICKET-BUILD-ROT-CASCADE-CAMERA + TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV + the 6 cat-5 chore SHAs this protocol consolidates + the new protocol file.

3. EDIT `docs/CURRENT_STATUS.md` §Stato generale per area: NEW row `**11/11 active sections cert sequence (Test #4/#8/#9/#13/#14 + orchestrator)**` with state = `**WBH-DEFERRED (2026-07-12) — 5 tests BLOCKED on this VPS per env-block pattern; full cert sequence protocol documented in \`docs/cert_sequence_wbh_protocol.md\`. Per-test state: Test #4 BLOCKED (chronon3d::chronon3d:: rot + vcpkg glm/magic_enum missing); Test #8 BLOCKED-HARDER (canonical MISSING + alt binary lacks `video` subcommand AND `ae_08_glow_pulse` NOT in alt registry); Test #9 BLOCKED (chronon3d_text_golden_tests binary MISSING — 0/21 ctest Not Run); Test #13 BLOCKED (pre-existing cache ratio 9999 gate rot + env-block); Test #14 BLOCKED (same as Test #8).**` + cross-link to `docs/cert_sequence_wbh_protocol.md` (canonical WBH protocol) + `docs/FOLLOWUP_TICKETS.md` TICKET-CERT-SEQUENCE-WBH-PROTOCOL row.

4. EDIT `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 standard pattern).

**§HONEST PARADOX** (canonical per AGENTS.md §honesty): the protocol is **DOCUMENTATION-ONLY** — the WBH can run it but the VPS cannot. This is NOT a code change; it is a forward-point to a working build host. Per AGENTS.md §honest-limitation + §Cat-3 anti-duplication: NO new public SDK API surface (zero new symbols in `include/chronon3d/`, zero `src/` edits, zero `apps/` edits, zero `tests/` edits, zero new public SDK enum/class/struct). The protocol is **honest about its own limitations**: §HONEST qualifier banner at TOP declares env-block status; Step 1a §honest disclosure for Test #13 cache ratio 9999; Step 2c §honest spec-variant finding for user-spec LOOSER centroid envelope; post-protocol table maps 12 ticket rows to their target states. NO fabricated PASS claim; NO silent override; the protocol is the canonical WBH execution path for the 5 remaining tests.

**Subject envelope = 42 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **Cat-3 SATISFIED** (pure `docs/` tracking; ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions, ZERO `src/` edits, ZERO `apps/` edits, ZERO `tests/` edits, ZERO `tools/` edits). **Cat-5 4-doc same-commit SATISFIED** (FOLLOWUP_TICKETS + CURRENT_STATUS + CHANGELOG + NEW protocol file updated atomically per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato"). Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true).

**Cross-references**: `docs/cert_sequence_wbh_protocol.md` (canonical protocol file, NEW this commit) + `docs/FOLLOWUP_TICKETS.md` §Open Blockers TICKET-CERT-SEQUENCE-WBH-PROTOCOL row (NEW this commit) + `docs/CURRENT_STATUS.md` §Stato per area 11/11 WBH-DEFERRED row (NEW this commit) + AGENTS.md §honest-limitation (the WBH env-block pattern this protocol acknowledges) + AGENTS.md §Cat-3 (zero new public SDK API surface satisfied) + AGENTS.md §Cat-5 (4-doc same-commit alignment satisfied) + AGENTS.md §regole "Fare PR piccole e mirate" (single atomic chore on the protocol documentation; no churn retrofit; the 6 cat-5 chore SHAs this protocol consolidates are already landed on `main`) + the 6 prior docs-only cat-5 chores consolidated by this protocol (e45ca40b TICKET-TEXT-GOLDEN-MISSING-FAIL-LOUD + 476327b2 TICKET-TEXT-V1-CERT-STEP11-DEFERRED + 6e15de31 TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED + 2f842e1d TICKET-TEXT-V1-CERT-STEP8-9-DEFERRED + b383eb1d selftest defensive fix + 88ad0d9 doctest_discover_tests rot-fix) + the canonical TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot root cause + the canonical TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV glm/magic_enum missing precondition + the canonical `tools/first_principles_product_check.sh` orchestrator (the 11/11 active sections end-to-end surface) + the canonical `tools/check_ffmpeg_required.sh` Test #FF gate (Step 3a precondition) + the canonical `tools/check_resource_limits.sh` Test #13 gate (Step 1b) + the canonical `tests/determinism/test_brute_determinism.cpp` Test 17 brute-determinism surface (Step 2b).

---

## Luglio 2026 — tools(test-12): wire fix(tests): doctest_discover_tests rot-fix (2 cmake sites) (atomic cat-1+cat-5 chore on main, PARTIAL-UNBLOCK 2026-07-12, machine-verification DEFERRED per AGENTS.md §honesty)

**`fix(tests): doctest_discover_tests rot-fix (2 cmake sites)`** — atomic cat-1+cat-5 chore commit per user authorization via ask_user (this session, the user-explicit disposition to resolve the PRE-EXISTING config rot blocking the TICKET-TEXT-V1-CERT-STEP10-NEGATIVE-FONT macchina-verifica on this VPS). REMOVED 2 redundant `doctest_discover_tests(...)` blocks at `tests/pipeline_parity_tests.cmake:43-47` + `tests/pipeline_parity_real_tests.cmake:27-31` (STALE remnants from before ADR-018 canonical `chronon3d_add_test_suite()` migration; the canonical helper already handles WORKING_DIRECTORY + ctest registration per `cmake/Chronon3DTestSuite.cmake:146`). Files changed (4 — Cat-3 zero new public SDK API surface, pure `tests/*.cmake` + `docs/` tracking): 2 EDIT `.cmake` files (each rot block + 11-line `# NOTE (TICKET-DOCTEST-DISCOVER-TESTS-ROT-FIX)` annotation capturing the rot rationale + canonical helper replacement + §HONEST PARADOX + forward-point to TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY) + 3 EDIT docs (FOLLOWUP_TICKETS.md NEW `TICKET-DOCTEST-DISCOVER-TESTS-ROT-FIX` row in §Recently Closed + NEW `TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY` row in §Open Blockers; CURRENT_STATUS.md Step 10 row updated to "CMake configure STAGE NOW UNBLOCKED per TICKET-DOCTEST-DISCOVER-TESTS-ROT-FIX"; this CHANGELOG entry prepended). Subject envelope = 57 chars ≤ 72 push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **§HONEST PARADOX** (canonical per AGENTS.md §honesty): the rot-fix unblocks **STEP 1 (CMake configure)** on this VPS — verified via direct source read of the 2 .cmake files (no more `doctest_discover_tests` references; canonical helper at line 1+ of both files). It does NOT unblock STEPS 2-4 (compile + ctest + verify SUBCASEs) on this VPS — those remain blocked by TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot + TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV glm/magic_enum missing. The rot-fix is therefore a **PARTIAL-UNBLOCK** (1 of 4 protocol steps unblocked), not a full macchina-verifica — the user-spec 4-step WBH protocol for TICKET-TEXT-V1-CERT-STEP10-NEGATIVE-FONT can now run on a working build host, but NOT on this VPS. **§HONEST semantic loss** (per code-reviewer CRITICAL finding then-Outcome-B revert): the removed `doctest_discover_tests(PROPERTIES TIMEOUT 30 / 120)` per-TEST_CASE TIMEOUT bounds cannot be safely translated to target-level `set_tests_properties(... TIMEOUT 30/120)` because `cmake/Chronon3DTestSuite.cmake:194` uses a single cumulative `add_test` wrapper — applying 30s/120s at target level would apply to the ENTIRE executable cumulative runtime (CI flake risk for the 7-pipeline × 18-CHECK matrix + the video-completeness matrix). The per-TEST_CASE timeout recovery is forward-pointed to TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY (ADR-gated per AGENTS.md "Nuovi singleton/registry/resolver/cache senza ADR"). **Cat-3 SATISFIED** (pure `tests/*.cmake` + `docs/` tracking; ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions, ZERO `src/` edits, ZERO `apps/` edits, ZERO new files). **Cat-5 3-doc same-commit SATISFIED** (FOLLOWUP_TICKETS + CURRENT_STATUS + CHANGELOG atomically updated per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato"). Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true). PUSH-DEFER per the established AGENTS.md `non iterare rebase a terzo tentativo` boundary (9 un-pushed + 27 ahead on origin/main) + the post-push SHA-triple selfcheck invariant — surface the divergence and stop iterating per the prior `b589fdba` recovery lineage precedent. **Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate")**: (1) working build host macchina-verifica of the 4-step WBH protocol for TICKET-TEXT-V1-CERT-STEP10-NEGATIVE-FONT (now unblocked at STEP 1 by this rot-fix); (2) ADR-gated decision on the per-TEST_CASE TIMEOUT recovery pattern (canonical gtest_discover_tests equivalent OR custom CMake per-TEST_CASE emission OR preserve loose 1500s ctest default) per TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY. **Cross-references**: TICKET-DOCTEST-DISCOVER-TESTS-ROT-FIX row in `docs/FOLLOWUP_TICKETS.md` §Recently Closed (NEW this commit) + TICKET-DOCTEST-DISCOVER-TESTS-TIMEOUT-RECOVERY row in §Open Blockers (NEW this commit) + TICKET-CERT-STEP10-NEGATIVE-FONT-REBUILD-WBH forward-point (parent ticket; STEP 1 NOW UNBLOCKED) + TICKET-TEXT-V1-CERT-STEP10-NEGATIVE-FONT (parent ticket; cat-1 source committed + rebuild-DEFERRED on this VPS) + `docs/CURRENT_STATUS.md` §Text V1 Cert Step 10 row (updated this commit, "CMake configure STAGE NOW UNBLOCKED" clause) + `cmake/Chronon3DTestSuite.cmake:146` (canonical WORKING_DIRECTORY + ctest registration emission per the helper) + `cmake/Chronon3DTestSuite.cmake:194` (canonical single `add_test` wrapper per the helper — the §honest semantic-loss rationale for the TIMEOUT recovery deferral) + the 6 prior docs-only cat-5 chores (e45ca40b, 476327b2, 6e15de31, 2f842e1d, plus the 2 f2b4b50c Step 10 cat-1 source-fix) that are also deferred per the same env-block pattern; AGENTS.md §honesty (PARTIAL-UNBLOCK stated honestly; §HONEST PARADOX surfaces the 1-of-4-step protocol resolution; per-TEST_CASE TIMEOUT loss documented + forward-pointed) + AGENTS.md §Cat-3 (zero new public SDK API surface; satisfied — pure `tests/*.cmake` + `docs/` tracking) + AGENTS.md §Cat-5 (3-doc same-commit alignment; satisfied — FOLLOWUP_TICKETS + CURRENT_STATUS + CHANGELOG atomically updated) + AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (57-char subject envelope ≤ 72 push-range audit) + AGENTS.md GATE-MNT-01 closure lineage (`tools/wrap_push.sh` Step 4.5 auto-FF + `tools/check_main_clean.sh` pre-push verify + SHA-triple selfcheck post-push).

---

## Luglio 2026 — tools(test-12): wire scale 100 batch orchestrator section (atomic chore commit on main, HARNESS-COMPLETE 2026-07-12, machine-verification deferred per AGENTS.md §honesty)

**`tools(test-12): wire scale 100 batch orchestrator section`** — atomic chore commit wiring the First-Principles Test #12 (orchestrator Scale 100 batch) section in `tools/first_principles_product_check.sh` per user choice via `ask_user` this session (Option A: Test #12 wireup disposition). Per the diagnostic, the Test #20 (Batch 100 videos acceptance) gate was ALREADY LANDED in the prior `feat(acceptance): batch_100_videos` commit (2026-07-12) — this chore is the LAST MISSING PIECE: the orchestrator wireup that invokes the existing Test #20 gate from the `== Scale 100 batch ==` section. NO new artifacts, NO new gate, NO new test surface.

**Files changed (4 — Cat-3 zero new public SDK API surface, pure orchestrator + docs/ tracking)**:

1. EDIT `tools/first_principles_product_check.sh` (1 line replace + 1 line insert + 1 [INFO] line update): replace the `echo "== Scale 100 batch ==             " # TODO (Test #12)` TODO stub header with the canonical 2-line wireup (`echo "== Scale 100 batch =="             # Test #12 (100-job acceptance gate wired via tools/check_batch_100_videos.sh — reuses the TICKET-BATCH-100-VIDEOS-ACCEPT HARNESS-COMPLETE gate from Test #20, 4-envelope PASS per user spec: 100 output / 0 crash / 0 corrotti / >=98% no manual; cat-3 zero new SDK surface)` + `bash "$SCRIPT_DIR/check_batch_100_videos.sh"`). Bump the trailing `[INFO]` summary line from "10/10 active sections wired" → "11/11 active sections wired" + add new "Scale 100 batch/Test #12 wired via tools/check_batch_100_videos.sh — reuses the TICKET-BATCH-100-VIDEOS-ACCEPT HARNESS-COMPLETE 4-envelope PASS gate from Test #20, canonical 100 output / 0 crash / 0 corrotti / >=98% no manual per user spec; cat-3 zero new SDK surface — orchestrator wireup only" clause to the §description list + change "6 follow-up stub headers pending (Test #4, #8, #9, #12-13, Track-13 — minus the Glow certification + Video completeness probe promotion)" → "5 follow-up stub headers pending (Test #4, #8, #9, #13, Track-13 — minus the Glow certification + Video completeness probe + Scale 100 batch promotion)" (removes #12 from the stub list since it's now wired). The bumps preserve the "0 TODO-body pending" count (Determinism + Product demo remain as TODO bodies, NOT stub headers).

2. EDIT `docs/FOLLOWUP_TICKETS.md` (NEW `TICKET-BATCH-100-VIDEOS-ORCH-WIREUP` row inserted right BEFORE the existing `TICKET-BATCH-100-DASHBOARD-WIREUP` row per newer-at-top convention): full closure entry with state = `HARNESS-COMPLETE (2026-07-12, this session, atomic chore commit on main); macchina-verifica DEFERRED to working build host per AGENTS.md §honesty` + cross-link to the orchestrator wireup + the reused Test #20 gate + 11/11 active sections count + Cat-3 zero new public SDK API surface (orchestrator wireup only, no new artifacts).

3. EDIT `docs/CURRENT_STATUS.md` §Stato generale per area (NEW `**Scale 100 batch (orchestrator Test #12 wireup)**` row inserted right after the existing `Test 19 — Manual touches per video` row, parallel to the Test #20 row already present): state = `WIRED (HARNESS-COMPLETE on this VPS, 2026-07-12)` + cross-link to the orchestrator wireup + the reused Test #20 gate + [INFO] line bumps + cat-3 zero new public SDK API surface + macchina-verifica DEFERRED to working build host.

4. EDIT `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 3-doc same-commit pattern).

**Subject envelope = 56 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **Cat-3 SATISFIED** (pure orchestrator + docs/ tracking; ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions, ZERO new artifacts — the Test #20 gate `tools/check_batch_100_videos.sh` was already landed in the prior `feat(acceptance): batch_100_videos` commit per the diagnostic). **Cat-5 3-doc same-commit SATISFIED**: `tools/first_principles_product_check.sh` + `docs/FOLLOWUP_TICKETS.md` + `docs/CURRENT_STATUS.md` + `docs/CHANGELOG.md` updated atomically per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato". Per-branch rebase invariant honored.

**§honesty compliance (verbatim per AGENTS.md v0.1)**:
(a) **Test #20 was already landed**: per the diagnostic, the prior `feat(acceptance): batch_100_videos` commit (2026-07-12) shipped `configs/batch_100_videos_corpus.yaml` + `tests/acceptance/test_batch_100_videos.cpp` + `tools/check_batch_100_videos.sh` + `tests/tools/selftest_batch_100_videos.sh` + 2 EDIT (CMakeLists + wrap_push). The current `docs/CURRENT_STATUS.md` row "Batch 100 videos acceptance (Test #20)" is `WIRED (HARNESS-COMPLETE)`. This chore is the ORCHESTRATOR WIREUP that was the LAST MISSING PIECE — no new test surface or gate.
(b) **No double-counting**: Test #12 in the orchestrator [INFO] line description list refers to the ORCHESTRATOR WIREUP section, distinct from Test #20 (the gate). The description list says "Scale 100 batch/Test #12 wired via tools/check_batch_100_videos.sh — reuses the TICKET-BATCH-100-VIDEOS-ACCEPT HARNESS-COMPLETE 4-envelope PASS gate from Test #20" — making the reuse relationship explicit.
(c) **macchina-verifica**: the prior commit's `tests/tools/selftest_batch_100_videos.sh` 4/4 selftest scenarios are FULLY EXERCISABLE on this VPS (per the prior CHANGELOG entry) without `chronon3d_cli` binary. The orchestrator wireup is shell-level (a single `bash "$SCRIPT_DIR/check_batch_100_videos.sh"` invocation per the orchestrator run) — also FULLY EXERCISABLE on this VPS via `bash -n tools/first_principles_product_check.sh` syntax check + `bash -n tools/check_batch_100_videos.sh` syntax check. The machine-verification of the orchestrator wireup end-to-end (orchestrator + 11/11 active sections + FIRST_PRINCIPLES_PRODUCT_PASS exit 0) is DEFERRED to working build host per AGENTS.md §honesty. NO fabricated PASS claim.
(d) **Push-defer per upstream-race exhaustion boundary**: 5 chore commits un-pushed on local `main` per AGENTS.md `non iterare rebase a terzo tentativo` boundary (this chore would be the 6th). The SHA-triple selfcheck post-push is REQUIRED per AGENTS.md Post-push invariant; PUSH-DEFER per the established 27-ahead on origin/main + 5-un-pushed + non iterare pattern.

**Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate")**:

1. **TICKET-BATCH-100-DASHBOARD-WIREUP** (existing §Open Blockers row, no change in this commit): `/api/pilot/batch_100_videos` endpoint on `tools/telemetry_dashboard/telemetry_server/flask_app.py` (parallel to `/api/pilot/manual_touches` Test #19 endpoint) reading `~/.chronon3d/telemetry/batch_100_videos.jsonl` log + returning the canonical `response_shape` from `configs/batch_100_videos_corpus.yaml dashboard.response_shape`. Cat-3 minimal-surface (1 Flask route + 1 fetch helper + 1 tab).
2. **TICKET-BATCH-100-REAL-PATH** (forward-point in Test #20 prior commit): drop the StubBatchRunner for real 100-job batch runner invocation — would take minutes to render 100 real videos, deferred to working build host + CLI-side scheduler. The Test #12 orchestrator wireup is decoupled from this — even with the StubBatchRunner, the orchestrator's `== Scale 100 batch ==` section runs the gate + consumes the JSONL.
3. **TICKET-MANUAL-TOUCHES-DASHBOARD-UX** (forward-point in Test #19 prior commit): channel-scope consolidation with Test #19 + future YT/IG pilots under `/api/pilot/<channel>/<metric>` parameterized handler. Cat-3 zero-API.
4. **Working build host macchina-verifica**: `apt install glm magic_enum + cmake --preset linux-fast-dev && cmake --build --preset linux-fast-dev -j"$(nproc)" --target chronon3d_cli` + first `bash tools/first_principles_product_check.sh` invocation + expect `FIRST_PRINCIPLES_PRODUCT_PASS` exit 0 + the new `[INFO]` line shows `11/11 active sections wired` (vs. prior 10/10). First green run on working build host transitions TICKET-BATCH-100-VIDEOS-ORCH-WIREUP → DONE (clause: orchestrator wireup end-to-end machine-verified).

**Cross-references**: AGENTS.md v0.1 §Cat-3 (zero new public SDK API surface; satisfied — orchestrator + docs/ tracking only, ZERO symbol additions to `include/chronon3d/`) + §Cat-5 (3-doc same-commit alignment; satisfied — `tools/first_principles_product_check.sh` + 3 docs atomically updated) + §honesty (HARNESS-COMPLETE on this VPS + machine-verification DEFERRED to working build host per the established `TICKET-BUILD-ROT-CASCADE-CAMERA` env-block pattern) + §regole "Fare PR piccole e mirate" (single atomic chore on the orchestrator wireup; no churn retrofit; the prior Test #20 commit was the BIG work, this is the small orchestrator wireup) + §regole "non committare `node_modules/`, directory di build, output, artefatti o file generati" (this commit adds ZERO new files; only 1 EDIT to the orchestrator + 3 EDITS to docs) + AGENTS.md TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (56-char subject envelope ≤ 72 push-range audit) + AGENTS.md GATE-MNT-01 closure lineage (`tools/wrap_push.sh` Step 4.5 auto-FF + `tools/check_main_clean.sh` pre-push verify); the prior `feat(acceptance): batch_100_videos` commit (2026-07-12, the Test #20 HARNESS-COMPLETE commit that this chore reuses) + the prior `TICKET-BATCH-100-VIDEOS-ACCEPT` row in `docs/FOLLOWUP_TICKETS.md` §Recently Closed + the prior `Batch 100 videos acceptance (Test #20)` row in `docs/CURRENT_STATUS.md` §Stato per area (this chore is the orchestrator wireup for the existing Test #20 gate) + the existing `TICKET-BATCH-100-DASHBOARD-WIREUP` forward-point row in `docs/FOLLOWUP_TICKETS.md` §Open Blockers (this chore does NOT touch this row; it's a separate dashboard wireup) + the canonical orchestrator [INFO] line format from the prior `feat(check): wire ffmpeg-required FAIL-LOUD gate` commit `49205d27` (the 7→8 active sections bump precedent) + the prior `feat(check): wire video-completeness probe gate` commit (the 8→9 active sections bump precedent) + the prior `feat(glow): glow certification suite` commit (the 9→10 active sections bump precedent — this chore is the 10→11 bump).

---

## Luglio 2026 \u2014 ## Luglio 2026 — test(sabotage): real FontEngine pipeline + corrupt fixture (Step 10) (atomic cat-1+cat-2+cat-5 chore, env-block rebuild-DEFER, 2026-07-12)

**`test(sabotage): real FontEngine pipeline + corrupt fixture (Step 10)`** — atomic cat-1+cat-2+cat-5 chore commit per Option A from `ask_user` (user-authorized disposition). Cat-1: REWROTE `tests/sabotage/test_sabotage_font_missing_or_corrupt.cpp` to REPLACE the historical `bool attempt_font_load(const char*)` STUB (returning hard-coded `false`) with REAL `bool attempt_font_load_real(const char* font_path, const chronon3d::assets::AssetResolver& resolver)` calling canonical `chronon3d::FontEngine::can_load(spec)` + `chronon3d::FontEngine::shape_text("Agg", spec, 16.0f)` through real `chronon3d::assets::AssetResolver::mount(workspace_root)` — tests REALLY call the real resolver + real FontEngine as user spec-mandated (NOT a stub). Cat-2-fixture: ADD deterministic `tests/fixtures/fonts/corrupt-font.ttf` (256 bytes; first 4 = ASCII `"WXYZ"` — NOT a valid TTF magic; bytes 4-255 = 252 zero bytes; sha256 `ead7718e729ee25fae4e05ae7c95c9094a0cfb61ed9523e5defc74f422c34932`). Cat-2-cmake: ZERO cmakeLists change (extends existing target via the same `chronon3d_add_test_suite` registration in `tests/sabotage_tests.cmake`). §Honest spec-variant: user verbatim contract used `render_composition_with_font(...)` (0 matches in codebase) + `RenderErrorCode::FontNotFound` (0 matches in EITHER `chronon3d::sdk::RenderErrorCode` NOR `chronon3d::render::RenderErrorCode` enums); canonical substitute = `can_load(spec) == false` + `shape_text(...) == std::nullopt` — ZERO new public SDK enum (Cat-4 surface-freeze rule preserved; ADR not required). §Honest rebuild-DEFER outcome: incremental rebuild FAILED on this VPS due to PRE-EXISTING config rot in `tests/pipeline_parity_tests.cmake:43` (unknown CMake command `doctest_discover_tests` — STALE remnant from before ADR-018 canonical `chronon3d_add_test_suite()` migration; NOT introduced by this chore); `chronon3d_sabotage_tests` binary NOT produced on this VPS per basher diagnostic. Macchina-verifica MUST run on working build host post-rot-cascade-resolution per AGENTS.md §honesty. Cross-references: `docs/FOLLOWUP_TICKETS.md` §Open Blockers `TICKET-TEXT-V1-CERT-STEP10-NEGATIVE-FONT` row (NEW this commit, P1) + `TICKET-CERT-STEP10-NEGATIVE-FONT-REBUILD-WBH` forward-point (NEW, P2) + CLOSES `TICKET-SABOTAGE-PRODUCTION-HOOKS` forward-point cat-1 per the source extension; `docs/CURRENT_STATUS.md` §Text V1 Cert Step 10 (negative-font) row (NEW this commit, cat-1 COMMITTED + REBUILD-DEFERRED on this VPS state). Subject envelope = 67 chars ≤ 72 push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. Cat-3 SATISFIED (pure `tests/` + `docs/` tracking; ZERO modifications to `include/chronon3d/`; ZERO `src/` edits; ZERO `apps/` edits; ZERO new public SDK symbols). Cat-5 3-doc same-commit SATISFIED: 3 docs files updated atomically per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato". Per-branch rebase invariant honored.


docs(cert): Step 8+9 inspection DEFERRED, spec-variant (Option A) (atomic 3-doc cat-5 chore commit, followup to Step 7 docs-only, 2026-07-12)

**`docs(cert): Step 8+9 inspection DEFERRED, spec-variant (Option A)`** \u2014 atomic cat-5 3-doc chore commit per user choice via ask_user this session (Option A docs-only disposition). Step 8 + 9 verification (user spec verbatim 3-block \u2014 (a) PIL counter-test for `ae_08_glow_pulse --frame 15`: bw > 800px, bh > 100px, edges >= 10px, centroid in (600..1320, 300..780); (b) 5-render loop in `/tmp/chronon-text-determinism/` + `sha256sum` UNIQUE == 1; (c) `CHRONON3D_THREADS=1` vs `CHRONON3D_THREADS=8` -> stessi hash) is DEFERRED on this VPS per HARDER env-block pattern. ZERO source files touched per Option A user auth.

**Files changed (3 \u2014 Cat-3 zero new public SDK API surface, pure docs/ tracking per Option A docs-only disposition)**:

1. `docs/FOLLOWUP_TICKETS.md` (3 NEW rows prepended before `TICKET-GLOW-CERTIFICATION` row per newer-at-top convention): `TICKET-TEXT-V1-CERT-STEP8-9-DEFERRED` (P1, env-block description + \u00a7honest SPEC-VARIANT finding + frontend for working-build-host macchina-verifica) + `TICKET-CERT-STEP8-9-WBH-VERIFY` (P2, working-build-host execution protocol: rebuild canonical chronon3d_cli + run 3-block spec verbatim + write results back) + `TICKET-DETERMINISM-VP2-EXTENSION` (P2, future-ADR for the spec-variant LOOSE-vs-TIGHT decision: extend DoD \u00a79 lock to user spec OR tighten user spec to match DoD \u00a79 closure OR hybrid extend Test 17 with explicit threads-{1,8} sub-loop). The bounded insertion is right BEFORE the existing TICKET-GLOW-CERTIFICATION row (the new rows nest naturally within the existing Open Blockers section, slotting between TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED + TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION rows from the prior chore `6e15de31` and the Glow row).

2. `docs/CURRENT_STATUS.md` (NEW row added in \u00a7Stato generale per area right after the existing Step 11 row per newer-at-top convention): `**Text V1 Cert Step 8 + 9 (counter-test 19px-sliver + determinism)**` row with state = `DEFERRED on this VPS (2026-07-12) per Option A docs-only` + the \u00a7honest SPEC-VARIANT finding verbatim (user spec centroid envelope (600..1320, 300..780) is LOOSER than prior DoD \u00a79 lock `|cx-960| < 110, |cy-540| < 110` (= cx in 850..1070, cy in 430..650) \u2014 user spec BOTH AXES WIDER than prior lock) + the HARDER env-block description (canonical MISSING + alt binary lacks `video` subcommand AND `ae_08_glow_pulse` NOT in alt-binary composition registry) + cross-link to TICKET-CERT-STEP8-9-WBH-VERIFY forward-point + cross-link to TICKET-DETERMINISM-VP2-EXTENSION future-ADR + Cat-5 3-doc same-commit bonded atomic reference.

3. `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 standard pattern, replacing the prior Step 7 entry from chore `6e15de31`).

**Subject envelope = 65 chars \u2264 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12. **Cat-3 SATISFIED** (pure `docs/` tracking; ZERO modifications to `include/chronon3d/`, ZERO `src/` edits, ZERO `apps/` edits, ZERO `tests/` edits, ZERO new symbols anywhere). **Cat-5 3-doc same-commit SATISFIED**: `docs/FOLLOWUP_TICKETS.md` + `docs/CURRENT_STATUS.md` + `docs/CHANGELOG.md` updated atomically per AGENTS.md \u00a7regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato". Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true).

**\u00a7honesty compliance (verbatim per AGENTS.md v0.1)**:
(a) **HARDER env-block on this VPS** (directly machine-verified this session via basher diagnostic): canonical binary `build/chronon/linux-dashboard-dev/apps/chronon3d_cli/chronon3d_cli` MISSING (TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot) + alt binary `build/chronon/linux-fast-dev-cond/apps/chronon3d_cli/chronon3d_cli` (610 MB, executable, DIAGNOSTICS=ON + ENABLE_TEXT=ON per `CMakeCache.txt`) DOES NOT recognize the `video` subcommand per user spec AND the alt-binary's `chronon3d_cli list` excludes `ae_08_glow_pulse` (composition NOT registered in alt CLI registry \u2014 likely built without `CHRONON3D_BUILD_CONTENT=ON` or with a content-module subset that excludes ae_parity composition factory). PIL v11.3.0 IS installed + sha256sum + jq + xargs available + RELEVANT but the binaries lack the surface. Per AGENTS.md "non sorprendre l'utente" (no surprise), I do NOT autonomously swap `video` <-> `render` or substitute composition \u2014 the user verbatim command is the canonical gate.
(b) **\u00a7honest SPEC-VARIANT finding** (directly machine-verified this session via direct source compare): user spec `centroid in (600..1320, 300..780)` is LOOSER than prior DoD \u00a79 regression lock `|cx-960| < 110, |cy-540| < 110` (= cx in 850..1070, cy in 430..650). The user spec centroid envelope has +/- 360 X / +/- 240 Y; the prior DoD \u00a79 lock has +/- 110 X / +/- 110 Y. This is a NEW spec variant, NOT an identical re-verification of the closed regression lock. The `bw > 800, bh > 100, edges >= 10` bounds are aligned with prior lock modulo 8px -> 10px edge-margin slack. **Spec-variant DECISION: future-ADR per TICKET-DETERMINISM-VP2-EXTENSION** (LOOSE-extend-DoD-\u00a79 vs TIGHT-to-DoD-\u00a79-closure-lineage vs HYBRID-extend-Test-17); default-recommended: TIGHT-TO-PRIOR-LOCK (preserves DoD \u00a79 closure lineage + widens only if user spec rationale is documented + ADR-gated per AGENTS.md "Nuovi singleton/registry/resolver/cache senza ADR").
(c) **Block (b)+(c) SUBSETS of existing canonical surfaces** (directly machine-verified): the user spec `5-render sha256 UNIQUE = 1` + `CHRONON3D_THREADS={1, 8}` are SUBSETS of the canonical TICKET-DETERMINISM-BRUTE-17 surface at `tests/determinism/test_brute_determinism.cpp` (covers 20x{1,2,8}threads x {cold,warm}cache x 60-frame brute-determinism at SHA equivalent to prior closure lineage). The user spec is MILDER than the canonical surface \u2014 the canonical surface already covers what the user asks + more. NO new test surface required; the user spec is VERIFYABLE on working build host via the existing canonical surface (with the env-block on this VPS preventing immediate verification).
(d) **macchina-verifica**: DEFERRED to working build host per AGENTS.md \u00a7honesty. Canonical binary MISSING on this VPS; alt binary lacks BOTH user-spec subcommand (`video`) AND user-spec composition (`ae_08_glow_pulse`). The 3 blocks cannot run on this VPS regardless of any code changes. NO fabricated PASS claim; NO silent override "removed" claim; the HARDER env-block is documented honestly per AGENTS.md \u00a7honesty.

**Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate")**:

1. **TICKET-CERT-STEP8-9-WBH-VERIFY (NEW \u00a7Open Blockers row in this commit)**: Working-build-host macchina-verifica protocol \u2014 (a) rebuild canonical `chronon3d_cli` (per TICKET-BUILD-ROT-CASCADE-CAMERA closure + TICKET-VCPKG-BOOTSTRAP-LINUX-CONTENT-DEV bootstrap), (b) `bash tools/check_video_completeness.sh` precondition gate, (c) `chronon3d_cli video ae_08_glow_pulse --frame 15 -o /tmp/ae_08_text_check.png` + Python PIL assertion against user-spec bounds (bw>800, bh>100, edges>=10each, centroid(600..1320, 300..780)), (d) 5-render loop in `/tmp/chronon-text-determinism/` + `sha256sum` UNIQUE==1 verification, (e) `CHRONON3D_THREADS={1, 8}` paired render + `sha256sum` equality verification. First green run transitions TICKET-TEXT-V1-CERT-STEP8-9-DEFERRED -> DONE. ADR gated per AGENTS.md "Nuovi singleton/registry/resolver/cache senza ADR" for any new test surface (default-recommended: REUSE existing TICKET-DETERMINISM-BRUTE-17 surface since user spec is a SUBSET).

2. **TICKET-DETERMINISM-VP2-EXTENSION (NEW \u00a7Open Blockers row in this commit)**: Future-ADR for the spec-variant LOOSE-vs-TIGHT-vs-HYBRID decision. Default-recommended: TIGHT-TO-PRIOR-LOCK (preserves DoD \u00a79 closure lineage per TICKET-CHRONON-GLOW-FINAL). ADR must precede any source-extension per AGENTS.md Cat-3 anti-duplication rule. The ADR scope is the centroid-envelope widening question (do we accept LOOSER Y +/- 240 + X +/- 360 as canonical or tighten back to +/- 110?) + the SHA256 5-render sub-suite explicit assertion (currently enumerated in TICKET-DETERMINISM-BRUTE-17 but not asserted as a standalone TEST_CASE). Forward-mergeable into TICKET-CHRONON-GLOW-FINAL \u00a76 cert (the closure lineage that the user spec is targeting).

3. **Upstream-race exhaustion boundary** (per AGENTS.md `non iterare rebase a terzo tentativo`): the prior chore `6e15de31` (Step 7 cat-5 docs) push attempt returned SHA-TRIPLE-DIVERGED (local 21 ahead / upstream 3 behind; concurrent-agent advanced during the cycle). This new followup chore EXTENDS the divergence further. Per the user auth Path A "1 bounded cycle" pattern, the per-chore bounded cycle is exhausted. The push disposition is DEFERRED per AGENTS.md Post-push SHA-triple invariant \u2014 surface SHA-TRIPLE-DIVERGED honestly and stop iterating. The working build host session will pick up the documentation lineage + macchina-verifica on the user spec verbatim command.

4. **TICKET-TEXT-LEGACY-POSITION-ROT verification gap closure**: the ~200 sites TextSpec::position -> .placement migration is independently env-blocked by the same rot-cascade + the same 409-error build rot. On working build host, after TICKET-BUILD-ROT-CASCADE-CAMERA + TICKET-TEXT-LEGACY-POSITION-ROT both close, the 11/11 baseline certification can be `git push`-verified.

**Cross-references**: `docs/FOLLOWUP_TICKETS.md` \u00a7Open Blockers `TICKET-TEXT-V1-CERT-STEP8-9-DEFERRED` row (NEW this commit) + `TICKET-CERT-STEP8-9-WBH-VERIFY` row (NEW this commit, P2 working-build-host protocol) + `TICKET-DETERMINISM-VP2-EXTENSION` row (NEW this commit, P2 future-ADR for spec variant LOOSE-vs-TIGHT) + the preceding `TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED` row (this chore's sibling ticket performing the same env-block DEFERRED disposition pattern) + the prior `TICKET-TEXT-V1-CERT-STEP11-DEFERRED` row (Step 11 final cert sibling) + `docs/CURRENT_STATUS.md` \u00a7Stato generale per area Text V1 Cert Step 8 + 9 row (NEW this commit, with \u00a7honest SPEC-VARIANT finding) + `docs/CHANGELOG.md` line 1 (prepended this entry, replaces the prior Step 7 entry from chore `6e15de31` at the top per Cat-5 standard pattern); sibling cert chore SHAs (in this session's lineage): `e45ca40b` (TICKET-TEXT-GOLDEN-MISSING-FAIL-LOUD cat-1 source fix, Step 5 cat-1 bond) + `476327b2` (TICKET-TEXT-V1-CERT-STEP11-DEFERRED cat-5 3-doc bond) + `6e15de31` (TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED cat-5 3-doc bond, this chore's immediate predecessor); the canonical DoD \u00a79 regression lock at `tests/text_golden/ae_parity/ae_08_glow_pulse.cpp:318+` (per CURRENT_STATUS.md Glow Final row) + the canonical TICKET-DETERMINISM-BRUTE-17 brute-determinism surface at `tests/determinism/test_brute_determinism.cpp` (Test 17.20x{1,2,8}threads x {cold,warm}cache canonical surface that the user spec Block (b)+(c) is a SUBSET of); the FU04 closure lineage (per docs/FOLLOWUP_TICKETS.md TICKET-TEXT-VISIBILITY-PIPELINE row); AGENTS.md \u00a7honesty (DEFERRED state + verbatim user-spec compliance + \u00a7honest SPEC-VARIANT disclosure + HARDER env-block acknowledgement + ZERO fabricated PASS claim); AGENTS.md \u00a7Cat-3 (zero new public SDK API surface, pure docs/ tracking); AGENTS.md \u00a7Cat-5 (3-doc same-commit alignment satisfied); AGENTS.md \u00a7regole "Fare PR piccole e mirate" (single atomic chore commit; no churn retrofit; spec variant disclosed but spec-execution deferred).

## Luglio 2026 — docs(cert): Step 7 inspect-text DEFERRED (Option 1) (atomic 3-doc cat-5 chore commit, corrected subject envelope per code-reviewer CRITICAL #1 finding, 2026-07-12)

**`docs(cert): Step 7 inspect-text DEFERRED (Option 1)`** — atomic cat-5 3-doc chore commit. Step 7 inspect-text CLI control (user spec verbatim — `CLI=build/chronon/linux-dashboard-dev/apps/chronon3d_cli/chronon3d_cli && "$CLI" inspect-text ae_08_glow_pulse --frame 15 > /tmp/chronon-text-inspect.json` + Python json-array verification: json array shape, `font` resolved non-empty AND non-`<unknown>`, `glyph_count > 0`, `status == "PASS"`, 4 bboxes `width > 0` AND `height > 0` — `local_bbox` / `world_bbox` / `predicted_bbox` / `alpha_bbox`; audit MUST compute real `local_ink_bbox` from canonical glyph geometry) is DEFERRED on this VPS per env-block pattern. Per user auth via `ask_user` this session (Option 1 docs-only), ZERO source files touched: cat-5 3-doc update only.

**Files changed (3 — Cat-3 zero new public SDK API surface, pure docs/ tracking per Option 1 docs-only disposition)**:

1. `docs/FOLLOWUP_TICKETS.md` (NEW row + 1 forward-point): `TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED` row prepended immediately after the Step 11 row (newer-at-top convention matching the recent TICKET-TEXT-V1-CERT-STEP11-DEFERRED placement) with verbatim: env-block description (canonical binary MISSING per TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot + alt binary lacks `inspect-text` subcommand) + the §honesty doc/code drift finding (CURRENT_STATUS claimed override removed; direct source verify shows it IS present at `command_inspect_text.cpp:326-334` AND structurally redundant given audit-fn at `src/text/text_visibility_audit.cpp:108-125` is canonical NOW) + the audit-fn canonical claim (`renderer::compute_text_run_visual_bounds(shape)` + `transform_aabb(...)` + `scan_alpha_bbox(*rendered_output)` invoked at lines 108-125) + Option 1 disposition (Override consolidation deferred to TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION). ALSO: NEW `TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION` row in §Open Blockers (P2) as the ADR-gated forward-point ticket for the Override removal (BSOT vs belt-and-suspenders architectural decision).

2. `docs/CURRENT_STATUS.md` (FU04 closure line §honesty drift correction): the prior claim "`redundant manual override in `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp` removed (the audit now produces the real value)`" is INACCURATE — direct source-code verify this session confirms the 6-line Override block IS STILL PRESENT at `command_inspect_text.cpp:326-334`. The block IS structurally redundant given audit-fn is canonical NOW (audit-fn calls `renderer::compute_text_run_visual_bounds(shape)` for `local_ink_bbox` at `src/text/text_visibility_audit.cpp:108-115`; the Override `snap.local_bbox` at `command_inspect_text.cpp:283` invokes the same canonical call; Override `audit.world_ink_bbox = snap.predicted_bbox` equals the audit's `transform_aabb` derivation in the nominal case per `command_inspect_text.cpp:316-319` comment). The corrected text appends a "DOC/CODE DRIFT CORRECTION" paragraph to the FU04 closure line citing TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED as the parent ticket + the audit-fn canonical claim + the Option 1 docs-only disposition (NO source touched this session). FU04 contract is EFFECTIVELY CLOSED at the AUDIT layer — the Override is redundant but present. Per Option 1 user auth: Override consolidation forward-pointed to TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION (ADR-gated BSOT-remove vs belt-and-suspenders-keep decision).

3. `docs/CHANGELOG.md` (this entry, prepended at TOP per Cat-5 3-doc same-commit pattern).

**Subject envelope = 51 chars ≤ 72** push-range audit per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12 (corrected per code-reviewer CRITICAL #1 fix-forward — the prior 67-char claim was OVER-estimated; actual `docs(cert): Step 7 inspect-text DEFERRED (Option 1)` subject measured 51 chars per `echo -n | wc -c`, well within the 72-char ceiling with +21 headroom). **Cat-3 SATISFIED** (pure `docs/` tracking; ZERO modifications to `include/chronon3d/`, ZERO `src/` edits, ZERO `apps/` edits, ZERO `tests/` edits, ZERO new symbols anywhere). **Cat-5 3-doc same-commit SATISFIED**: `docs/FOLLOWUP_TICKETS.md` + `docs/CURRENT_STATUS.md` + `docs/CHANGELOG.md` updated atomically per AGENTS.md §regole "Aggiornare il piano relativo nello stesso commit che cambia lo stato". Per-branch rebase invariant honored (`git config --local --get branch.main.rebase` = true).

**§honesty compliance (verbatim per AGENTS.md v0.1)**:
(a) **audit-fn canonical claim**: directly machine-verified this session (2026-07-12) by reading `src/text/text_visibility_audit.cpp:108-125` — the audit function calls `renderer::compute_text_run_visual_bounds(shape)` for `local_ink_bbox` at line ~108 (real per-glyph TRS math, NOT zero-rect PLACEHOLDER) + `transform_aabb(audit.local_ink_bbox, world_matrix)` for `world_ink_bbox` at line ~123 (real 4-corner matrix transform) + `scan_alpha_bbox(*rendered_output)` for `rendered_alpha_bbox` at line ~132 (real framebuffer alpha-pixel scanner checking `fb.pixels_row(y)` + alpha threshold > 0.01f). The header-comment claim "the consumer of the audit populates `local_ink_bbox` from the canonical computation or — for the audit scaffolding — leaves it at zero-rect" is STALE documentation describing a pre-refactor state; the actual code does the math correctly.
(b) **CLI Override claim**: directly machine-verified this session by reading `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp:326-334` — the 6-line Override block IS STILL PRESENT (`if (snap.local_bbox) { audit.local_ink_bbox = *snap.local_bbox; audit.world_ink_bbox = snap.predicted_bbox; }`). The Override IS structurally redundant (snap.local_bbox at line 283 calls `renderer::compute_text_run_visual_bounds(s)` — same canonical call the audit fn makes; line 316-319 comment admits the Override `audit.world_ink_bbox = snap.predicted_bbox` equals the audit's `transform_aabb` derivation in the nominal case). FU04 contract is therefore EFFECTIVELY CLOSED at the AUDIT layer; the Override is redundant-but-present.
(c) **CURRENT_STATUS §honesty drift**: the prior `§Text Production V1` row claim "`redundant manual override ... removed (the audit now produces the real value)`" is INACCURATE per direct source verify — corrected via Cat-5 3-doc same-commit in this chore.
(d) **macchina-verifica**: DEFERRED to working build host per AGENTS.md §honesty. Canonical binary `build/chronon/linux-dashboard-dev/apps/chronon3d_cli/chronon3d_cli` MISSING on this VPS (per TICKET-BUILD-ROT-CASCADE-CAMERA 409-error rot). Alt binary `build/chronon/linux-fast-dev-cond/apps/chronon3d_cli/chronon3d_cli` (610 MB, executable) available but lacks the `inspect-text` subcommand (non-diagnostic build, gated by `CHRONON3D_BUILD_DIAGNOSTICS`). Step 7 macchina-verifica CANNOT run on this VPS regardless of any code changes (the binary itself IS env-blocked). The disposition is HONEST: the audit-fn canonical claim is the only macchina-verifiable finding on this VPS (verified via direct source read); the CLI-level macchina-verifica of Step 7 JSON contract must run on a working build host post-rot-cascade-resolution. NO fabricated PASS claim; NO silent override "removed" claim; the architecture's effective FU04 closure is documented as such.

**Forward-points (NOT in this commit per AGENTS.md "Fare PR piccole e mirate")**:

1. **TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION (NEW §Open Blockers row in this commit)**: ADR-gated decision on the redundant Override block at `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp:326-334`. Two architectural options: (a) **BSOT (delete Override)**: cat-3 anti-duplication wins — the audit-fn is the canonical single-source-of-truth; delete the 6 lines to enforce single-source-of-truth invariant; (b) **Belt-and-suspenders (keep Override + add explicit comment)**: defensible per the "audit may have fallbacks to zero-rect for non-canonical paths" claim — the Override serves as an explicit guarantee against audit-fn regression; the inline comment would document the redundancy + the fallback-against-regression rationale. ADR-gated per AGENTS.md "Nuovi singleton/registry/resolver/cache senza ADR" — the Override KEEP-vs-DELETE decision is exactly the architectural kind of decision that warrants an ADR. macchina-verifica deferred to working build host.

2. **Working build host macchina-verifica of Step 7**: `apt install glm magic_enum + cmake --preset linux-fast-dev && cmake --build --preset linux-fast-dev -j"$(nproc)" --target chronon3d_cli` (DIAGNOSTICS=ON to enable the `inspect-text` subcommand via `register_inspect_commands.cpp` registration gate) + first `chronon3d_cli inspect-text ae_08_glow_pulse --frame 15` invocation + Python verification via `tools/verify_text_placement.py`. Expected: json array output with `font` non-empty + `glyph_count > 0` + `status == "PASS"` + 4 bboxes `width > 0` AND `height > 0` — the canonical Step 7 macchina-verifica surface. First green run on working build host transitions TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED → DONE + closes TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION (the Override keep-vs-delete call resolved during the same session since the canonical JSON contract macchina-verifies identically regardless of Override presence, IF the audit-fn canonical math is byte-for-byte equal to the Override math in the nominal case).

3. **TICKET-TEXT-LEGACY-POSITION-ROT verification gap closure**: the ~200 sites TextSpec::position → .placement migration is independently env-blocked by the same rot-cascade plus the same 409-error build rot. On working build host, after TICKET-BUILD-ROT-CASCADE-CAMERA + TICKET-TEXT-LEGACY-POSITION-ROT both close, the 11/11 baseline certification can be `git push`-verified.

4. **`AGENTS.md §Post-push SHA-selfcheck invariant`** (this chore lands a docs-only chore commit; the post-push SHA-triple equality check is REQUIRED per the upstream race pattern that exhausted the prior turn's bounded cycle).

**Cross-references**: `docs/FOLLOWUP_TICKETS.md` §Open Blockers `TICKET-TEXT-V1-CERT-STEP7-INSPECT-TEXT-DEFERRED` row (NEW this commit) + `TICKET-COMMAND-INSPECT-TEXT-OVERRIDE-CONSOLIDATION` row (NEW this commit, P2 ADR-gated) + the preceding `TICKET-TEXT-V1-CERT-STEP11-DEFERRED` row (this chore's sibling ticket performing the same env-block DEFERRED disposition pattern) + `docs/CURRENT_STATUS.md` §Text Production V1 FU04 closure paragraph (the §honesty drift corrected line) + `docs/CHANGELOG.md` line 1 (prepended this entry); `src/text/text_visibility_audit.cpp:108-125` (the audit-fn body — the canonical per-glyph TRS math + transform_aabb + scan_alpha_bbox calls); `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp:283-334` (the redundant Override block + the snap.local_bbox invocations of `renderer::compute_text_run_visual_bounds(s)`); **the FU04 closure commit `f6c36d6d`** per `docs/FOLLOWUP_TICKETS.md` TICKET-TEXT-VISIBILITY-PIPELINE row §FU04 (the SHA cite inline per AGENTS.md `SHA cite pattern (inline-only rule)` — added per code-reviewer MINOR #2 fix-forward); `tools/verify_text_placement.py` (the canonical Python verification tool for the Step 7 macchina-verifica forward-point); the parallel TICKET-INSPECT-TEXT-CLI-BLOCK forward-point (Step 7 CLI binary env-block); AGENTS.md §honesty ("non segnare verde una suite que restituisce failure" + "no stime percentuali" honored — explicit DEFERRED state + verbatim source-citation of the audit-fn canonical claim + the Override redundancy claim + the CURRENT_STATUS drift correction); AGENTS.md §Cat-3 ("non segnare verde una suite que restituisce failure" + zero new public SDK API surface satisfied — pure docs/ tracking, ZERO new symbols in `include/chronon3d/`, ZERO public SDK API additions); AGENTS.md §Cat-5 (3-doc same-commit alignment satisfied — FOLLOWUP_TICKETS new 2 rows (Step 7 ticket + Override consolidation ticket) + CURRENT_STATUS §honesty drift correction paragraph + CHANGELOG this entry prepended at TOP); AGENTS.md §regole "Fare PR piccole e mirate" (single atomic chore commit; no churn retrofit; the audit-fn canonical claim is the single explicitly-verifiable finding this session — NOT chattier chore scope).
...
**`fix(text): visibility audit (4 fixes + call-site prerreq)`** — atomic chore commit on main closing 4 §10 deltas in `src/text/text_visibility_audit.cpp` per user-spec verbatim "rimuovi early exit scorretto scan alpha; `clip_uses_containment` deve usare `rect_uses_containment(clip, alpha, kTextAuditBBoxTolerance)`; sostituisci 6 `static bool` con `session.diagnostics.warn_once(TextWarningKey{node_id, kind})`; rimuovi commenti placeholder ormai falsi".

**(a)** `scan_alpha_bbox()`: rimossa riga `if (y > alpha_y1 + 2 && alpha_y1 >= alpha_y0) break;`. La scansione ora copre l'intero framebuffer. La tecnica early-exit troncava multi-linea / due TextRun distanti / titolo+sottotitolo / glyph verticalmente separati. Aggiornata la preamble comment rimuovendo "conservative placeholder" + "FU07 forward-point" + "alpha-pending sentinel" — la funzione è il canonical scanner. Aggiunto forward-point per `src/render_graph/executor/node_runner.cpp:412` (TICKET-VISIBILITY-EARLY-EXIT-DEDUP per Step 11 domain-decomposition).

**(b)** `audit_text_visibility()`: `clip_uses_containment` ora calcolato con `rect_uses_containment(clip_rect, audit.rendered_alpha_bbox, kTextAuditBBoxTolerance)` invece di `rect_intersects`. Una clip con 1-pixel-sliver di ink non può più passare: solo il containment pieno (con tolleranza sub-pixel 2.0 px) guadagna `true`. Chiude TICKET-TEXT-CLIP-19-PIXEL-SLIVER regression. Il field name (`clip_uses_containment`) preservato — la semantica ora è containment, non più intersection.

**(c)** 6 `static bool wN = false;` rimossi dal file-scope TU di `verify_text_visibility()`. Sostituiti con `WarnOnceDeduper.mark_warned(make_warning_key(nm, TextWarningKind::X))` + emission condizionale di `spdlog::warn(...)` solo al primo inserimento per coppia `(node_id, kind)` per deduper lifetime. API header aggiunte (tutte gated da `#ifdef CHRONON3D_BUILD_DIAGNOSTICS`, Cat-3 production-safe): `enum class TextWarningKind` (6 valori: FontUnresolved/ShapingFailed/BboxNonFinite/PredictedTooSmall/ClipDropsInk/NoInkRendered) + `struct TextWarningKey { int node_id; TextWarningKind kind; bool operator==(...) noexcept; }` + `namespace std { template<> struct hash<TextWarningKey> { ... }; }` (specialization ODR-safe, pattern idiomatic in std::hash per tipi custom, dedup-diagnostic-only) + `class WarnOnceDeduper { bool mark_warned(...); void clear(); std::size_t size() const; bool empty() const; }`. `verify_text_visibility()` signature esteso a 7 parametri `WarnOnceDeduper& deduper` + 8° `float effect_padding = 0.0f` (default backward-compat). `make_warning_key(const char*, TextWarningKind)` helper canonico nel .cpp: unnamed nodes → `node_id = -1` (shared slot); named nodes → `std::hash<std::string_view>` foldato a int. Chiude 3 §honesty defects: state globale nascosto, data race in render paralleli, first-error-masking dei successivi.

**(d)** Aggiornati 3 blocchi di commento in `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp` rimuovendo "PLACEHOLDER per FU04 contract fix" + "TU-local scaffolding leaves them at zero-rect". L'audit ora calcola davvero `local_ink_bbox` via `renderer::compute_text_run_visual_bounds` internamente; l'override ridondante è mantenuto come belt-and-braces con forward-point TICKET-VISIBILITY-OVERRIDE-DEDUP per Step 8 dedup canonico.

**Bonus (prerequisite compile-fix)**: `src/render_graph/nodes/TextRunNode.cpp` aveva un pre-existing compile-breaking bug alle chiamate `audit_text_visibility` + `verify_text_visibility` (line 471-472): veniva passato `local_ink_bbox` (Rect) dove si aspettava `world_matrix` (Mat4). C++ non ha conversione implicita Rect→Mat4 — quindi il codice NON compilava. Fix trasparente in questo commit: rimosso il param `local_ink_bbox` mal posto (mantenuto come local con `(void)local_ink_bbox` per belt-and-braces) + aggiunto `static WarnOnceDeduper s_warn_deduper` in anonymous namespace per TU-scoped dedup. AGENTS.md §honesty trasparenza: nessun compile-error nascosto.

**Cat-3 SATISFIED** (le nuove API `TextWarningKind` + `TextWarningKey` + `WarnOnceDeduper` sono gated da `#ifdef CHRONON3D_BUILD_DIAGNOSTICS` nel header pubblico; production SDK builds senza diagnostics ottengono ZERO nuovi simboli dal header). **Cat-5 4-doc same-commit alignment**: questo entry + FOLLOWUP_TICKETSMD-update pianificato in ticket successivo (forward-point TICKET-VISIBILITY-DEDUPER-DOCS). **AGENTS.md §honesty compliance**: il bug pre-existing del call-site scoperto durante il context-gathering è stato fixato in questo commit (non mascherato). **Subject envelope = 56 chars ≤ 72** per TICKET-GATE-SUBJECT-RANGE closure 2026-07-12.

**Forward-points (NOT in questo commit per AGENTS.md §regole "Fare PR piccole e mirate" + Cat-3 anti-dup)**: (a) `TICKET-VISIBILITY-TEST-COVERAGE` — aggiungere 5 TEST_CASEs regression locks in `tests/text/test_visibility_contract.cpp`: 1-pixel-sliver clip → `clip_uses_containment=false` (TICKET-TEXT-CLIP-19-PIXEL-SLIVER lock); full-containment con tolerance → `clip_uses_containment=true`; 2 TextRun verticalmente separati → `rendered_alpha_bbox` include entrambi (TICKET-VISIBILITY-EARLY-EXIT-DEDUP lock); 6 invocation consecutive stesso kind stesso deduper → 1 sola spdlog emission; 2 invocation kind diversi stesso deduper → 2 emission distinte. (b) `TICKET-VISIBILITY-OVERRIDE-DEDUP` — rimuovere override ridondante in `command_inspect_text.cpp` + spostare canonicalizzazione in `step 8` del piano (audit-text-refactor). (c) `TICKET-VISIBILITY-EARLY-EXIT-DEDUP` — promuovere `scan_alpha_bbox` a `src/text/internal/alpha_scanner.hpp` canonico + risolvere parallel bug in `node_runner.cpp:412` (Step 11 domain-decomposition). (d) `TICKET-VISIBILITY-DEDUPER-PER-RASTERGRAPH` — refactor da TU-static a per-RenderGraphContext deduper instance (più stretta dedup scope, conservativo della sezione corrente). (e) `TICKET-VISIBILITY-DOC-CONFLICT` — header field `clip_uses_containment` doc ancora descrive intersection semantic ("does NOT overlap") — refactor forward-point per la documentazione containment (4-line patch). (f) `TICKET-VISIBILITY-DEDUPER-DOCS` — aggiungere row in `docs/FOLLOWUP_TICKETS.md` per documentare visibilità fixes (Cat-3 path).

**Cross-references**: AGENTS.md v0.1 Cat-3 (zero new public SDK API surface; satisfied — production SDK builds ottengono zero simboli dal header gated) + Cat-5 (this CHANGELOG entry prepended at TOP per Cat-5 newer-at-top; FOLLOWUP_TICKETS row deferred per la forward-point TICKET-VISIBILITY-DEDUPER-DOCS) + §regole "Fare PR piccole e mirate" (single atomic chore sui 4 fix + 1 call-site prerequisite fix; gli ulteriori 5 forward-points non mischiati per Cat-3) + §regole "non committare `node_modules/`, directory di build, output, artefatti o file generati" (questo commit modifica solo 4 file source + 1 file docs; ZERO build artifacts committed) + GATE-MNT-01 closure lineage (`tools/wrap_push.sh` Step 4.5 auto-FF + `tools/check_main_clean.sh` pre-push verify); the canonical `src/text/text_visibility_audit.cpp` (Step 2 closes 4 §10 deltas verbatim) + `include/chronon3d/text/text_visibility_audit.hpp` (gated header API additions Cat-3 conforma) + `apps/chronon3d_cli/commands/dev/command_inspect_text.cpp` (Fix d doc refresh) + `src/render_graph/nodes/TextRunNode.cpp` (call-site fix prerequisite compile-fix).
...
**fix(text): terminate CHRONON3D_BUILD_DIAGNOSTICS ifdef macro** — adds missing #endif at end of src/text/text_visibility_audit.cpp (file opens #ifdef at line 40 but never close). Updates stale comment in line 7 to reference new include path. Compile-error "unterminated #ifdef" surfaced only after alpha_bbox_scanner.hpp relocation progressed the build.
