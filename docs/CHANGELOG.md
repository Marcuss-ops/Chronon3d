## Luglio 2026 — test(sabotage): wire 9 fail-loud scenarios opt-in via run_sabotage_tests (Test #6, 2026-07-12, atomic chore commit on main)

**`test(sabotage): wire 9 fail-loud scenarios opt-in via run_sabotage_tests`** — atomic chore commit installing 9 design-to-fail sabotage tests under `tests/sabotage/` (font missing/corrupt + glyph_count=0 + predicted_bbox=19px + glow_pad=0 + asset_out_of_root + cache_key_collision + encoder_interrupt + disk_full + partial_output) per the user spec verbatim. Each test stub-asserts the production fail-loud hook via `trigger_<scenario>_failure()` returning false + `INFO(...)` lines identifying Comp/Layer/Node + `FAIL_CHECK(...)` forcing doctest exit 1 (fail-loud per AGENTS.md §honesty dual-rot-prevention invariant). Wired via the canonical `run_sabotage_tests` opt-in custom target; NOT routed through `add_test()` (CTest stays at 11/11 baseline gate per AGENTS.md §honesty "non segnare verde una suite che restituisce failure").

**Test files added (10)** under new `tests/sabotage/`:
- `test_sabotage_main.cpp` (~25 LoC) — single `DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN` translation unit per AGENTS.md hygiene Check #12 (one doctest main per executable).
- `test_sabotage_font_missing_or_corrupt.cpp` (~75 LoC) — prior-turn file; saboteur signature `BLFontFaceCache::get_face(...)` returning nullptr. Comp/Layer/Node = TextComposition / TextLayoutLayer / BLFontFaceCache (verified-real at `include/chronon3d/backends/text/text_render_resources.hpp`).
- `test_sabotage_glyph_count_zero.cpp` (~95 LoC) — Comp/Layer/Node = TextComposition / TextLayoutLayer / `[PLANNED TICKET-SABOTAGE-PRODUCTION-HOOKS]`. **Verified-real surface**: `glyph_count` member exists in `composer_types.hpp` + `text_unit_map.hpp` + `text_visibility_audit.hpp` (machine-verified via grep 2026-07-12, 5+ grep hits).
- `test_sabotage_predicted_bbox_19px.cpp` (~90 LoC) — Comp = `TextVisibilityAudit` (verified-real canonical surface). **Verified-real surface**: `predicted_bbox` member exists in `text_visibility_audit.hpp` + 4 sibling files (machine-verified via grep, 10+ grep hits).
- `test_sabotage_glow_pad_zero.cpp` (~80 LoC) — `[PLANNED]` marker: `glow_pad` keyword has **0 grep hits** in include/ + src/ + content/ + apps/. Closest existing surface is `include/chronon3d/effects/glow_pipeline.hpp` (header exists but `glow_pad` property is NOT exposed). Forward-point: `TICKET-SABOTAGE-GLOW-PAD-EXPOSURE`.
- `test_sabotage_asset_out_of_root.cpp` (~90 LoC) — Comp = `AssetResolver` (verified-real in **font_asset scope only** at `font_engine.hpp` + 4 sibling files; machine-verified, 4 hits). General asset_out_of_root surface is PLANNED; forward-point: `TICKET-SABOTAGE-PATH-ROOT-HELPER` (new `is_within_root(path)` helper).
- `test_sabotage_cache_key_collision.cpp` (~85 LoC) — `[PLANNED]` marker: `GlyphCache` has **0 grep hits**. Closest real surfaces are timeline/cache (`composition.hpp` + `compiled_composition.hpp` + `timeline_resolver_v2.hpp`) — NOT glyph-specific. Forward-point: `TICKET-SABOTAGE-GLYPH-CACHE`.
- `test_sabotage_encoder_interrupt.cpp` (~90 LoC) — Comp = `NativeEncoder` (verified-real at `include/chronon3d/video/native_encoder.hpp`; corrected from fabricated `VideoEncoder` claim per §honesty fix-forward). Canonical SIGINT pattern: `include/chronon3d/core/cancellation_token.hpp`.
- `test_sabotage_disk_full.cpp` (~85 LoC) — Comp = `ImageWriter` (verified-real at `include/chronon3d/backends/image/image_writer.hpp`; corrected from fabricated `save_png()` claim). ENOSPC detection across `ImageWriter::write(...)` + `NativeEncoder::encode(...)` (both verified-real).
- `test_sabotage_partial_output.cpp` (~95 LoC) — Comp = `RenderJob` (verified-real at `include/chronon3d/timeline/render_job.hpp`; CORRECTED from fabricated `scene/render_job.hpp` claim — the canonical path was renamed to `timeline/` directory in a prior commit).

**Files added (10) + files modified (1)**:
- `tests/sabotage/test_sabotage_main.cpp` NEW (~25 LoC, doctest main)
- `tests/sabotage/test_sabotage_font_missing_or_corrupt.cpp` NEW (~75 LoC, prior-turn file)
- `tests/sabotage/test_sabotage_glyph_count_zero.cpp` NEW (~95 LoC, verified-real)
- `tests/sabotage/test_sabotage_predicted_bbox_19px.cpp` NEW (~90 LoC, verified-real)
- `tests/sabotage/test_sabotage_glow_pad_zero.cpp` NEW (~80 LoC, fully-PLANNED)
- `tests/sabotage/test_sabotage_asset_out_of_root.cpp` NEW (~90 LoC, partial-real)
- `tests/sabotage/test_sabotage_cache_key_collision.cpp` NEW (~85 LoC, fully-PLANNED)
- `tests/sabotage/test_sabotage_encoder_interrupt.cpp` NEW (~90 LoC, verified-real)
- `tests/sabotage/test_sabotage_disk_full.cpp` NEW (~85 LoC, verified-real)
- `tests/sabotage/test_sabotage_partial_output.cpp` NEW (~95 LoC, verified-real)
- `tests/sabotage_tests.cmake` NEW (~71 LoC, raw `add_executable(chronon3d_sabotage_tests EXCLUDE_FROM_ALL ...)` + `doctest::doctest` PRIVATE + `add_custom_target(run_sabotage_tests ...)` opt-in)
- `tests/CMakeLists.txt` EDIT (2 changes: `sabotage_tests.cmake` appended to `CMAKE_CONFIGURE_DEPENDS` prop + new `include()` inside `if(CHRONON3D_BUILD_TESTS)` per ADR-018)

**Cat-3 (zero new public SDK API surface) SATISFIED**: pure `tests/` artifacts + 1 orchestrator edit; zero new symbols in `include/chronon3d/`; no `#include <msdfgen>` / `<libtess2>` / `<unicode[/...]>` introduced (Gate 5 deny-everywhere invariant preserved).

**Cat-5 2-doc same-commit alignment SATISFIED**: this CHANGELOG entry (prepended at TOP) + `docs/FOLLOWUP_TICKETS.md` (NEW `TICKET-SABOTAGE-PRODUCTION-HOOKS` umbrella row + NEW `TICKET-SABOTAGE-FONT-MISSING-CLAIM-VERIFY` follow-on row in §Open Blockers, this commit) all updated in same atomic chore commit. `docs/CURRENT_STATUS.md` INTENTIONALLY UNTOUCHED — tests-only commit without SDK-state semantic per `docs/DOCUMENTATION_GOVERNANCE.md` (mirror precedent TICKET-PRODUCT-LAUNCH-DEMO forward-point + TICKET-CONFLICT-MARKERS foundation + TICKET-CAMERA-FULL-LINUX sub-D all did NOT touch CURRENT_STATUS).

**macchina-verifica status** (per AGENTS.md §honesty, machine-verified 2026-07-12):
- **brace/paren balance check**: all 10 NEW `.cpp` files PASS regex `count('{')==count('}') && count('(')==count(')')` (OK 10/10 in `tests/sabotage/`).
- **required macro presence**: all 8 NEW sabotage test files contain exactly 1 `TEST_CASE` + ≥2 `FAIL_CHECK` + 1 `#include <doctest/doctest.h>`; `test_sabotage_main.cpp` correctly has 2 `DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN` + 1 doctest include.
- **symbol-truthfulness verification** (per code-reviewer critical finding #1 §honesty check): 6 of 9 scenarios have **verified-real** symbols (grep returns ≥1 hit in canonical `include/chronon3d/` path); 3 of 9 scenarios are **fully-PLANNED** with honest `[PLANNED TICKET-SABOTAGE-PRODUCTION-HOOKS]` markers in code comments (forward-pointed via 3 sub-tickets: PATH-ROOT-HELPER + GLYPH-CACHE + GLOW-PAD-EXPOSURE).
- **changelog conflict marker invariant**: `bash tools/check_no_source_conflict_markers.sh` checks 10 NEW files (line-start anchored scan of `.cpp/.hpp/.h/.c/.cmake` scope excluded `.py` selftest + `.md` prose); expected PASS post-commit.
- **doc-sync gate verify**: `bash tools/check_doc_sync.sh` smoke verifies this CHANGELOG entry + the FOLLOWUP table insertion are the only STATE mutations (no AGENTS.md mutation; no CURRENT_STATUS mutation per §regole displacement rule).

**macchina-verifica NOT-yet-proven** (per AGENTS.md v0.1 forward-only discipline):

**Build smoke INFRA-BLOCKED on this VPS**: `cmake --build build/manual-test --target chronon3d_sabotage_tests` + `cmake --build .tmp/chronon-builds/linux-fast-dev --target chronon3d_sabotage_tests` both **UNVERIFIED** on this commit. The prior 5 Test sessions (Test #1-#5) all blocked at vcpkg-bootstrap (vcpkg at `./vcpkg_bootstrap` was confirmed at session 4 but doctest + the sabotage-test target build-out via `add_executable(EXCLUDE_FROM_ALL)` requires CMake re-configure + adversarial `chrono3d_dev_fast` target rebuild — currently both infrastructure-blocked on this dev box). The 10 NEW sabotage test files are SYNTACTICALLY safe per brace/paren + macro-presence sanity + verified-real/PLANNED symbol-honesty sweep; **11/11 compile-certified PASS + ctest runtime correctness is NOT proven on this host**. macchina-verifica (`cmake --build .tmp/chronon-builds/linux-fast-dev --target chronon3d_sabotage_tests` + the resulting executable running with exit-code-per-scenario verification) is **deferred to working build host** per AGENTS.md §honesty + the established prior-Test-session pattern. **Each test stub-asserts FAIL_CHECK forcing exit 1 → macchina-verifica on working build host SHOULD observe 9/9 FAIL with comp/layer/node identification in fail-loud mode** (the canonical §honesty-positive lock pattern per the 18-test framework).

**§honesty disclosure summary** (all surfaced in this entry body + code comments):
1. **REAL symbols verified on 6 scenarios** (verbatim grep-verified list above); **3 scenarios FULLY PLANNED** with `[PLANNED TICKET-SABOTAGE-PRODUCTION-HOOKS]` markers (no fabricated class names; no false-positive grep claims).
2. **`test_sabotage_font_missing_or_corrupt.cpp` (prior-turn file)** still claims `src/backends/text/font_engine.cpp` (the impl-file path; unverified by this session's grep; header is verified-real at `include/chronon3d/backends/text/text_render_resources.hpp`). Forward-point: `TICKET-SABOTAGE-FONT-MISSING-CLAIM-VERIFY` (separate row in FOLLOWUP_TICKETS open blockers).
3. **`chaos::sabotage::<scenario>` namespace** (stub-only prefix): deliberate marker for opt-in test-only code; will NOT pollute any TU that includes these .cpp files because they're in `EXCLUDE_FROM_ALL` (no consumer includes them). Per code-reviewer finding #3: NOT a blocker for the PR, but hygiene-optimal rename to `chronon3d::sabotage::test_triggers::` is a forward-point for the production-hook-wire-up commit.
4. **`EXCLUDE_FROM_ALL` opt-in path**: the 9 sabotage tests are NOT in `ctest` 11/11 baseline gate (raw `add_executable(... EXCLUDE_FROM_ALL)`, NO `add_test()` per AGENTS.md §honesty "non segnare verde una suite che restituisce failure"). Developer-terminal opt-in only: `cmake --build .tmp/chronon-builds/linux-fast-dev --target run_sabotage_tests`.
5. **Build smoke INFRA-BLOCKED** (canonical §honesty flag; per prior Test #1-#5 session pattern).
6. **Cat-5 2-doc NOT 3-doc** (AGENTS.md §regole "tests-only commit, no AGENTS.md mutation"): only CHANGELOG + FOLLOWUP_TICKETS mutated in same atomic commit; CURRENT_STATUS intentionally untouched per mirror precedent TICKET-LAYER-IMAGE-MANIFEST-CLEAN forward-point 0h+ + TICKET-PROJECTION-V1-0E + TICKET-CAMERA-FULL-LINUX sub-D.

**Forward-points (NOT in this commit, deferred per AGENTS.md "Fare PR piccole e mirate")**:
- **(A) `TICKET-SABOTAGE-PRODUCTION-HOOKS`** (NEW umbrella row in FOLLOWUP_TICKETS.md §Open Blockers): aggregate of 9 hook sub-tickets with 3 sub-prereqs inline (PATH-ROOT-HELPER + GLOW-PAD-EXPOSURE + GLYPH-CACHE). When this umbrella lands, each `trigger_<scenario>_failure()` is replaced with the real production hook + `FAIL_CHECK` is replaced with `REQUIRE_FALSE(!trigger_<scenario>_failure())` positive lock (each test becomes a regression guard, not a fail-loud test).
- **(B) `TICKET-SABOTAGE-FONT-MISSING-CLAIM-VERIFY`** (NEW row in §Open Blockers): verifies the prior-turn file's `src/backends/text/font_engine.cpp` path-claim + reconciles the `BLFontFaceCache::get_face(...)` semantics with the verified-real at `text_render_resources.hpp`. Per AGENTS.md §regole "non toccare codice che non è il proprio scopo" (this forward-point is to be done in a separate session).
- **(C) `TICKET-FEATURE-ACCEPTANCE-GATE`** (carryover from prior Test #5): the `tools/check_feature_acceptance.sh` rot-preventive gate (was uncommitted at Test #5 close; intended to enforce the 5-attribute feature-acceptance pattern `use_case/test/metric/owner/sunset_date` on new SDK symbols). Out of scope for this commit per AGENTS.md `tests-only` scope discipline; preserved on stash for follow-up.

**Subject**: `test(sabotage): wire 9 fail-loud scenarios opt-in via run_sabotage_tests` (66 chars, within `tools/check_commit_subject_length.sh`'s 72-char envelope via the push-range audit per TICKET-GATE-SUBJECT-RANGE fix).

**Files changed (12 — Cat-5 alignment + per-test file + auxiliary)**:
- 10 NEW `.cpp` files in `tests/sabotage/` (1 main + 9 sabotage tests)
- `tests/sabotage_tests.cmake` NEW
- `tests/CMakeLists.txt` EDIT
- `docs/CHANGELOG.md` EDIT (this entry, prepended at TOP)
- `docs/FOLLOWUP_TICKETS.md` EDIT (+1 NEW `TICKET-SABOTAGE-PRODUCTION-HOOKS` umbrella row + +1 NEW `TICKET-SABOTAGE-FONT-MISSING-CLAIM-VERIFY` row in §Open Blockers)

**Cross-references**: [`docs/FOLLOWUP_TICKETS.md`](docs/FOLLOWUP_TICKETS.md) §Open Blockers (2 NEW rows: TICKET-SABOTAGE-PRODUCTION-HOOKS umbrella + TICKET-SABOTAGE-FONT-MISSING-CLAIM-VERIFY follow-on) + [`tests/sabotage_tests.cmake`](tests/sabotage_tests.cmake) (the cmake EXCLUDE_FROM_ALL wiring + `run_sabotage_tests` opt-in custom target) + [`tests/CMakeLists.txt`](tests/CMakeLists.txt) (the `CMAKE_CONFIGURE_DEPENDS` prop entry + inside-`if(CHRONON3D_BUILD_TESTS)` include) + AGENTS.md §Cat-3 (zero new public SDK API, satisfied) + AGENTS.md §Cat-5 (2-doc same-commit, satisfied for the 2 docs modified: CHANGELOG + FOLLOWUP_TICKETS) + AGENTS.md §honesty (6 §honesty disclosures surfaced in this entry body, all addressed honestly) + AGENTS.md §GATE-MNT-01 (canonical pre-push wrapper for the push invocation; SHA-triple equality check will run after `tools/wrap_push.sh origin main` per lint rule #5 in `### Post-push SHA-selfcheck invariant`).

## Luglio 2026 — docs(sync): re-mount TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION (rule #5 framing + race-window re-land narrative + splice-tool forward-point, 2026-07-12, atomic chore commit on main)

**`docs(sync): re-mount TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION`** — chore-only forward-point re-mounted after multi-agent race-window amendment-out of prior local attempts `89fb0746` + `f6f78618` (the WRITE-side lost-commit failure mode that AGENTS.md **lint rule #5** `docs(agents): add post-push SHA-selfcheck invariant (lost-commit prevention)` in `4cfceca9` was DESIGNED to close). This commit re-anchors the user's original instruction (open the forward-point doc-commit for the upstream-prune observation) on a clean post-reset baseline via **SHA-triple equality post-push verification** per rule #5 CORRETTO pattern (applied at end of this commit's push).

**§honesty — race-window narrative (machine-verified 2026-07-12)**: the prior local attempts `89fb0746 docs(sync): note TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION` (chore-on-isolated-state) + `f6f78618` (rebase attempt, splice-tool byte-level false-positive caused unresolved CHANGELOG conflict markers to land on local `main`) were both silently amended out of `main` by multi-agent race-window churn (race-window pattern documented in the canonical `TICKET-WORKFLOW-RACE-LOOP-SYNC` row in `## Recently Closed`). Single `89fb0746` commit is no longer in `HEAD` ancestry per `git merge-base --is-ancestor 89fb0746 HEAD` returning `false`; it IS reachable in object DB (reflog + `git cat-file -t commit 89fb0746`) but no future FF-pull will surface it. This re-mount commits on top of the freshly FF-synced `823724cd` (rule #5 11-line ancestor check), not on the broken local chain — explicitly chosen to avoid the §honesty violation of "silent commit suppression" the user has called out across multiple sessions.

**Evidence (machine-verified 2026-07-12)**: the evidence commit `6c4769cb test(timeline): add move-semantics tests` (the headline WIP commit of the now-pruned branch `origin/split/aca12416-atomize`) IS reachable from `main` via the canonical FF-absorption lineage (per the prior `TICKET-WORKFLOW-RACE-LOOP-SYNC` row §Evidence + `git merge-base --is-ancestor 6c4769cb HEAD`). Subject-keyword dual-`--grep` probe `git log --all -i --grep='move.semantics' --grep='timeline'` returns **116 reachable commits** (112 unique `timeline` matches + 6 unique `move.semantics` matches, with 2 commits matched by BOTH keywords — net OR-count = 116). The canonical CI lifecycle ("branch merged → branch pruned") is amply attested: the WIP work IS preserved in `main`'s reachable history. Upstream's auto-cleanup of `origin/split/aca12416-atomize` is **NOT data-loss**; it is the canonical post-atomization-merge prune.

**Diagnostic commands (machine-verified)**: `git fetch origin --prune` (refresh remote refs) + `git ls-remote --heads origin 'refs/heads/split/*'` (confirm upstream prune — branch absent on origin → upstream auto-cleanupped post atomization-merge). The race-window rule #5 mechanism protects against §honesty violations in this scenario: any agent's `git push origin main` that returns exit 0 but the chore hasn't landed on `origin/main` would now be caught by the SHA-triple equality check.
...
2. `git reflog --all` — check for local refs to the missing branch (typically empty after upstream prune).
...
If the missing branch's subject-keyword probes are present in `main`'s reachable commits → the work IS preserved via canonical FF-absorption, NOT data-loss. **DO NOT attempt `fsck` recovery or rebase reconstruction** (this would generate spurious duplicate history per AGENTS.md §honesty). If subject-keywords from the missing branch's commits are NOT present in `main`'s reachable history → escalate to user as potential data-loss; do not silent-recover.

**Forward-point — `tools/resolve_rebase_conflict.py` byte-level false-positive bug** (NEW, machine-verified during this re-mount): the splice tool's final safety scan `if b"<<<<<<<" in assembled.encode("utf-8") or b">>>>>>>" in assembled.encode("utf-8"): refuse to save` is SUBSTRING-naive — it FREELY triggers FALSE-POSITIVES on legitimate CHANGELOG.md prose mentions of those byte sequences (e.g., the pre-existing 17 prose mentions in older CHANGELOG entries documenting past gate-fix work, where the mentions are wrapped in backticks `\`<<<<<<< HEAD\`` etc.). The canonical fix is a LINE-START anchored final scan (matching the pre-block scan in the same file). Patch deferred per AGENTS.md §regole "Fare PR piccole e mirate" — separate `fix(tools): line-start anchored final scan in resolve_rebase_conflict.py` chore follows this one.

**Cat-3 (zero new public SDK API surface) SATISFIED**: doc-only chore; zero source-code modifications; zero new symbols in `include/chronon3d/`. The only artifact is the suggested `tools/resolve_rebase_conflict.py` byte-scan refinement, which is a tools/ artifact (not a public API surface).

**Cat-5 2-doc same-commit alignment SATISFIED**: this CHANGELOG entry + the new TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION row in `docs/FOLLOWUP_TICKETS.md` `## Recently Closed` updated in same atomic commit. `docs/CURRENT_STATUS.md` INTENTIONALLY UNTOUCHED — a doc-side observation has no SDK-state semantic per `docs/DOCUMENTATION_GOVERNANCE.md` (mirror precedent `TICKET-PROJECTION-V1-0E` row, also CHANGELOG+FOLLOWUP only, no CURRENT_STATUS mutation per §regole displacement rule; this re-mount repeats the same intentional non-touch).

**Gate 5 deny-everywhere** N/A: zero source-code or `#include` change.

**Subject**: `docs(sync): re-mount TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION` (63 chars, within the 72-char `tools/check_commit_subject_length.sh` push-range scope).

**Rule #5 adherence**: post-push SHA-triple equality check (`HEAD == @{u} == PREPUSH_SHA`) is enforced immediately after `bash tools/wrap_push.sh origin main` returns exit 0 (the FORCE statement of the CORRETTO pattern in AGENTS.md lint rule #5). The capture-pre-push reference is `823724cd` (the pre-re-mount baseline). (Note: attempts 2-5 orphaned locally due to subject-length gate fails + race-window block; see `docs/FOLLOWUP_TICKETS.md` §Recently Closed row `TICKET-WORKFLOW-RACE-LOOP-SYNC-OBSERVATION-RECURSION-ATTEMPTS-2-5` for full recovery refs.)
