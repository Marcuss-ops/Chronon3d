...
### TICKET-104 -- PendingTextRun::consumed real-decrement + commit() selector/animator chain validation (cat-3 #5, 2026-06-30)

2026-06-30 -- Refactor cat-3 freeze-compliant che rinforza `PendingTextRun::consumed` come decremento REALE dello stato pending (era documentato come no-op nel header comment).  Nuovi NEW `src/text/pending_text_run_impl.{hpp,cpp}` (sotto `src/text/`, MAI in `include/chronon3d/`) espongono `chronon3d::text_internal::mark_consumed(PendingTextRun&)` + `consumed_decrement_count()` + `reset_consumed_decrement_count()` su namespace `chronon3d::text_internal` (analogo a `src/text/text_document_compile_result.hpp`). Implementazione: `run.consumed = true` flags the spec + `std::atomic<std::size_t> g_consumed_decrements.fetch_add(1, std::memory_order_relaxed)` con diagnostico contatore processo-wide.  Sostituzione con `mark_consumed(spec)` di `spec.consumed = true` in `LayerBuilder::build()` (chiude il silent no-op pre-TICKET-104).  Aggiunto selector/animator chain validation in `TextRunBuilder::commit()`: `.selector(...)` chained senza `.animator(...)` → drop orphan selectors + `spdlog::warn` consistent con `build_text_run` skip+warn pattern.  `mutable_pending()` resta pubblico (back-compat 12+ callers in `tests/authoring/test_animator_dsl.cpp` + authoring façade) ma HPP doc-comment flagga now il bypass del canonical consumed-decrement path.  Doc comments su `PendingTextRun::consumed` aggiornati per riflettere il nuovo contract + helper dichiarazione NOT forward-declared in public header (cat-3 freeze: zero nuovi public symbols anche in sub-namespace).  Cat-3 freeze-aligned per AGENTS.md v0.1 (internal hygiene, zero `include/chronon3d/` API surface expansion).  2 nuovi TEST_CASEs deterministici (no threads, no time, no PRNG -- cat-2 freeze-fit) in `tests/text/test_builder_consumed_commit_validation.cpp`: (1) `.selector(...) .commit()` senza `.animator(...)` → spec.params.animators.size() == 0u + spdlog::warn emesso (locks the orphan-drop contract); (2) `mark_consumed(run)` imposta `run.consumed == true` AND `consumed_decrement_count()` incrementa di 1 (locks the real-decrement contract; pre-TICKET-104 il flag flip era silent contatore-frozen); re-call dopo `consumed=true` ri-incrementa (HPP doc locks this idempotency contract explicitly).  Registrato in `tests/core_tests.cmake` `CORE_BLEND2D_TESTS`.  Co-update rule rispettata: TICKET-104 row in `docs/FOLLOWUP_TICKETS.md` + recently-closed row + `docs/CURRENT_STATUS.md` § Testo (closure line) + `docs/CHANGELOG.md` (questa entry) aggiornati nello stesso atomic commit.  File modificati: NEW `src/text/pending_text_run_impl.hpp` + NEW `src/text/pending_text_run_impl.cpp` + `src/text/CMakeLists.txt` (registrazione in `chronon3d_text_core`) + `include/chronon3d/scene/builders/text_run_builder.hpp` (solo HPP doc comments on PendingTextRun + mutable_pending, zero code changes) + `src/scene/builders/layer_builder.cpp` (mark_consumed replace + comment block) + `src/scene/builders/text_run_builder.cpp` (#include + commit() selector/animator chain validation + mark_consumed at end + comment block) + NEW `tests/text/test_builder_consumed_commit_validation.cpp` + `tests/core_tests.cmake` (registrazione) + 3 docs files.

### TICKET-104 CR5 follow-up -- spdlog::warn capture via custom CapturingWarnSink + RAII CaptureSinkGuard (cat-3-clean anonymous namespace, 2026-06-30)

2026-06-30 -- Follow-up layered su TICKET-104 (cat-3 #5) per rinforzare il test (1) lock sul `spdlog::warn` user-visible fail-fast contract.  Pre-CR5, il test (1) asserta solo `spec.params.animators.size() == 0u` (structural post-condition); una regression che droppasse `spdlog::warn` ma mantenesse il silent drop sarebbe passato.  CR5 aggiunge: (a) anonymous-namespace `CapturingWarnSink` che deriva da `spdlog::sinks::base_sink<std::mutex>` (header-only, transitive-link via SDK target chain) + override `sink_it_()` che filtra `spdlog::level::warn` + materializza payload in `std::vector<std::string> messages_` guarded da `base_sink::mutex_` (lock automatico prima di `sink_it_()`); (b) anonymous-namespace `CaptureSinkGuard` RAII struct che push del sink in `spdlog::default_logger()->sinks()` al ctor + erase via `std::remove_if` + pointer equality al dtor (delta-fail-safe: anche se REQUIRE fails early, dtor fa cleanup); (c) `CaptureSinkGuard warn_capture;` installato PRIMA di `trb.commit()` in TEST_CASE (1); (d) `REQUIRE(warn_capture.sink->message_count() >= 1u)` + `CHECK(found_orphan_drop_token)` che cerca `"selector spec(s) dropped"` nelle captured strings.  Cat-3-clean: entrambi struct sono ANONYMOUS-NAMESPACE-local al TU `tests/text/test_builder_consumed_commit_validation.cpp` (zero nuovi public symbols, zero expand `include/chronon3d/` API surface).  Include aggiuntivi: `<spdlog/spdlog.h>` + `<spdlog/sinks/base_sink.h>` (entrambi header-only, linkage transitivo via SDK targets; niente nuove `target_link_libraries` entries necessarie).  Co-update rule rispettata: TICKET-104 recently-closed row in `docs/FOLLOWUP_TICKETS.md` (CR5 follow-up annotation) + `docs/CURRENT_STATUS.md` § Testo closure line CR5 increment + `docs/CHANGELOG.md` (questa entry) aggiornati nello stesso atomic commit.  File modificati: `tests/text/test_builder_consumed_commit_validation.cpp` (anonymous-namespace `CapturingWarnSink` + `CaptureSinkGuard` + includes + CR5 assertions in TEST_CASE (1)) + `docs/FOLLOWUP_TICKETS.md` (recently-closed row extension) + `docs/CURRENT_STATUS.md` § Testo (closure line CR5 follow-up annotation) + `docs/CHANGELOG.md` (questa entry).
