...
> **Phase E (2026-07-06, action-plan landing from AE-parity cinematic strategy):** Strategia AE-parity cinematic text-golden expansion formalizzata in ticket index (FOLLOWUP_TICKETS.md: 15 nuovi ticket cinematici `TICKET-AE-PARITY-CINEMATIC-06..20` + 2 nuovi killer test `TICKET-AE-PARITY-KILLER-PER-CHAR-3D` + `TICKET-AE-PARITY-KILLER-WIGGLY-WAVE-EXPRESSION` + 2 driver `TICKET-AE-PARITY-DRIVER` referee AE-side + `TICKET-AE-PARITY-FLOOR` deliverable). I killer test `motion_blur` + `subtitle_word_timing` gi&agrave; coperti da `TICKET-MOTION-BLUR-TEXT` + `TICKET-GOLDEN-30` forward-only, NON duplicati ma cross-link. ROADMAP.md milestone "M1.AE-PARITY-CINEMATIC-EXPANSION" 5-step workplan (1=scene IMPL 06..20, 2=killer test driver, 3=288 PNG floor, 4=referee pipeline, 5=cinematic preset registry + SDK certifier) &egrave; dichiarato forward-only post-baseline-verde (gate #10 11/11 PASS macchina-verificato sullo stesso commit) e post `TICKET-GOLDEN-CAPTURE` closure. Doc-only questo commit (3 file canonical: CURRENT_STATUS.md + FOLLOWUP_TICKETS.md + ROADMAP.md); zero codice in `src/`, `include/chronon3d/`, `apps/`, `cmake/`, `tests/` toccato. AGENTS.md v0.1 §Anti-greenwashing compliance: tutti i 17+ ticket di questo workstream sono PLANNED post-baseline-verde; nessuna claim di PASS macchina-verificato e nessuna stima % (vedi AGENTS.md revoca feature-freeze richiede 11/11 PASS sullo stesso commit). **Stato osservabile (snapshot 2026-07-06):** cinematic AE-parity text-golden scene IMPL = **5/20** (ae_01..05 gi&agrave; shipped storicamente, ae_06..20 PLANNED in questo commit); PNG tracked = **30/240 floor target** (Phase D floor 30 ✓; post-285 floor expansion = 120 cinematic + 20 user-spec + 6 motion-blur + 4 killer = ~150 floor deliverable); user-spec golden IMPL = **12/12** (ADR-014 Phase A+B); capture tracked = **20 PNG** user-spec; motion-blur text = PLANNED (`TICKET-MOTION-BLUR-TEXT`); per-char-3D = PLANNED (`TICKET-AE-PARITY-KILLER-PER-CHAR-3D`); wiggly-wave-expression = PLANNED (`TICKET-AE-PARITY-KILLER-WIGGLY-WAVE-EXPRESSION`); subtitle-word-timing PLANNED (`TICKET-GOLDEN-30`); referee pipeline = PLANNED (`TICKET-AE-PARITY-DRIVER`); cinematic preset registry v1 SDK certifier = PLANNED dentro ROADMAP milestone M1.AE step 5. Nessun GREENWASHING: pre-fix cat-2 coverage depth = PARTIAL (5/20 scene IMPL, 30/240 floor PNG); cat-2 freeze rimane attivo fino a 11/11 gate macchina-verificato.
