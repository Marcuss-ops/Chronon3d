...
| Text Production / CapCut-grade V1 | PARTIAL | Text Export V1 certified; geometric ink-bbox fix landed + golden matrix subtitle FAST mode PASS without overflow; SubtitleTrackBuilder now uses composition frame rate with [start,end) semantics (7 rates verified). `text_preset_visual`/`text_golden` regressions remain. 5/20 general presets, 4/8 subtitle presets golden, no SRT/word-timing, per-word highlight not wired; vedi [TICKET-TEXT-PRODUCTION-STATUS-CORRECTION](tickets/TICKET-TEXT-PRODUCTION-STATUS-CORRECTION.md) + [TICKET-TEXT-BBOX-OVERFLOW](tickets/TICKET-TEXT-BBOX-OVERFLOW.md). |
