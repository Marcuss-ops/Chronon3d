...
- 🟢 **FASE 4 thread_local permutation cache → static+mutext (Cat-3)** (chiusa per docs in [FOLLOWUP_TICKETS.md §TICKET-086](FOLLOWUP_TICKETS.md)): `thread_local std::unordered_map<PermutationKey, …>` rimosso da `src/text/glyph_selector.cpp::get_or_build_permutation()`; sostituito da file-local `static std::unordered_map<…>` + `static std::mutex` (lock on miss + insert). Fisher–Yates algorithm + `hash_to_unit_float` tie-breaking byte-identical output per seed. Cat-3 freeze-aligned, zero pubblica API change.
- 🟢 **FASE 4 HarfBuzz buffer pool in FontEngine (Cat-1 internal perf)** (chiusa per docs in [FOLLOWUP_TICKETS.md §TICKET-088](FOLLOWUP_TICKETS.md)): `hb_buffer_create()`/`hb_buffer_destroy()` per-call sostituiti da per-`Impl` pool (`kMaxHbBufferPoolSize=64`, `mutable std::mutex hb_buffer_pool_mutex`); `acquire_hb_buffer()` pops+lock; `release_hb_buffer()` chiama `hb_buffer_reset()` + push back (o destroy se pool pieno); `~Impl()` chiama `destroy_hb_buffer_pool()` prima di `clear_cache_unlocked()`. Cat-1 freeze-compliant. Effect full si attiva post-TICKET-094 (FontFaceLease) che sostituirà il `unique_lock` globale.
- 🟢 **FASE 4 draw_text_run → private stages (Cat-3 internal refactor)** (chiusa per docs in [FOLLOWUP_TICKETS.md §TICKET-090 / TICKET-090b](FOLLOWUP_TICKETS.md)): 2 lambda self-contained (`expand_per_glyph`, `apply_box_blur`) lifted ad anonymous-namespace free functions `expand_per_glyph_bbox(min_x/min_y/max_x/max_y, glyphs, placed)` + `apply_separable_box_blur(image, radius, scratch_state)`; lambda orchestrator `draw_run_layer` rinominata `render_tier_to_image`. TICKET-090b aggiunge 9 inline `// ── Stage N — XYZ ───` markers in `draw_text_run()` (Stage 0 validation → Stage 8 composite), collassa 60-line banner block sopra i helpers, drop dead-comment near `apply_separable_box_blur()`. Zero behaviour change. Cat-3 freeze-aligned.
- 🔵 FASE 4 residual sub-tasks deferred post-baseline-verde per violare freeze v0.1 (vedi [FOLLOWUP_TICKETS.md §TICKET-091/093/094/095/096/097](FOLLOWUP_TICKETS.md)): `class CompiledTextAnimatorProgram` (TICKET-091 = NUOVA pubblica classe); umbrella tier-buffer completion in `TextScratchState` (TICKET-093 = public-header accessor); `class FontFaceLease` per-face (TICKET-094 = NUOVA pubblica classe + lock-narrowing coupling); `measure_text_fast()` cheap-path (TICKET-095 = public-API add); rimozione external `shared_mutex` da glyph atlas + text raster cache (TICKET-096 = public RAII accessor); Cat-2 regression harness per TICKET-088/090 (TICKET-097 = TICKET-011 build-rot dipendenza).
- 🟢 **FASE 5 text_preset_registry per-category register helpers + internal domain aliases (Cat-3)** (chiusa per docs in [FOLLOWUP_TICKETS.md §TICKET-098 / §TICKET-099a](FOLLOWUP_TICKETS.md)): 4 per-category register helpers (`register_text_preset_cinematic/reveal/emphasis/subtitle`) added inside l'anonymous namespace di `src/registry/text_preset_registry.cpp`; `register_builtin_presets(r)` ora delega ai 4 helpers preservando l'ordine di inserimento (Cinematic → Reveal → Emphasis → Subtitle). Nuovo `src/text/aliases.hpp` con 7 typed aliases in `namespace chronon3d::text::domain` (`FontAssetId` = `std::string_view`, `ResolvedFontPath`/`FontFamilyName`/`Bcp47LanguageTag`/`TextPresetId`/`SelectorId` = `std::string`, `FontWeight` = `int`) — strictly under `src/text/` (NO public `include/chronon3d/` exposure). Zero nuovi surface API pubblici = Cat-3 freeze-aligned. FASE 5 residue (header splits pubblici, public domain type promotion, `commit() → Result<LayerBuilder&, TextAuthoringError>`, `enum class BidiCapability { Full, LTRFallbackOnly }` + diagnostic, `mutable_pending()` removal + 47+ test callsite migration, full text_preset_registry.cpp file-level extraction → 4 .cpp files) deferred post-baseline-verde per violare freeze v0.1 (vedi [FOLLOWUP_TICKETS.md §TICKET-100..106](FOLLOWUP_TICKETS.md)).
