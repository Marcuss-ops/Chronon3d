...
| Text Production V1                              | NOT RUN  | word timing, rich text produttivo, preset, golden da chiudere. **M1.5#7** canonical ownership di `TextRenderResources` ora su `SoftwareSessionResources.text_resources` (WP-3: fuori da engine-generic RenderSession); TICKET-M1.5#7-FULL-SPLIT aperto per split monolitico + caller migration. |
