...
| **AE_CAM telemetry dashboard live**           | PARTIAL  | 8 nuovi render AE_CAM_02..09 pushati in `render_runs` (155→163), tutti `success=1`. MP4 AE_CAM_02_zoom_fov generato (61 frames @ 30fps, ISO Media MP4 base media v1). Flask back-end `:8000` serving `GET /api/runs` (163 entries JSON array); dashboard SPA live su `http://57.131.20.173:5173/` con 10/10 composizioni AE_CAM in sidebar. `tools/check_video_subcommand.sh` Cat-1 build verifier introdotto (commit `8a76d4a`). 2/10 AE_CAM ancora da renderizzare (`01_static_grid`, `10_near_clip`). Cat-5 doc-only questo commit; zero codice toccato in `src/`, `include/chronon3d/`, `apps/chronon3d_cli/`, `cmake/presets/linux-fast-dev.json`. |
