...
- **Convenzione precisione**: tutti i line-ref `~NNN` in questo doc sono **range approssimativi** (per modifiche continue del codice post-`4105c547`); la riga esatta richiede `git blame -L NNN,+1 <file>` (range obbligatorio) o `git log -L '<start_regex>,<end_regex>:<file>'` (con `:` finale prima del path) su `main@651f6d64`.
