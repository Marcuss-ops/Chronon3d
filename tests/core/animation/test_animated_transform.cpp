...
TEST_CASE("AnimatedTransform: evaluate opacity") {
    ...
    CHECK(at.evaluate(SampleTime::from_frame_int(Frame{0}, FrameRate{30, 1})).opacity  == doctest::Approx(0.0f));
    CHECK(at.evaluate(SampleTime::from_frame_int(Frame{15}, FrameRate{30, 1})).opacity == doctest::Approx(0.5f));
    CHECK(at.evaluate(SampleTime::from_frame_int(Frame{30}, FrameRate{30, 1})).opacity == doctest::Approx(1.0f));
}

TEST_CASE("AnimatedTransform: evaluate anchor") {
    AnimatedTransform at;
    at.anchor.key(0, Vec3{0, 0, 0}).key(60, Vec3{100, 50, 0});
    Transform t = at.evaluate(SampleTime::from_frame_int(Frame{60}, FrameRate{30, 1}));
...
TEST_CASE("AnimatedTransform: static (no keys) returns defaults") {
    AnimatedTransform at;
    Transform t = at.evaluate(SampleTime::from_frame_int(Frame{999}, FrameRate{30, 1}));
