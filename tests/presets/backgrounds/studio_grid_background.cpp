...
void studio_grid_background(
    ...
    s.layer("studio_grid_anim_3d", [params, ctx](LayerBuilder& l) {
        l.enable_3d();
        l.position({0.0f, 0.0f, 400.0f});
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            float offset_y = 0.0f;
            if (params.animated) {
                offset_y = -std::fmod(static_cast<float>(ctx.frame) * speed * 1.5f, 1000.0f);
            }
