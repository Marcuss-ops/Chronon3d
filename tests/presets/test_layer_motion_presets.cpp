...
TEST_CASE("LayerBuilder::slide_in creates animated keyframes") {
    ...
    Transform t0 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{0}, FrameRate{30, 1}));
    ...
    Transform t45 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{45}, FrameRate{30, 1}));
...
TEST_CASE("LayerBuilder::soft_pop creates scale + opacity keyframes") {
    ...
    Transform t30 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{30}, FrameRate{30, 1}));
...
TEST_CASE("LayerBuilder::float_idle creates looping position keyframes") {
    ...
    Transform t60 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{60}, FrameRate{30, 1}));
    CHECK(t60.position.y == doctest::Approx(0.0f).epsilon(0.1f));

    // End of cycle should also be back to origin.
    Transform t120 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{120}, FrameRate{30, 1}));
...
TEST_CASE("LayerBuilder::settle creates overshoot keyframes") {
    ...
    Transform t20 = l.anim_transform.evaluate(SampleTime::from_frame_int(Frame{20}, FrameRate{30, 1}));
