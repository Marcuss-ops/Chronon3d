...
TEST_CASE("Authoring/Material: material::neon() enables emissive finish") {
...
TEST_CASE("Authoring/Material: material::flat() stays a neutral material") {
    Material m = material::flat();
    auto built = MaterialTestAccess::release(std::move(m));
    CHECK(built.enabled             == true);
    CHECK(built.emissive           == doctest::Approx(1.0f));
}
...
TEST_CASE("Authoring/Material: premium() + chained setters matches hand-built") {
    ...
    CHECK(built.emissive         == doctest::Approx(manual.emissive));
}
...
TEST_CASE("Authoring/Material: chain returns *this for stable identity") {
    ...
    Material& r3 = r2.inner_shadow({0.0f, 2.0f}, 4.0f, Color::black());
    CHECK(&m  == &r1);
    CHECK(&r1 == &r2);
    CHECK(&r2 == &r3);
}
