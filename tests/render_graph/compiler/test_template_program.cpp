...
// ── Target First-Principles Architecture Tests ──────────────────────────────
TEST_CASE("FirstPrinciples: PixelDomain properties and conversions") {
    CHECK(pixel_domain_is_yuv(PixelDomain::NV12));
    CHECK(pixel_domain_is_yuv(PixelDomain::P010));
    CHECK_FALSE(pixel_domain_is_yuv(PixelDomain::RGBA8));
    CHECK(pixel_domain_is_rgb(PixelDomain::RGBA8));
    CHECK(pixel_domain_is_rgb(PixelDomain::RGBA16F));
    CHECK(pixel_domain_name(PixelDomain::NV12) == "NV12");
    CHECK(pixel_domain_name(PixelDomain::RGBA16F) == "RGBA16F");
    CHECK(pixel_domain_bytes_per_pixel_approx(PixelDomain::RGBA8) == 4);
    CHECK(pixel_domain_bytes_per_pixel_approx(PixelDomain::RGBA16F) == 8);
}

TEST_CASE("FirstPrinciples: MaterializationReason and MaterializationBarrier") {
    CHECK(materialization_reason_name(MaterializationReason::NeighborhoodOperation) == "NeighborhoodOperation");
    CHECK(materialization_reason_name(MaterializationReason::TemporalHistory) == "TemporalHistory");
    CHECK(materialization_reason_name(MaterializationReason::DependentMask) == "DependentMask");
    CHECK(materialization_reason_name(MaterializationReason::ColorDomainTransition) == "ColorDomainTransition");
    CHECK(materialization_reason_name(MaterializationReason::BackendCapability) == "BackendCapability");
    CHECK(materialization_reason_name(MaterializationReason::ExternalInterop) == "ExternalInterop");

    auto g = build_synthetic_compiled_graph();
    auto prog = compile_template_program(std::move(g));

    // Validates that all materialized intermediate surfaces declare an explicit MaterializationReason
    for (const auto& barrier : prog.materializations) {
        CHECK(barrier.producer != k_invalid_node);
        CHECK(materialization_reason_name(barrier.reason) != "Unknown");
    }
}

TEST_CASE("FirstPrinciples: PrimitiveStream and Batches in CompiledTemplateProgram") {
    auto g = build_synthetic_compiled_graph();
    auto prog = compile_template_program(std::move(g));

    CHECK(prog.primitives.total_batches() == 2);
    CHECK(prog.primitives.total_primitives() == 3);
    CHECK(prog.working_domain == PixelDomain::NV12);
}

TEST_CASE("FirstPrinciples: FastPathVerificationMetrics and Canonical Fallback Register") {
    chronon3d::runtime::FastPathVerificationMetrics perfect_metrics{};
    CHECK(perfect_metrics.passes_strict_fast_path());

    chronon3d::runtime::FastPathVerificationMetrics degraded_metrics{};
    degraded_metrics.raw_node_execute_count = 1;
    CHECK_FALSE(degraded_metrics.passes_strict_fast_path());

    const auto reg = chronon3d::runtime::canonical_fallback_register();
    CHECK(reg.size() >= 5);
    for (const auto& entry : reg) {
        CHECK_FALSE(entry.legacy_path.empty());
        CHECK_FALSE(entry.replacement.empty());
        CHECK(fallback_status_name(entry.status) != "UNKNOWN");
    }
}
