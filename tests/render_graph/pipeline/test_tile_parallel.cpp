...
Composition make_two_corner_scene(int width, int height, int duration) {
    return Composition(CompositionSpec{
        ...
        float ax = 20.0f + static_cast<float>(ctx.frame.frame.frame) * 3.0f;
        s.circle("objA", {
            .radius = 8.0f,
            .color = Color::red(),
            .pos = {ax, 30.0f, 0}
        ...
        float bx = static_cast<float>(width) - 20.0f
                   - static_cast<float>(ctx.frame.frame.frame) * 3.0f;
        s.circle("objB", {
            .radius = 8.0f,
            .color = Color::green(),
    ...
    Composition comp(CompositionSpec{
        ...
        for (int i = 0; i < 8; ++i) {
            float x = 40.0f + static_cast<float>(i) * 60.0f
                      + static_cast<float>(ctx.frame.frame.frame) * 2.0f;
...
TEST_CASE("TileParallel: Stress — full-screen dirty (all tiles dirty) still correct") {
    ...
    Composition comp(CompositionSpec{
        ...
        SceneBuilder s(ctx.resource);
        float r = 0.2f + static_cast<float>(ctx.frame.frame.frame) * 0.1f;
        float g = 0.3f;
        float b = 0.5f + static_cast<float>(ctx.frame.frame.frame) * 0.05f;
        s.rect("full", {
            .size = {128.0f, 128.0f},
