...
//                         current sentinel uses this — it's declared
//                         so future sentinels can declare intent
...
// 0.05 alpha filter in compute_metrics suppresses diluted pixels —
// those are caught by PartialCoverage's permissive lower bound. A
// future Blocco 3 PR will introduce real GoldenReference matching
// and may re-tune this threshold if alpha_coverage ratios prove better.
...
inline std::string_view expectation_name(VisualExpectation e) {
    ...
    // unreachable on a well-formed enum value; suppresses -Wswitch warning
    // on compilers that don't track switch exhaustiveness on enum class.
    return "Unknown";
}

// Empirical gap between Transparent/PartialCoverage and Visible. Re-tune if
// the visible ink minimum moves below this floor; Blocco 3 will replace
// ink-based checks with hash capture, after which this constant is retired.
// The observed distribution that calibrates this value lives in the Blocco 2
// baseline doc under docs/baselines/ — do not duplicate here to avoid drift.
constexpr std::size_t kVisibleMinPixels = 2500;

// Sentinel-gate macro for Text Preset visual regression. 'short_label' is a
// stable token used by the grep-based capture workflow. NOT auto-derived
// from a TEST_CASE name to avoid whitespace / special-character fragility.
//
// Branch semantics (kept intentionally lean — no body comments; see the
// VisualExpectation enum block above for the WHY behind each branch):
//
//   Transparent      → frame is intentionally blank. ink_pixels == 0.
//   PartialCoverage  → frame is mid-animation (diluted alpha or compressed
//                      layout — the audit-cited BlurIn F020 focus_in blur
//                      and MaskedLineReveal F020 center_split scale_y both
//                      produce ink == 0 under the 0.05 alpha filter in
//                      compute_metrics()). Permissive upper bound only; do
//                      NOT add a hard ink > 0 lower bound here, or those
//                      frames would FAIL and revert the bug this enum was
//                      meant to fix. Blocco 3 will tighten via hash gating.
//   Visible          → frame has substantial text render.
//                      ink_pixels >= kVisibleMinPixels (gap between
//                      Transparent+PartialCoverage and full-render frames).
//   GoldenReference  → PLACEHOLDER for Blocco 3 (hash + bbox + PNG compare).
//                      Any sentinel tagged this today FAILS the build until
//                      the hash gate lands — a CI landmine guard, NOT a
//                      silent placeholder.
//
// TICKET-038: `emit_preset_gate` declares an outer `auto m = compute_metrics(...)`
// and passes `m` as `metrics_expr` here; the macro-local binding is named
// `gate_m` so the RHS `auto gate_m = (metrics_expr)` unambiguously receives
// the caller's `m` (or fresh inline expression). TU-local rename only.
...
// AGENT 2 (TICKET-A2) — preset is now resolver-driven (tracking 0.18→0,
// opacity 0→1 over 35f). Existing captured framebuffer hashes no longer
// reflect the actual rendering; sentinels reset to kUncapturedSentinel so
// the visual test runs in capture mode ("first hash to capture" message)
// until the next visual-rebaseline PR captures new goldens. Same macro
// applies at both 16:9 (4 sentinels) and 9:16 (4 sentinels).
constexpr std::uint64_t kRefTextPresTrackingClose_169_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_169_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_169_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_169_F040 = 0xDEADBEEFDEADBEEFULL;

constexpr std::uint64_t kRefTextPresTrackingClose_916_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_916_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_916_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresTrackingClose_916_F040 = 0xDEADBEEFDEADBEEFULL;
...
// AGENT 2 (TICKET-A2) — preset is now resolver-driven (Word-unit selector
// with animated `end` sweeping 0→100 over 48f; OpacityProperty/Position/
// Scale ramp 0→end_state over 36f). All 8 sentinels reset so existing
// captured framebuffer hashes (old layer chain + fade_in) do not engage.
constexpr std::uint64_t kRefTextPresWordCascade_169_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_169_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_169_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_169_F040 = 0xDEADBEEFDEADBEEFULL;

constexpr std::uint64_t kRefTextPresWordCascade_916_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_916_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_916_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresWordCascade_916_F040 = 0xDEADBEEFDEADBEEFULL;

// AGENT 2 (TICKET-A2) — preset is now resolver-driven (Glyph-unit
// selector with animated `end` sweeping 0→100 over 24f; OpacityProperty/
// Position/Scale ramp 0→end_state over 18f). All 8 sentinels reset
// since the rendering now differs frame-by-frame (per-word opacity ramp,
// glyph stagger) and old captured framebuffer hashes no longer apply.
constexpr std::uint64_t kRefTextPresCharacterCascade_169_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_169_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_169_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_169_F040 = 0xDEADBEEFDEADBEEFULL;

constexpr std::uint64_t kRefTextPresCharacterCascade_916_F000 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_916_F020 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_916_F030 = 0xDEADBEEFDEADBEEFULL;
constexpr std::uint64_t kRefTextPresCharacterCascade_916_F040 = 0xDEADBEEFDEADBEEFULL;
...
TEST_CASE("VRTextPreset/TrackingClose") {
    auto renderer = make_renderer();    // AGENT 2 (TICKET-A2) — tracking_close IS now entrance-animated
    ...
    //   F030 \u2192 Visible (opacity \u22480.86 \u2192 ink above VisibleMinPixels,
    //                          tracking cropped)
...
TEST_CASE("VRTextPreset/WordCascade") {
    ...
    // F020: 5-word cascade at 3-frame stagger, 36-frame-per-word opacity ramp. Avg opacity ≈ 0.39 → diluted alpha falls below the 0.05 ink threshold for many glyphs. Mid-animation, not a regression.
    ...
    // F020: same physics as 16:9 (see comment above).
...
TEST_CASE("VRTextPreset/CharacterCascade") {
    ...
    // F020: glyph cascade at 1-frame stagger, 18-frame-per-glyph opacity ramp. By F020 all glyphs are post-ramp, but per-glyph positional offset still compresses alpha below the 0.05 ink threshold. Mid-animation, not a regression.
