...
//
/// @brief   Tests for the unified Camera Projection Contract.
///
/// Verifies that camera_math::project_world_point conforms to the contract:
///   - Y sign: inverted (screen Y increases downward)
///   - Viewport centre is always added in screen coordinates
///   - Points behind camera are invisible
///   - FOV and Zoom produce equivalent results when focal is equal
///   - Depth is positive and correct
// ============================================================================
...
TEST_CASE("Camera projection contract: Y sign is inverted (screen Y down)") {
    ...
    // A point above camera centre (positive Y in world with lookAtLH up=(0,1,0))
    // should project to a LOWER screen Y (above centre in screen space).
    auto above = camera_math::project_world_point(
        cam,
        Vec3{0, 100, 0},
        camera_math::Viewport2D{1920, 1080}
    );

    auto center = camera_math::project_world_point(
        cam,
        Vec3{0, 0, 0},
        camera_math::Viewport2D{1920, 1080}
    );
    ...
    // World Y positive = "up" → screen Y should be lower (above centre)
    CHECK(above.screen.y < center.screen.y);

    // A point below camera centre (negative Y in world) should project
    // to a HIGHER screen Y (below centre).
    auto below = camera_math::project_world_point(
        cam,
        Vec3{0, -100, 0},
        camera_math::Viewport2D{1920, 1080}
    );
...
TEST_CASE("Camera projection contract: X sign is correct") {
    ...
    auto right = camera_math::project_world_point(
        cam,
        Vec3{100, 0, 0},
        camera_math::Viewport2D{1920, 1080}
    );
    ...
    CHECK(right.visible);
    CHECK(right.screen.x > center.screen.x);  // right of centre
}
...
TEST_CASE("Camera projection contract: depth increases with distance") {
    ...
    auto near_pt = camera_math::project_world_point(
        cam,
        Vec3{0, 0, -500},
        camera_math::Viewport2D{1920, 1080}
    );

    auto far_pt = camera_math::project_world_point(
        cam,
        Vec3{0, 0, 500},
        camera_math::Viewport2D{1920, 1080}
    );
...
TEST_CASE("Camera projection contract: perspective scale varies with depth") {
    ...
    // Near layer: depth 500 → ps = 1000/500 = 2
    CHECK(near_pt.perspective_scale == doctest::Approx(2.0f).epsilon(0.01f));
    // Far layer: depth 1500 → ps = 1000/1500 = 0.667
...
TEST_CASE("Camera projection contract: FOV and zoom focal equivalence") {
    // When camera.zoom == focal_from_camera(FOV), both modes must produce
    // the same projection for the same input.
    Camera2_5D cam_fov;
...
TEST_CASE("Camera projection contract: project_world_to_screen matches contract") {
    ...
    // Path 1: camera_projection.cpp
    auto sp = project_world_to_screen(world, cam, vp);

    // Contract
    auto cp = camera_math::project_world_point(
        cam, world, camera_math::Viewport2D{vp.width, vp.height}
    );
...
TEST_CASE("Camera projection contract: project_world_point_2_5d matches contract") {
    ...
    // Legacy Path 2 helper
    Vec2 legacy_screen;
    f32 legacy_depth;
    bool legacy_ok = project_world_point_2_5d(cam, view, true, focal, world,
                                               legacy_screen, legacy_depth);

    // Contract
    auto cp = camera_math::project_world_point(
        cam, world, camera_math::Viewport2D{1920, 1080}
    );

    // project_world_point_2_5d returns centred coords (no viewport centre),
    // while the contract returns screen coords. Compare after removing centre.
...
TEST_CASE("Camera projection contract: project_layer_2_5d centred position matches contract centred") {
    ...
    // Path 2: project_layer_2_5d
    auto proj = project_layer_2_5d(tr, cam, 1920.0f, 1080.0f);
    REQUIRE(proj.visible);

    // Contract: project_world_point at the layer position
    auto cp = camera_math::project_world_point(
        cam, Vec3{100, 50, 0}, camera_math::Viewport2D{1920, 1080}
    );
    REQUIRE(cp.visible);

    // Layer transform.position is in centred coords; contract centred is also centred
...
// ────────────────────────────────────────────────────────────────────────────
// ProjectionContext (Path 3) cross-check
// ────────────────────────────────────────────────────────────────────────────
...
TEST_CASE("Camera projection contract: ProjectionContext matches contract with POI enabled") {
    ...
    // Note: make_projection_context does not set ready = true;
    // prepare_projection_context() does. Skip the ready check.

    Vec3 world{100, 50, 0};

    // Path 3: ProjectionContext
    bool ctx_ok = false;
...
TEST_CASE("Camera projection contract: focal_from_camera matches focal_length_from_fov") {
    // Backward-compatible wrapper must match the contract
    const f32 h = 720.0f;
    const f32 fov = 50.0f;
