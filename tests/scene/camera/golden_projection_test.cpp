...
// `kEpsGoldenProjection` to file-scoped unique name to avoid the redefinition
// error in the unified TU built by chronon3d_scene_tests (three
// test files shared an anonymous-namespace `kEpsGoldenProjection` that collide
// in Unity build: see TICKET-120 build-redefinition group).
constexpr f32   kEpsGoldenProjection = 1e-3f;              // 0.1% relative tolerance
...
TEST_CASE("Golden projection: 6 canonical modes project expected analytical values") {
    ...
    SUBCASE("Mode 1/6: Zoom") {
        ...
        REQUIRE(p.visible);
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(1.0f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1060.0f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(490.0f).epsilon(kEpsGoldenProjection));
    }
    ...
    SUBCASE("Mode 2/6: FOV 50°") {
        ...
        REQUIRE(p.visible);
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(1.158008f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1075.80f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(482.10f).epsilon(kEpsGoldenProjection));
        ...
        CHECK(snap.focal_x_px == doctest::Approx(snap.focal_y_px).epsilon(kEpsGoldenProjection));
    }
    ...
    SUBCASE("Mode 3/6: PhysicalLens (ARRI Alexa 35)") {
        ...
        CHECK(fxy.x == doctest::Approx(3552.35f).epsilon(kEpsGoldenProjection));
        CHECK(fxy.y == doctest::Approx(3789.47f).epsilon(kEpsGoldenProjection));

        const auto p = project_world_point(cam, kWorldPoint, Viewport2D{kVpW, kVpH});
        REQUIRE(p.visible);
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(3.789f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1315.24f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(350.53f).epsilon(kEpsGoldenProjection));
        ...
        CHECK(snap.focal_x_px == doctest::Approx(3551.61f).epsilon(kEpsGoldenProjection));
        CHECK(snap.focal_y_px == doctest::Approx(3789.47f).epsilon(kEpsGoldenProjection));
    ...
    SUBCASE("Mode 4/6: GateFit::Stretch (full-frame 50mm)") {
        ...
        const camera_math::FocalPx fxy = camera_math::focal_xy_from_camera(cam, kVpW, kVpH);
        CHECK(fxy.x == doctest::Approx(2666.67f).epsilon(kEpsGoldenProjection));
        CHECK(fxy.y == doctest::Approx(2250.0f).epsilon(kEpsGoldenProjection));
        CHECK(fxy.x != doctest::Approx(fxy.y).epsilon(0.01f));

        const auto p = project_world_point(cam, kWorldPoint, Viewport2D{kVpW, kVpH});
        REQUIRE(p.visible);
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(2.25f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1226.67f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(427.50f).epsilon(kEpsGoldenProjection));
        ...
        const auto snap = make_evaluated_projection(cam, Viewport2D{kVpW, kVpH});
        CHECK(snap.is_anamorphic);
        CHECK(snap.focal_x_px == doctest::Approx(2666.67f).epsilon(kEpsGoldenProjection));
        CHECK(snap.focal_y_px == doctest::Approx(2250.0f).epsilon(kEpsGoldenProjection));
        CHECK(snap.active_viewport.width  == doctest::Approx(kVpW));
        CHECK(snap.active_viewport.height == doctest::Approx(kVpH));
    }
    ...
    SUBCASE("Mode 5/6: GateFit::Overscan (pillarbox on full-frame 50mm)") {
        ...
        CHECK(eff.x      == doctest::Approx(150.0f).epsilon(kEpsGoldenProjection));
        CHECK(eff.y      == doctest::Approx(0.0f).epsilon(kEpsGoldenProjection));
        CHECK(eff.width  == doctest::Approx(1620.0f).epsilon(kEpsGoldenProjection));
        CHECK(eff.height == doctest::Approx(1080.0f).epsilon(kEpsGoldenProjection));

        // Step 2: focal is computed against the effective (post-bars) size.
        const camera_math::FocalPx fxy = camera_math::focal_xy_from_camera(cam, kVpW, kVpH);
        CHECK(fxy.x == doctest::Approx(2250.0f).epsilon(kEpsGoldenProjection));
        CHECK(fxy.y == doctest::Approx(2250.0f).epsilon(kEpsGoldenProjection));
        ...
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(2.25f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1185.0f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(427.50f).epsilon(kEpsGoldenProjection));
        ...
        CHECK(snap.active_viewport.width  == doctest::Approx(1620.0f).epsilon(kEpsGoldenProjection));
        CHECK(snap.active_viewport.height == doctest::Approx(1080.0f).epsilon(kEpsGoldenProjection));
        CHECK(snap.principal_point_px.x == doctest::Approx(960.0f).epsilon(kEpsGoldenProjection));
        CHECK(snap.principal_point_px.y == doctest::Approx(540.0f).epsilon(kEpsGoldenProjection));
    }
    ...
    SUBCASE("Mode 6/6: Anamorphic 2× squeeze (anamorphic_50mm)") {
        ...
        const camera_math::FocalPx fxy = camera_math::focal_xy_from_camera(cam, kVpW, kVpH);
        CHECK(fxy.x == doctest::Approx(8747.15f).epsilon(kEpsGoldenProjection));
        CHECK(fxy.y == doctest::Approx(2904.79f).epsilon(kEpsGoldenProjection));
        ...
        CHECK(raw_x == doctest::Approx(4373.58f).epsilon(kEpsGoldenProjection));
        CHECK(raw_y == doctest::Approx(2904.79f).epsilon(kEpsGoldenProjection));
        CHECK(raw_x == doctest::Approx(1.506f * raw_y).epsilon(0.01f));

        const auto p = project_world_point(cam, kWorldPoint, Viewport2D{kVpW, kVpH});
        REQUIRE(p.visible);
        CHECK(p.depth == doctest::Approx(kDepth).epsilon(kEpsGoldenProjection));
        CHECK(p.perspective_scale == doctest::Approx(2.905f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.x == doctest::Approx(1834.72f).epsilon(kEpsGoldenProjection));
        CHECK(p.screen.y == doctest::Approx(394.76f).epsilon(kEpsGoldenProjection));

        // Snap-check: anamorphic true, squeeze + pixel_aspect flow through.
        const auto snap = make_evaluated_projection(cam, Viewport2D{kVpW, kVpH});
        CHECK(snap.is_physical_lens);
        CHECK(snap.is_anamorphic);
        CHECK(snap.pixel_aspect        == doctest::Approx(1.0f).epsilon(kEpsGoldenProjection));
        CHECK(snap.anamorphic_squeeze  == doctest::Approx(2.0f).epsilon(kEpsGoldenProjection));
