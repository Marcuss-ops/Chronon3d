    ...
    void VulkanBackend::Impl::draw_text_batch(runtime::RenderSurfaceHandle destination,
        ...
        if (use_tiled_text) {
            ...
            const std::int32_t tiles_x =
                (static_cast<std::int32_t>(dst_image.width) + kTileSize - 1) / kTileSize;
            const std::int32_t tiles_y =
                (static_cast<std::int32_t>(dst_image.height) + kTileSize - 1) / kTileSize;
            const VkDeviceSize tile_count_bytes = static_cast<VkDeviceSize>(tiles_x) *
                static_cast<VkDeviceSize>(tiles_y) * sizeof(std::uint32_t);
            const VkDeviceSize tile_index_bytes = tile_count_bytes * kMaxGlyphsPerTile;
            ...
            const VkBufferMemoryBarrier upload_barriers[] = {
            ...
            vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                 ...
                                 0, nullptr, 3, upload_barriers, 0, nullptr);
            record_text_tile_bin(cmd, bin_descriptors,
                                 static_cast<std::int32_t>(glyphs.size()), tiles_x, tiles_y,
                                 kMaxGlyphsPerTile,
                                 static_cast<std::int32_t>(dst_image.width),
                                 static_cast<std::int32_t>(dst_image.height));
            ...
            record_text_tile_raster(cmd, raster_descriptors, tiles_x, tiles_y,
                                    kMaxGlyphsPerTile, dst_image, atlas_image);
            ++frame_batch.pass_count;
            dst_image.initialized = true;
            stats.passes_executed += 2;
            return;
        }
...
struct alignas(16) GpuLayerInstance {
    ...
    void VulkanBackend::Impl::ensure_staging(VkDeviceSize bytes) {
        ...
        if (debug_context) debug_context->set_buffer_name(staging.buffer, "Chronon3D.Buffer.Staging");
    ...
    void VulkanBackend::Impl::ensure_glyph_instance_buffer(VkDeviceSize bytes, std::size_t index) {
        ...
        const VkBufferCreateInfo info{
            ...
            VK_SHARING_MODE_EXCLUSIVE, 0, nullptr};
        glyph_instance_buffers[index] = memory_manager.create_buffer(info, VulkanMemoryClass::DeviceLocal);
        if (debug_context) debug_context->set_buffer_name(glyph_instance_buffers[index].buffer, "Chronon3D.Buffer.GlyphInstance");
    ...
    void VulkanBackend::Impl::ensure_layer_instance_buffer(VkDeviceSize bytes, std::size_t index) {
        ...
        const VkBufferCreateInfo info{
            ...
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_SHARING_MODE_EXCLUSIVE, 0, nullptr};
        layer_instance_buffers[index] = memory_manager.create_buffer(info, VulkanMemoryClass::DeviceLocal);
        if (debug_context) debug_context->set_buffer_name(layer_instance_buffers[index].buffer, "Chronon3D.Buffer.LayerInstance");
    ...
    void VulkanBackend::Impl::ensure_text_run_dynamic_buffer(VkDeviceSize bytes, std::size_t index) {
        ...
        const VkBufferCreateInfo info{
            ...
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            VK_SHARING_MODE_EXCLUSIVE, 0, nullptr};
        text_run_dynamic_buffers[index] = memory_manager.create_buffer(info, VulkanMemoryClass::DeviceLocal);
        if (debug_context) debug_context->set_buffer_name(text_run_dynamic_buffers[index].buffer, "Chronon3D.Buffer.TextRunDynamic");
    ...
    void VulkanBackend::Impl::composite(Framebuffer& destination, const Framebuffer& source) {
        ...
        std::memcpy(staging.mapped, packed.data(), static_cast<std::size_t>(image_bytes * 2));
        ...
        vkCmdCopyBufferToImage(command_buffer, staging.buffer, src.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &source_copy);
        vkCmdCopyBufferToImage(command_buffer, staging.buffer, dst.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &destination_copy);
        ...
        vkCmdCopyImageToBuffer(command_buffer, dst.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, staging.buffer, 1, &output_copy);
        transition(command_buffer, dst.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL);
        src.initialized = true;
        dst.initialized = true;
        submit();

        const float* output = static_cast<const float*>(staging.mapped) + (image_bytes * 2 / sizeof(float));
