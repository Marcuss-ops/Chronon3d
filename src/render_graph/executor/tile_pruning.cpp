...
std::optional<raster::BBox> compute_dirty_clip(
    ...
    if (ctx.node_exec.dirty_rect) {
        if (clip) {
            clip = raster::BBox{
                .x0 = std::max(clip->x0, ctx.node_exec.dirty_rect->x0),
                .y0 = std::max(clip->y0, ctx.node_exec.dirty_rect->y0),
                .x1 = std::min(clip->x1, ctx.node_exec.dirty_rect->x1),
                .y1 = std::min(clip->y1, ctx.node_exec.dirty_rect->y1)
            };
            if (clip->x0 >= clip->x1 || clip->y0 >= clip->y1) {
                clip = raster::BBox{0, 0, 0, 0};
            }
        } else {
            clip = ctx.node_exec.dirty_rect;
        }
    }
    ...
    if (ctx.node_exec.counters) {
        if (predicted_bbox) {
            ...
            ctx.node_exec.counters->increment_dirty_full_fallback_reason(reason);
        }
    }
