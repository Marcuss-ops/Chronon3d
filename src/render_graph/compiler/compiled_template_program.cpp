...
PhysicalResourcePlan analyze_physical_resources(
    ...
    const auto& alloc = compiled.physical_framebuffer_plan;
    ...
    for (const auto& slot : alloc.slots) {
        ...
        sized.slot_id      = slot.id;
        sized.max_width    = slot.max_width;
        sized.max_height   = slot.max_height;
        sized.surface_bytes = static_cast<std::uint64_t>(slot.max_width) *
                              slot.max_height * bytes_per_pixel;
...
compile_template_program(CompiledFrameGraph compiled_arg) {
    ...
    // ── First-Principles Architecture Population ───────────────────────
    // 1. PixelDomain inference: stay native YUV/NV12 unless linear/float effects require RGB
    bool requires_float_rgb = false;
    for (const auto& node : prog.compiled->nodes) {
        if (node.kind == RenderGraphNodeKind::MotionBlur ||
            node.kind == RenderGraphNodeKind::Effect) {
            requires_float_rgb = true;
            break;
        }
    }
    prog.working_domain = requires_float_rgb ? PixelDomain::RGBA16F : PixelDomain::NV12;

    // 2. Explicit Materialization Barriers
    for (const auto& lifetime : prog.compiled->lifetimes) {
        if (lifetime.producer >= prog.compiled->nodes.size()) continue;
        const auto& pnode = prog.compiled->nodes[lifetime.producer];
        MaterializationReason reason = MaterializationReason::NeighborhoodOperation;
        if (pnode.kind == RenderGraphNodeKind::Video) {
            reason = MaterializationReason::ExternalInterop;
        } else if (pnode.kind == RenderGraphNodeKind::MotionBlur) {
            reason = MaterializationReason::NeighborhoodOperation;
        } else if (pnode.kind == RenderGraphNodeKind::Mask) {
            reason = MaterializationReason::DependentMask;
        } else if (pnode.kind == RenderGraphNodeKind::ColorConvert) {
            reason = MaterializationReason::ColorDomainTransition;
        } else {
            reason = MaterializationReason::BackendCapability;
        }
        MaterializationBarrier barrier{
            .reason = reason,
            .producer = lifetime.producer,
            .consumer = k_invalid_node,
            .domain = prog.working_domain,
            .lifetime_start_level = static_cast<std::uint32_t>(lifetime.first_level),
            .lifetime_end_level = static_cast<std::uint32_t>(lifetime.last_level),
            .physical_slot = 0,
            .detail = pnode.name,
        };
        prog.materializations.push_back(barrier);
    }

    // 3. Primitive Batches
    for (std::size_t b = 0; b < prog.batches.size(); ++b) {
        const auto& batch = prog.batches[b];
        PrimitiveBatch pb{
            .batch_id = static_cast<std::uint32_t>(b),
            .primitive_offset = static_cast<std::uint32_t>(prog.primitives.headers.size()),
            .primitive_count = static_cast<std::uint32_t>(batch.member_nodes.size()),
            .domain = prog.working_domain,
            .target_slot = batch.output_physical_slot,
            .requires_barrier_after = false,
        };
        for (const auto nid : batch.member_nodes) {
            GpuPrimitiveHeader hdr{
                .type = PrimitiveType::Rect,
                .flags = 0,
                .clip_index = 0,
                .transform_index = 0,
                .resource_index = static_cast<std::uint32_t>(nid),
                .parameter_index = 0,
            };
            prog.primitives.headers.push_back(hdr);
        }
        prog.primitives.batches.push_back(pb);
    }
