...
static raster::BBox compute_layer_bbox(const LayerGraphItem& item, const RenderGraphContext& ctx, SoftwareRenderer* renderer) {
    ...
    // For 2.5D projected layers, the effective screen-space matrix depends on the
    // per-layer projection_matrix which is not propagated per-node here.
    // Use the full canvas as a conservative bbox to avoid false culling.
    if (item.projected) {
        return raster::BBox{0, 0, ctx.width, ctx.height};
    }
