...
void append_lighting_pass_if_needed(RenderGraph& graph, GraphNodeId& layer_output,
    ...
    if (!ctx.camera.camera.light_context.enabled) {
        return;
    }
    if (item.native_3d) {
        return;
...
void append_depth_grade_pass_if_needed(RenderGraph& graph, GraphNodeId& layer_output,
    ...
    if (!ctx.camera.camera.light_context.enabled) return;
    ...
    const RenderGraphContext& ctx)
{
    ...
    if (!ctx.camera.camera.light_context.enabled || !ctx.camera.camera.light_context.directional_enabled) return;
    ...
    for (const auto& caster : casters) {
        ...
        auto shadow_id = graph.add_node(ShadowNode::create(
            ...
            ctx.camera.camera.light_context.direction,
            ctx.camera.camera.light_context.shadows
        ));
