...
void LayerPipelinePass::run(GraphBuildContext& ctx) {
    ...
    auto append_item = [&](LayerGraphItem item,
        ...
        auto* renderer = dynamic_cast<SoftwareRenderer*>(rctx.backend);
        ...
        if (bbox.is_empty()) {
            ...
            bool intersects = !(bbox.x1 <= 0 || bbox.x0 >= rctx.width ||
                                bbox.y1 <= 0 || bbox.y0 >= rctx.height);
            if (!intersects) {
                is_culled = true;
        ...
        if (rctx.counters) {
            rctx.counters->layer_culling_tests.fetch_add(1, std::memory_order_relaxed);
            if (is_culled) {
                rctx.counters->layers_culled.fetch_add(1, std::memory_order_relaxed);
            } else {
                rctx.counters->layers_visible.fetch_add(1, std::memory_order_relaxed);
            }
        }

        // Record culling telemetry
        telemetry::CullingTelemetryRecord cull_rec;
        cull_rec.frame_number = static_cast<int>(rctx.frame);
        ...
        if (!is_culled) {
            ...
            visible_bbox.clip_to(rctx.width, rctx.height);
        ...
        if (item.layer->track_matte.active()) {
            ...
            if (it != ctx.name_to_resolved.end()
                && it->second->layer->active_at(rctx.frame)) {
    ...
    for (const auto& resolved_layer : ctx.resolved.layers) {
        ...
        if (!layer.active_at(rctx.frame)) continue;
        ...
        if (cam25d.enabled && layer.uses_2_5d_projection) {
            ...
            auto proj = project_layer_2_5d(
                ...
                static_cast<f32>(rctx.width), static_cast<f32>(rctx.height),
                rctx.diagnostics_enabled);
            if (proj.visible) {
