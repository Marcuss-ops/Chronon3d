    ...
    for (GraphNodeId i = 0; i < static_cast<GraphNodeId>(graph.size()); ++i) {
        ...
        if (isect_area == 0) {
            ...
            rec.visibility      = VisibilityStatus::OutsideCanvas;
                   ...
                   rec.predicted_bbox.x1 >= ctx.frame.frame.width &&
                   rec.predicted_bbox.y1 >= ctx.frame.frame.height) {
        ...
        if (w > 0 && h > 0) {
            ...
            mem_bytes = static_cast<size_t>(ctx.frame.frame.width * ctx.frame.frame.height * 16);
        }
