...
// analysis.cpp
//
// Output/formatting functions for preflight render graph analysis:
// format_memory, report query methods, enriched DOT generation,
// text report output, layer node selection.
//
// Core analysis phases have been extracted to:
//   analysis_graph.cpp      — node population, topo sort, peak memory, metrics
//   analysis_diagnostics.cpp — warnings, asset check, coordinate mismatch, dirty chains
//   analysis_helpers.hpp    — shared internal helpers (bbox, shape complexity, asset checks)
// ---------------------------------------------------------------------------
...
std::string build_enriched_dot(
    ...
    for (GraphNodeId i = 0; i < static_cast<GraphNodeId>(graph.size()); ++i) {
        ...
        switch (node.kind()) {
            ...
            case RenderGraphNodeKind::TextRun:   color = "#A9CCE3"; break;
