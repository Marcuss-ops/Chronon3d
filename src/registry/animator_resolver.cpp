...
//   `scene/builders/*.hpp` files that drag content-side state.  The
//   canonical TextSpec type is pulled in via the single include of
//   `<chronon3d/registry/animator_resolver.hpp>`, which the header
//   already declared #include-ing `<chronon3d/scene/builders/builder_params.hpp>`
//   to expose TextSpec + TextAnimatorSpec.
// ==============================================================================
#include <chronon3d/registry/animator_resolver.hpp>  // struct AnimatorResolver + types from builder_params.hpp

namespace chronon3d::registry {

// ── spec_is_rich — Stage 4 contract ──────────────────────────────────────────
...
// ── rich_paint_anchor — Stage 4 contract ─────────────────────────────────────
...
// ── compose_for — Stage 5 (Cluster A DoD #2 closure — all 22 presets) ────────
// Maps each of the 22 built-in motion presets to a TextAnimatorSpec
// capturing the END-STATE glyph contribution of the canonical motion
// chain.  When the caller invokes the factory body of preset P, the
// registry routes through this resolver:
//
//   1. wire_through_resolver() (text_preset_registry.cpp) calls
//      `compose_for(P).value_or(nullopt)` to obtain the property-
//      composed TextAnimatorSpec;
//   2. if non-null, the helper pushes it onto TextRunParams.animators
//      and routes through `lb.text_run(...).commit()`;
//   3. if null (no branch matched — `minimal_white` OR any unknown id)
//      the helper falls back to a plain `lb.text(...)` — no canonical
//      motion mapped → no resolver output.
//
// The 22 branches encode each preset's terminal glyph state as
// END-STATE static values; ramps are produced by the layer-level
// motion canon that runs AFTER the resolver.  Stagger / center-split /
// float-idle do NOT have a static end-state property and therefore
// route via the canonical Opacity/Position path.
...
AnimatorResolver::compose_for(std::string_view preset_id) {
    ...
    // Global glyph selector — every glyph receives weight=1.
    GlyphSelectorSpec sel;
    sel.id     = a.id + "_sel_global";
    sel.unit   = TextSelectorUnit::Glyph;
    sel.shape  = TextSelectorShape::Square;
