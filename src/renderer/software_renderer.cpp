...
bool hit_test(const Shape& s, Vec2 p, f32 spread = 0.0f) {
    ...
    switch (s.type) {
        ...
        case ShapeType::GridPlane:
            return false; // custom rendering path — no per-pixel hit test
...
// Project a world-space point through the 2.5D camera (view matrix + focal length).
// Returns screen-space (x, y) and sets ok=false if the point is behind the camera.
static Vec2 project_2_5d(const Vec3& wp, const Mat4& view, f32 focal, f32 vp_cx, f32 vp_cy, bool& ok) {
...
// Scanline fill of a convex quad. Vertices must wind CW or CCW — works for either.
static void fill_convex_quad(Framebuffer& fb, const Vec2 v[4], const Color& color) {
    if (color.a <= 0.0f) return;
...
// Bresenham line for the grid plane (avoids depending on SoftwareRenderer::draw_line member).
static void bline(Framebuffer& fb, Vec2 p0, Vec2 p1, const Color& color) {
    if (color.a <= 0.0f) return;
...
static void draw_fake_box3d(Framebuffer& fb, const RenderNode& node, const RenderState& state) {
    ...
    // 8 corners: front face at z=wp.z, back face at z=wp.z+depth (away from camera).
    // World Y-up: higher Y = higher visually.
    Vec3 c[8] = {
    ...
    // Camera world position for face visibility (backface culling)
    ...
    // Face definitions: corner indices (CW in front-view) and outward normals.
    struct FaceDef { int idx[4]; Vec3 normal; };
    constexpr int NFACES = 6;
    const FaceDef faces[NFACES] = {
        {{0,1,2,3}, { 0, 0,-1}},  // front  — faces camera (-Z)
        {{5,4,7,6}, { 0, 0,+1}},  // back
        {{4,5,1,0}, { 0,+1, 0}},  // top    (+Y up)
    ...
    for (int oi = 0; oi < NFACES; ++oi) {
        ...
        if (glm::dot(to_cam, faces[fi].normal) <= 0.0f) continue;  // backface cull
...
// Project and optionally clip a line segment to the camera near plane.
// Returns true if at least part of the line is visible, with clipped endpoints in p0/p1.
static bool clip_and_project_line(const Vec3& w0, const Vec3& w1,
                                   const Mat4& view, f32 focal, f32 vp_cx, f32 vp_cy,
                                   Vec2& p0, Vec2& p1) {
    Vec4 c0 = view * Vec4(w0, 1.0f);
    Vec4 c1 = view * Vec4(w1, 1.0f);
    constexpr f32 near = 0.5f;  // near-plane clip distance

    // Both behind
    if (c0.z >= -near && c1.z >= -near) return false;

    // Clip endpoint that is behind the near plane
    if (c0.z >= -near) {
        // Interpolate c0 to near plane
...
static void draw_grid_plane(Framebuffer& fb, const RenderNode& node, const RenderState& state) {
...
[[nodiscard]] u64 hash_shape(const Shape& shape) {
    ...
    switch (shape.type) {
        ...
        case ShapeType::FakeBox3D:
            ...
            // cam_* fields excluded: camera state is already covered by scene_hash
            break;
        case ShapeType::GridPlane:
