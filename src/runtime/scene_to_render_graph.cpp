...
graph::RenderGraph build_render_graph_from_scene(
    ...
    bridge_ctx.camera.light_context = scene.light_context();
    
    const auto resolved_camera = graph::resolve_scene_camera(scene);
    if (resolved_camera.camera.enabled) {
        bridge_ctx.camera.camera_2_5d = resolved_camera.camera;
        bridge_ctx.camera.has_camera_2_5d = true;
        bridge_ctx.camera.projection_ctx = renderer::make_projection_context(
            bridge_ctx.camera.camera_2_5d,
            bridge_ctx.frame.width,
            bridge_ctx.frame.height
        );
        bridge_ctx.camera.projection_ctx.ready = true;
    }
