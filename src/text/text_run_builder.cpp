...
compile_text_layout(
    ...
    // ── UnsupportedMultiFontRun check (verdict Issue #3 stabilization) ──
    // If the resolved-tree paragraph carries MORE THAN ONE run AND the
    // resolved FontSpec differs across runs (different font_path /
    // font_family / font_weight / font_style), the renderer cannot
    // safely use the single `text_layout->font` field with glyph IDs
    // generated by N different fonts — wrong-glyph corruption.  We
