...
VideoExecutionDecision resolve_video_execution(
    ...
    if (request.has_gop_source && request.gop_copy_only) {
        return {VideoExecutionPath::BitstreamCopy, VideoExecutionPath::DirectYuv,
                "explicit_gop_copy", true,
    ...
    if (request.has_gop_source && request.allow_hybrid_gop && native_nvenc) {
        ...
        return {VideoExecutionPath::SmartGopCopy, VideoExecutionPath::DirectYuv,
                "hybrid_gop_source", true,
