...
void draw_diagnostic_overlay(const OverlayContext& ctx, const CameraDiagnosticOverlay& diag) {
    auto& l = ctx.layer;
    ...
    if (diag.show_center_cross) {
        ...
        l.text("diag_center_lbl", TextSpec{
            .content = {.value = "(SCREEN)"},
            .font = {.font_size = 8.0f},
            .appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.15f}},
            .position = {center.x + 20.0f, center.y + 4.0f, 0.0f}
        });
    }
    ...
    if (diag.show_target_marker) {
        ...
        if (has_target) {
            ...
            if (!sp.behind_camera) {
                ...
                if (err > 3.0f) {
                    tc = Color{1.0f, 0.2f, 0.2f, 0.85f};
                } else if (err > 1.5f) {
                    tc = Color{1.0f, 0.8f, 0.0f, 0.85f};
                }
                ...
                l.text("diag_target_err", TextSpec{
                    .content = {.value = oss.str()},
                    .font = {.font_size = 10.0f},
                    .appearance = {.color = tc},
                    .position = {sp.position.x + 22.0f, sp.position.y + 6.0f, 0.0f}
                });
            }
        }
    }
    ...
    if (diag.show_camera_axes) {
        ...
        auto draw_axis = [&](const char* id, const Vec3& dir, const Color& col, const char* label) {
            ...
            l.text(std::string("diag_axis_lbl_") + id, TextSpec{
                .content = {.value = label},
                .font = {.font_size = 11.0f},
                .appearance = {.color = col},
                .position = {sp_tip.position.x + 4.0f, sp_tip.position.y + 4.0f, 0.0f}
            });
        };

        l.text("diag_axes_lbl", TextSpec{
            .content = {.value = "(WORLD)"},
            .font = {.font_size = 8.0f},
            .appearance = {.color = Color{0.5f, 0.5f, 0.5f, 0.15f}},
    ...
    if (diag.show_projected_bbox) {
        ...
        for (const auto& lr : ctx.report.layers) {
            ...
            Color bc = lr.passed ? Color{0.2f, 0.9f, 0.2f, 0.45f} : Color{1.0f, 0.2f, 0.2f, 0.55f};
            ...
            std::ostringstream oss;
            oss << lr.name << "  " << std::fixed << std::setprecision(0) << (lr.bounds.visible_ratio * 100.0f) << "%";
            if (!lr.passed) oss << " FAIL";

            l.text("diag_bbox_lbl_" + std::to_string(idx), TextSpec{
                .content = {.value = oss.str()},
                .font = {.font_size = 10.0f},
                .appearance = {.color = bc},
                .position = {lr.bounds.min.x + 4.0f, lr.bounds.min.y + 14.0f, 0.0f}
            });
    ...
    if (diag.show_metrics_text) {
        ...
        auto draw_line = [&](const std::string& text, const Color& col = Color{1, 1, 1, 1}) {
            l.text("diag_metric_" + std::to_string(static_cast<int>(panel_y)), TextSpec{
                .content = {.value = text},
                .font = {.font_size = 13.0f},
                .layout = {.align = TextAlign::Left},
                .appearance = {.color = col},
                .position = {panel_x, panel_y, 0.0f}
            });
            panel_y += line_h;
        };

        Color hdr_col = ctx.report.passed ? Color{0.2f, 0.9f, 0.2f, 1.0f} : Color{1.0f, 0.2f, 0.2f, 1.0f};
        ...
        {
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(0) << "Cam: (" << ctx.camera.position.x << ", " << ctx.camera.position.y << ", " << ctx.camera.position.z << ")";
...

        {
            ...
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(0) << "Tgt: (" << tw.x << ", " << tw.y << ", " << tw.z << ")";
            draw_line(oss.str(), Color{0.8f, 0.7f, 0.4f, 0.8f});
        }

        {
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(2) << "Tgt err: " << ctx.report.target_center_error_px << " px";
            Color ec = ctx.report.target_center_error_px <= 3.0f ? Color{0.2f, 0.9f, 0.2f, 0.8f} : Color{1.0f, 0.6f, 0.0f, 0.8f};
...

        {
            ...
            Color qc = (std::abs(norm - 1.0f) < 1e-4f) ? Color{0.2f, 0.9f, 0.2f, 0.8f} : Color{1.0f, 0.2f, 0.2f, 0.8f};
...

        {
            ...
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(1) << "Vis ratio: " << (avg_vis * 100.0f) << "%";
            Color vc = avg_vis > 0.005f ? Color{0.2f, 0.9f, 0.2f, 0.8f} : Color{1.0f, 0.2f, 0.2f, 0.8f};
