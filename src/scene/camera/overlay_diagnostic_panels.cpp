...
void draw_diagnostic_overlay(
    ...
    if (diag.show_target_marker) {
        ...
        if (has_target) {
            ...
            if (!sp.behind_camera) {
                ...
                l.text("diag_target_err", TextParams{
                    .text = oss.str(),
                    .pos  = {sp.position.x + 22.0f, sp.position.y + 6.0f, 0.0f},
                    .font_size = 10.0f, .color = tc
                });
            }
        }
    }
    ...
    if (diag.show_camera_axes) {
        ...
        auto draw_axis = [&](const char* id, const Vec3& dir, const Color& col, const char* label) {
            ...
            l.text(std::string("diag_axis_lbl_") + id, TextParams{
                .text = label,
                .pos  = {sp_tip.position.x + 4.0f, sp_tip.position.y + 4.0f, 0.0f},
                .font_size = 11.0f, .color = col
            });
        };
    ...
    if (diag.show_projected_bbox) {
        ...
        for (const auto& lr : ctx.report.layers) {
            ...
            l.text("diag_bbox_lbl_" + std::to_string(idx), TextParams{
                .text = oss.str(),
                .pos  = {lr.bounds.min.x + 4.0f, lr.bounds.min.y + 14.0f, 0.0f},
                .font_size = 10.0f, .color = bc
            });
            idx++;
        }
    }
    ...
    if (diag.show_metrics_text) {
        ...
        auto draw_line = [&](const std::string& text, const Color& col = Color{1,1,1,1}) {
            l.text("diag_metric_" + std::to_string(static_cast<int>(panel_y)), TextParams{
                .text = text, .pos = {panel_x, panel_y, 0.0f},
                .font_size = 13.0f, .color = col, .align = TextAlign::Left
            });
            panel_y += line_h;
        };
