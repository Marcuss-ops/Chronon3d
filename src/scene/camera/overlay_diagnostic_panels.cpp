...
void draw_diagnostic_overlay(
    ...
    if (diag.show_center_cross) {
        ...
        l.text("diag_center_lbl", TextSpec{.content = {.value = "(SCREEN)"},.placement = {TextPlacementKind::Absolute, {center.x + 20.0f, center.y + 4.0f}},.font = {.font_size = 8.0f},.appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.15f}}});
    }
    ...
    if (diag.show_target_marker) {
        ...
        if (has_target) {
            ...
            if (!sp.behind_camera) {
                ...
                l.text("diag_target_err", TextSpec{.content = {.value = oss.str()},.placement = {TextPlacementKind::Absolute, {sp.position.x + 22.0f, sp.position.y + 6.0f}},.font = {.font_size = 10.0f},.appearance = {.color = tc}});
            }
        }
    }
    ...
    if (diag.show_camera_axes) {
        ...
        auto draw_axis = [&](const char* id, const Vec3& dir, const Color& col, const char* label) {
            ...
            l.text(std::string("diag_axis_lbl_") + id, TextSpec{.content = {.value = label},.placement = {TextPlacementKind::Absolute, {sp_tip.position.x + 4.0f, sp_tip.position.y + 4.0f}},.font = {.font_size = 11.0f},.appearance = {.color = col}});
        };

        l.text("diag_axes_lbl", TextSpec{.content = {.value = "(WORLD)"},.placement = {TextPlacementKind::Absolute, {center.x + 20.0f, center.y - 12.0f}},.font = {.font_size = 8.0f},.appearance = {.color = Color{0.5f, 0.5f, 0.5f, 0.15f}}});
    ...
    if (diag.show_projected_bbox) {
        ...
        for (const auto& lr : ctx.report.layers) {
            ...
            l.text("diag_bbox_lbl_" + std::to_string(idx), TextSpec{.content = {.value = oss.str()},.placement = {TextPlacementKind::Absolute, {lr.bounds.min.x + 4.0f, lr.bounds.min.y + 14.0f}},.font = {.font_size = 10.0f},.appearance = {.color = bc}});
            idx++;
        }
    }
    ...
    if (diag.show_metrics_text) {
        ...
        auto draw_line = [&](const std::string& text, const Color& col = Color{1,1,1,1}) {
            l.text("diag_metric_" + std::to_string(static_cast<int>(panel_y)), TextSpec{.content = {.value = text},.placement = {TextPlacementKind::Absolute, {panel_x, panel_y}},.font = {.font_size = 13.0f},.layout = {.align = TextAlign::Left},.appearance = {.color = col}});
            panel_y += line_h;
        };
