...
void draw_diagnostic_overlay(
    ...
    if (diag.show_center_cross) {
        ...
        l.text("diag_center_lbl", TextDefinition{
    .content = {.value = "(SCREEN)"},
    .style = {
        .font = {.font_size = 8.0f},
        .color = Color{1.0f, 1.0f, 1.0f, 0.15f}
    },
    .frame = {
        .placement = TextPlacement{TextPlacementKind::Absolute, {center.x + 20.0f, center.y + 4.0f}}
    }
});
    }
    ...
    if (diag.show_target_marker) {
        ...
        if (has_target) {
            ...
            if (!sp.behind_camera) {
                ...
                l.text("diag_target_err", TextDefinition{
    ...
    .frame = {
        .placement = TextPlacement{TextPlacementKind::Absolute, {sp.position.x + 22.0f, sp.position.y + 6.0f}}
    }
});
            }
        }
    ...
    if (diag.show_camera_axes) {
        ...
        auto draw_axis = [&](const char* id, const Vec3& dir, const Color& col, const char* label) {
            ...
            l.text(std::string("diag_axis_lbl_") + id, TextDefinition{
    .content = {.value = label},
    .style = {
    ...
    .frame = {
        .placement = TextPlacement{TextPlacementKind::Absolute, {sp_tip.position.x + 4.0f, sp_tip.position.y + 4.0f}}
    }
});
        };

        l.text("diag_axes_lbl", TextDefinition{
    .content = {.value = "(WORLD)"},
    .style = {
        .font = {.font_size = 8.0f},
        .color = Color{0.5f, 0.5f, 0.5f, 0.15f}
    },
    .frame = {
    ...
    if (diag.show_projected_bbox) {
        ...
        for (const auto& lr : ctx.report.layers) {
            ...
            l.text("diag_bbox_lbl_" + std::to_string(idx), TextDefinition{
    .content = {.value = oss.str()},
    .style = {
        .font = {.font_size = 10.0f},
        .color = bc
    ...
    .frame = {
        .placement = TextPlacement{TextPlacementKind::Absolute, {lr.bounds.min.x + 4.0f, lr.bounds.min.y + 14.0f}}
    }
});
            idx++;
        }
    ...
    if (diag.show_metrics_text) {
        ...
        auto draw_line = [&](const std::string& text, const Color& col = Color{1,1,1,1}) {
            l.text("diag_metric_" + std::to_string(static_cast<int>(panel_y)), TextDefinition{
    .content = {.value = text},
    .style = {
    ...
    .frame = {
        .placement = TextPlacement{TextPlacementKind::Absolute, {panel_x, panel_y}},
