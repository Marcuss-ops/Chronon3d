...
void draw_jerk_graph(const OverlayContext& ctx) {
    ...
    l.text("graph_title", TextParams{
        .text = "CAMERA PATH KINEMATIC JERK (" + std::to_string(ctx.path->current_frame) + "/" + std::to_string(ctx.path->total_frames) + ")",
        .pos = {graph_x + 10.0f, graph_y + 20.0f, 0.0f},
        .font_size = 11.0f, .color = Color{0.9f, 0.9f, 0.9f, 0.8f}
    });
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (i % 15 == 0) {
            ...
            l.text("marker_txt_" + std::to_string(i), TextParams{
                ...
                .font_size = 8.0f, .color = Color{0.7f, 0.7f, 0.7f, 0.6f}
            });
        }
    }
}
...
void draw_path_trace(const OverlayContext& ctx) {
    ...
    l.text("trace_title", TextParams{
        .text = "CAMERA 3D PATH TRACE (" + std::to_string(ctx.path->current_frame) + "/" + std::to_string(ctx.path->total_frames) + ")",
    ...
    if (!projected.empty() && !projected[0].behind) {
        ...
        l.text("trc_start_lbl", TextParams{.text = "START", .pos = {sp_start.x + 8.0f, sp_start.y - 4.0f, 0.0f}, .font_size = 9.0f, .color = Color{0.0f, 0.9f, 1.0f, 0.8f}});
    }
    ...
    for (size_t i = 0; i < n_proj; i += 15) {
        ...
        clamp_to_panel(mp);
        l.text("trc_frame_" + std::to_string(i), TextParams{.text = std::to_string(i), .pos = {mp.x - 4.0f, mp.y - 14.0f, 0.0f}, .font_size = 8.0f, .color = Color{0.6f, 0.6f, 0.7f, 0.5f}});
    }
