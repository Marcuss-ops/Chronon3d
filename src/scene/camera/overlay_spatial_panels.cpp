...
void draw_topdown_preview(const OverlayContext& ctx) {
    ...
    for (const auto& pair : ctx.resolved.resolved) {
        ...
        if (!is_null && td_idx < 8) { l.text("td_lbl_" + std::to_string(td_idx), TextParams{.text = pair.first, .pos = {screen.x + 5.0f, screen.y - 4.0f, 0.0f}, .font_size = 7.0f, .color = Color{0.7f, 0.75f, 0.9f, 0.55f}}); }
        td_idx++;
    }
...
void draw_sideview_depth(const OverlayContext& ctx) {
    ...
    for (const auto& pair : ctx.resolved.resolved) {
        ...
        if (!is_null && sv_idx < 6) { l.text("sv_lbl_" + std::to_string(sv_idx), TextParams{.text = pair.first, .pos = {screen.x + 10.0f, screen.y - 3.0f, 0.0f}, .font_size = 7.0f, .color = Color{0.7f, 0.9f, 0.8f, 0.55f}}); }
        sv_idx++;
    }
