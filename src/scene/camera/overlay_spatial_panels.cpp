...
void draw_topdown_preview(const OverlayContext& ctx) {
    ...
    l.text("topdown_title", TextSpec{.content = {.value = "TOP-DOWN VIEW (XZ)"},.position = {td_x + 10.0f, td_y + 16.0f, 0.0f},.font = {.font_size = 10.0f},.appearance = {.color = Color{0.8f, 0.85f, 1.0f, 0.8f}}});
    ...
    l.text("td_axis_x", TextSpec{.content = {.value = "+X"},.position = {td_x + td_w - draw_margin - 14.0f, draw_y0 + draw_h + 2.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.4f, 0.4f, 0.5f}}});
    l.text("td_axis_z", TextSpec{.content = {.value = "+Z"},.position = {td_x + draw_margin - 2.0f, draw_y0 - 6.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.4f, 0.4f, 0.6f, 0.5f}}});
    ...
    for (size_t pair_idx = 0; pair_idx < ctx.resolved.size(); ++pair_idx) {
        ...
        if (!is_null && td_idx < 8) { l.text("td_lbl_" + std::to_string(td_idx), TextSpec{.content = {.value = pair_name},.position = {screen.x + 5.0f, screen.y - 4.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.7f, 0.75f, 0.9f, 0.55f}}}); }
        td_idx++;
    }
    ...
    if (cam_screen.x >= td_x + draw_margin && cam_screen.x <= td_x + td_w - draw_margin && cam_screen.y >= draw_y0 && cam_screen.y <= draw_y0 + draw_h) {
        ...
        l.text("td_cam_lbl", TextSpec{.content = {.value = "CAM"},.position = {cam_screen.x + 7.0f, cam_screen.y - 8.0f, 0.0f},.font = {.font_size = 8.0f},.appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.8f}}});
    }

    l.circle("td_leg_near", CircleParams{.radius = 3.0f, .color = Color{0.4f, 0.9f, 1.0f, 0.85f}, .pos = {td_x + 10.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_near_t", TextSpec{.content = {.value = "near"},.position = {td_x + 16.0f, td_y + td_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}});
    l.circle("td_leg_far", CircleParams{.radius = 3.0f, .color = Color{0.3f, 0.5f, 0.9f, 0.75f}, .pos = {td_x + 52.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_far_t", TextSpec{.content = {.value = "far"},.position = {td_x + 58.0f, td_y + td_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}});
    l.circle("td_leg_cam", CircleParams{.radius = 3.0f, .color = Color{1.0f, 1.0f, 1.0f, 0.9f}, .pos = {td_x + 88.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_cam_t", TextSpec{.content = {.value = "cam"},.position = {td_x + 94.0f, td_y + td_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}});
}
...
void draw_sideview_depth(const OverlayContext& ctx) {
    ...
    l.text("sv_title", TextSpec{.content = {.value = "DEPTH SIDE VIEW (X vs Z)"},.position = {sv_x + 10.0f, sv_y + 16.0f, 0.0f},.font = {.font_size = 10.0f},.appearance = {.color = Color{0.8f, 1.0f, 0.85f, 0.8f}}});
    ...
    if (w_min_z < 0.0f && w_max_z > 0.0f) {
        ...
        l.text("sv_z0_lbl", TextSpec{.content = {.value = "Z=0"},.position = {sv_x + sv_w - d_margin + 2.0f, z0_y - 4.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.5f, 0.8f, 0.5f, 0.5f}}});
    }

    l.text("sv_axis_x", TextSpec{.content = {.value = "+X"},.position = {sv_x + sv_w - d_margin - 14.0f, d_y0 + d_h + 2.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.5f, 0.4f, 0.5f}}});
    l.text("sv_axis_z", TextSpec{.content = {.value = "+Z"},.position = {sv_x + d_margin - 2.0f, d_y0 - 6.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.4f, 0.6f, 0.4f, 0.5f}}});
    ...
    for (size_t pair_idx = 0; pair_idx < ctx.resolved.size(); ++pair_idx) {
        ...
        if (!is_null && sv_idx < 6) { l.text("sv_lbl_" + std::to_string(sv_idx), TextSpec{.content = {.value = pair_name},.position = {screen.x + 10.0f, screen.y - 3.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.7f, 0.9f, 0.8f, 0.55f}}}); }
        sv_idx++;
    }
    ...
    if (cam_sv.x >= sv_x + d_margin && cam_sv.x <= sv_x + sv_w - d_margin && cam_sv.y >= d_y0 && cam_sv.y <= d_y0 + d_h) {
        ...
        l.text("sv_cam_lbl", TextSpec{.content = {.value = "CAM"},.position = {cam_sv.x + 7.0f, cam_sv.y - 4.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.8f}}});
    }

    l.circle("sv_leg_pass", CircleParams{.radius = 3.0f, .color = Color{0.2f, 0.9f, 0.3f, 0.85f}, .pos = {sv_x + 10.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_pass_t", TextSpec{.content = {.value = "pass"},.position = {sv_x + 16.0f, sv_y + sv_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.7f, 0.65f, 0.5f}}});
    l.circle("sv_leg_fail", CircleParams{.radius = 3.0f, .color = Color{1.0f, 0.3f, 0.15f, 0.85f}, .pos = {sv_x + 50.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_fail_t", TextSpec{.content = {.value = "fail"},.position = {sv_x + 56.0f, sv_y + sv_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.7f, 0.6f, 0.6f, 0.5f}}});
    l.circle("sv_leg_cam", CircleParams{.radius = 3.0f, .color = Color{1.0f, 1.0f, 1.0f, 0.9f}, .pos = {sv_x + 88.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_cam_t", TextSpec{.content = {.value = "cam"},.position = {sv_x + 94.0f, sv_y + sv_h - 12.0f, 0.0f},.font = {.font_size = 7.0f},.appearance = {.color = Color{0.6f, 0.7f, 0.65f, 0.5f}}});
}
