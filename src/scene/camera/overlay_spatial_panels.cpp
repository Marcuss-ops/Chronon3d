...
void draw_topdown_preview(const OverlayContext& ctx) {
    ...
    l.text("td_axis_x", TextSpec{.content = {.value = "+X"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.4f, 0.4f, 0.5f}}, .position = {td_x + td_w - draw_margin - 14.0f, draw_y0 + draw_h + 2.0f, 0.0f}});
    ...
    for (const auto& pair : ctx.resolved.resolved) {
        ...
        for (const auto& lr : ctx.report.layers) { if (lr.name == pair.first) { layer_color = lr.passed ? Color{0.2f, 0.9f, 0.2f, 0.85f} : Color{1.0f, 0.3f, 0.15f, 0.85f}; break; } }
        ...
        if (!is_null && td_idx < 8) { l.text("td_lbl_" + std::to_string(td_idx), TextSpec{.content = {.value = pair.first}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.7f, 0.75f, 0.9f, 0.55f}}, .position = {screen.x + 5.0f, screen.y - 4.0f, 0.0f}}); }
        td_idx++;
    }
    ...
    if (cam_screen.x >= td_x + draw_margin && cam_screen.x <= td_x + td_w - draw_margin && cam_screen.y >= draw_y0 && cam_screen.y <= draw_y0 + draw_h) {
        ...
        l.text("td_cam_lbl", TextSpec{.content = {.value = "CAM"}, .font = {.font_size = 8.0f}, .appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.8f}}, .position = {cam_screen.x + 7.0f, cam_screen.y - 8.0f, 0.0f}});
    }

    l.circle("td_leg_near", CircleParams{.radius = 3.0f, .color = Color{0.4f, 0.9f, 1.0f, 0.85f}, .pos = {td_x + 10.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_near_t", TextSpec{.content = {.value = "near"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}, .position = {td_x + 16.0f, td_y + td_h - 12.0f, 0.0f}});
    l.circle("td_leg_far", CircleParams{.radius = 3.0f, .color = Color{0.3f, 0.5f, 0.9f, 0.75f}, .pos = {td_x + 52.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_far_t", TextSpec{.content = {.value = "far"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}, .position = {td_x + 58.0f, td_y + td_h - 12.0f, 0.0f}});
    l.circle("td_leg_cam", CircleParams{.radius = 3.0f, .color = Color{1.0f, 1.0f, 1.0f, 0.9f}, .pos = {td_x + 88.0f, td_y + td_h - 8.0f, 0.0f}});
    l.text("td_leg_cam_t", TextSpec{.content = {.value = "cam"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.6f, 0.7f, 0.5f}}, .position = {td_x + 94.0f, td_y + td_h - 12.0f, 0.0f}});
}
...
void draw_sideview_depth(const OverlayContext& ctx) {
    ...
    if (w_min_z < 0.0f && w_max_z > 0.0f) {
        ...
        l.text("sv_z0_lbl", TextSpec{.content = {.value = "Z=0"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.5f, 0.8f, 0.5f, 0.5f}}, .position = {sv_x + sv_w - d_margin + 2.0f, z0_y - 4.0f, 0.0f}});
    }

    l.text("sv_axis_x", TextSpec{.content = {.value = "+X"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.5f, 0.4f, 0.5f}}, .position = {sv_x + sv_w - d_margin - 14.0f, d_y0 + d_h + 2.0f, 0.0f}});
    ...
    for (const auto& pair : ctx.resolved.resolved) {
        ...
        for (const auto& lr : ctx.report.layers) { if (lr.name == pair.first) { in_report = true; passed = lr.passed; break; } }
        bool is_null = pair.first.find("_null") != std::string::npos || pair.first.find("_parent") != std::string::npos;
        Color bar_color;
        ...
        if (!is_null && sv_idx < 6) { l.text("sv_lbl_" + std::to_string(sv_idx), TextSpec{.content = {.value = pair.first}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.7f, 0.9f, 0.8f, 0.55f}}, .position = {screen.x + 10.0f, screen.y - 3.0f, 0.0f}}); }
        sv_idx++;
    }
    ...
    if (cam_sv.x >= sv_x + d_margin && cam_sv.x <= sv_x + sv_w - d_margin && cam_sv.y >= d_y0 && cam_sv.y <= d_y0 + d_h) {
        ...
        l.text("sv_cam_lbl", TextSpec{.content = {.value = "CAM"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{1.0f, 1.0f, 1.0f, 0.8f}}, .position = {cam_sv.x + 7.0f, cam_sv.y - 4.0f, 0.0f}});
    }

    l.circle("sv_leg_pass", CircleParams{.radius = 3.0f, .color = Color{0.2f, 0.9f, 0.3f, 0.85f}, .pos = {sv_x + 10.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_pass_t", TextSpec{.content = {.value = "pass"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.7f, 0.65f, 0.5f}}, .position = {sv_x + 16.0f, sv_y + sv_h - 12.0f, 0.0f}});
    l.circle("sv_leg_fail", CircleParams{.radius = 3.0f, .color = Color{1.0f, 0.3f, 0.15f, 0.85f}, .pos = {sv_x + 50.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_fail_t", TextSpec{.content = {.value = "fail"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.7f, 0.6f, 0.6f, 0.5f}}, .position = {sv_x + 56.0f, sv_y + sv_h - 12.0f, 0.0f}});
    l.circle("sv_leg_cam", CircleParams{.radius = 3.0f, .color = Color{1.0f, 1.0f, 1.0f, 0.9f}, .pos = {sv_x + 88.0f, sv_y + sv_h - 8.0f, 0.0f}});
    l.text("sv_leg_cam_t", TextSpec{.content = {.value = "cam"}, .font = {.font_size = 7.0f}, .appearance = {.color = Color{0.6f, 0.7f, 0.65f, 0.5f}}, .position = {sv_x + 94.0f, sv_y + sv_h - 12.0f, 0.0f}});
}
