...
#include <chronon3d/scene/builders/builder_params.hpp>  // P1 — PreparedText (PreparedText alias) for the migrated shape_ids::TextRun call site.
...
void draw_content(LayerBuilder& l, const MotionObject& obj, const MotionState& st, const std::string& layer_name) {
    ...
    case MotionObjectType::Text: {
        ...
        // direct caller now wraps the TextDefaults into a PreparedText (alias
        // PreparedText) and reaches the render graph via shape_ids::TextRun
        ...
        definition.frame.size = obj.size_value;
        definition.frame.align = obj.text_style.align == TextAlign::Left
            ? chronon3d::TextAlign::Left
            : obj.text_style.align == TextAlign::Right
                ? chronon3d::TextAlign::Right
                : chronon3d::TextAlign::Center;
        definition.frame.vertical_align = obj.text_style.vertical_align;
        definition.frame.wrap = TextWrap::Word;
        definition.frame.overflow = TextOverflow::Clip;
        definition.frame.line_height = obj.text_style.line_height;
        definition.frame.tracking = obj.text_style.tracking;
        definition.style.color = obj.color_value;
        chronon3d::PreparedText run_params = chronon3d::prepare_text(definition);
