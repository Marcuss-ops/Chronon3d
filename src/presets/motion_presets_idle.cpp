...
// `ctx.frame.frame` as a phase index (independent of `t`).
// =============================================================================
...
void register_idle_presets(MotionPresetRegistry& r) {
    r.register_preset({
        MotionPreset::ParallaxDrift, "ParallaxDrift", [](const FrameContext& ctx, const MotionObject& obj, f32, MotionState& st) {
            st.position.x += std::sin(static_cast<f32>(ctx.frame.frame) * 0.025f) * 20.0f * obj.motion3d.parallax;
            st.position.y += std::cos(static_cast<f32>(ctx.frame.frame) * 0.018f) * 12.0f * obj.motion3d.parallax;
            st.rotation.y += std::sin(static_cast<f32>(ctx.frame.frame) * 0.02f) * 4.0f;
            st.rotation.x += std::cos(static_cast<f32>(ctx.frame.frame) * 0.017f) * 2.5f;
        }
    });
