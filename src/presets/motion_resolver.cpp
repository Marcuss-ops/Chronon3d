...
MotionState resolve_motion_state(const FrameContext& ctx, const MotionObject& obj) {
    ...
    st.visible = obj.time_value.contains(ctx.frame.frame);
    ...
    if (obj.motion3d.sweep_2_5d.enabled) {
        const f32 wave = sweep_wave(ctx.frame.frame, obj.time_value.start, obj.motion3d.sweep_2_5d);
    ...
    const f32 t = obj.time_value.normalized(ctx.frame.frame);
