...
Composition create_100_images_scene(const std::string& image_path) {
    return composition({
        ...
        SceneBuilder s(ctx);
        for (int i = 0; i < 100; ++i) {
            float x = static_cast<float>((i % 10) * 192 - 864);
            float y = static_cast<float>((i / 10) * 108 - 486);
...
Composition create_100_unique_texts_scene() {
    return composition({
        .name = "MicroBench_100UniqueTexts",
        .width = 1920, .height = 1080,
        .frame_rate = {30, 1},
        .duration = 408,
    }, [](const FrameContext& ctx) -> Scene {
        ...
        for (int i = 0; i < 100; ++i) {
            float x = static_cast<float>((i % 10) * 192 - 864);
            float y = static_cast<float>((i / 10) * 108 - 486);
            std::string label = "Txt_" + std::to_string(i) + "_Unique";
            s.screen_layer("txt_" + std::to_string(i), [label, x, y](LayerBuilder& l) {
                l.text("label", text_preset(
                    label, 20.0f, 700,
                    {1.0f, 1.0f, 1.0f, 1.0f},
                    TextAlign::Center, {180.0f, 80.0f}, {x, y, 0.0f}
                ));
            });
        }
        return s.build();
